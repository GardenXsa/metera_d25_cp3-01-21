...
function syncPlayerContainerBindings() {
    if (!player) return;
    ...
    if (player.container_backpack && ContainerRegistry.has(player.container_backpack)) {
        ContainerRegistry.get(player.container_backpack).location = { ...playerBinding };
    }
    if (player.container_equipment && ContainerRegistry.has(player.container_equipment)) {
        ContainerRegistry.get(player.container_equipment).location = { ...playerBinding };
    }

    ContainerRegistry.forEach(cont => {
        if (cont.owner_id === 'player' && cont.type === 'magical_pocket') {
            cont.location = normalizeContainerLocation({ world_coords: null, parent_entity: 'player', parent_container: null, region_id: 'astral' });
        }
    });
}
...
function getOrCreateGroundPile(locationData) {
    ...
    if (existing) return existing.id;
    return CoreInventorySystem.createContainer('ground_pile', null, 9999, 999, normalized, {
        physical_props: { health: 100, flammable: false }
    });
}
...
function getGoldAmountInContainer(containerId) {
    ...
    const physicalGold = cont.items.reduce((sum, itemId) => {
        ...
        return sum + (item?.prototype_id === 'gold' ? item.stack_size : 0);
    }, 0);
...
const TradeSystem = {
        ...
        containerIds.forEach(containerId => {
            ...
            if (cont) {
                cont.items.forEach(itemId => {
                    if (ItemRegistry.get(itemId)?.prototype_id === 'gold') itemIds.add(itemId);
                });
            }
        });
        return {
    ...
    _transferMerchantGold: function(trade, createdItemIds) {
        ...
        const physicalGoldItems = [...targetContainer.items].filter(itemId => ItemRegistry.get(itemId)?.prototype_id === 'gold');
...
function buildFullPlayerSnapshot() {
    ...
    const nearbyContainers = Array.from(ContainerRegistry.values())
        ...
        const entityKey = args.aiIdentifier; // Явное объявление для обратной совместимости
            ...
            case 'initiate_trade': {
                ...
                const saleOfferItems = directItemId
                    ? [{ id: directItemId, quantity: args.quantity || 1 }]
            ...
            case 'fleePackAnimal':
                if (args.containerId) {
                    const cont = ContainerRegistry.get(resolveSpecialContainerId(args.containerId));
                    if (cont) {
                        cont.location = normalizeContainerLocation({ world_coords: [0, 0, 0], parent_entity: null, parent_container: null, region_id: "unknown_wilderness" });
                        feedback = `[СОБЫТИЕ] Вьючное животное испугалось и сбежало вместе с контейнером ${cont.id}!`;
                    } else {
                        feedback = `[ERROR] Контейнер не найден.`;
                    }
                } else {
