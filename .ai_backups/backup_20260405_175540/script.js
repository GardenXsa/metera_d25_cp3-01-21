...
async function generateVisionImage(prompt, btnElement, bubbleElement) {
    if (!imgApiKey) {
        showCustomAlert('Введите API ключ в настройках!');
        return;
    }
    ...
    let targetUrl = (imgApiProvider === 'llmost') ? 'https://llmost.ru/api/v1/chat/completions' : 'https://api.openai.com/v1/images/generations';
    if (imgApiProvider === 'openrouter') targetUrl = 'https://openrouter.ai/api/v1/images/generations';

    let body = (imgApiProvider === 'llmost') ? 
        { model: imgModelId, messages: [{ role: 'user', content: prompt }], modalities: ['text', 'image'] } : 
        { model: imgModelId, prompt: prompt, n: 1, size: "1024x1024" };
    ...
    try {
        ...
        console.log("RAW RESPONSE FROM LLMOST:", rawData);

        let data;
        try {
            data = JSON.parse(rawData);
        } catch(e) {
            throw new Error("Сервер прислал не JSON: " + rawData.substring(0, 100));
        }

        let foundImage = null;

        // --- ГЛУБОКИЙ ЖАДНЫЙ ПОИСК (РЕКУРСИВНЫЙ) ---
        const deepSearch = (obj) => {
            if (!obj || foundImage) return;
            if (typeof obj === 'string') {
                if (obj.startsWith('http') && (obj.includes('webp') || obj.includes('png') || obj.includes('jpg') || obj.includes('clouddlare') || obj.includes('storage'))) {
                    foundImage = { type: 'url', val: obj };
                } else if (obj.length > 500 && (obj.includes(';base64,') || /^[A-Za-z0-9+/=]+$/.test(obj.substring(0, 100)))) {
                    foundImage = { type: 'b64', val: obj.includes(';base64,') ? obj : 'data:image/png;base64,' + obj };
                }
            }
        ...
        if (foundImage) {
            let base64result = foundImage.val;
            if (foundImage.type === 'url') {
                const imgRes = await fetch(foundImage.val);
                const imgBlob = await imgRes.blob();
                base64result = await new Promise(r => { const rd = new FileReader(); rd.onloadend = () => r(rd.result); rd.readAsDataURL(imgBlob); });
            }
            
            // Отрисовка
            if (player && player.gameLogHistory) {
                for(let i = player.gameLogHistory.length - 1; i >= 0; i--) {
                    if(player.gameLogHistory[i].type === 'gm-message') {
                        player.gameLogHistory[i].savedImageBase64 = base64result;
                        break;
                    }
                }
            }
            ...
            imgContainer.innerHTML = `<img src="${base64result}" alt="Видение" onclick="window.open().document.write('<img src=\'${base64result}\'>')"><div class="img-watermark">Эфирный Слепок</div>`;
            bubbleElement.appendChild(imgContainer);
            btnElement.style.display = 'none';
            setTimeout(() => gameLog.scrollTo({ top: gameLog.scrollHeight, behavior: 'smooth' }), 300);

        } else {
            // ЕСЛИ НЕ НАШЛИ - ВЫВОДИМ ВЕСЬ ОТВЕТ ДЛЯ АНАЛИЗА
            console.error("DEEP SEARCH FAILED", data);
            alert("ВНИМАНИЕ! КАРТИНКА НЕ НАЙДЕНА. ВОТ ЧТО ПРИСЛАЛ СЕРВЕР:\n\n" + JSON.stringify(data).substring(0, 1000));
        }
    } catch (e) {
...
function renderGeneratedImageInChat(base64data, bubbleElement) {
    if (player && player.gameLogHistory) {
        for(let i = player.gameLogHistory.length - 1; i >= 0; i--) {
            if(player.gameLogHistory[i].type === 'gm-message') {
                player.gameLogHistory[i].savedImageBase64 = base64data;
                break;
            }
        }
    }
    const imgContainer = document.createElement('div');
    imgContainer.className = 'chat-illustration-container';
    imgContainer.innerHTML = `<img src="${base64data}" alt="Видение" onclick="window.open().document.write('<img src=\'${base64data}\'>')"><div class="img-watermark">Эфирный Слепок</div>`;
    bubbleElement.appendChild(imgContainer);
    setTimeout(() => gameLog.scrollTo({ top: gameLog.scrollHeight, behavior: 'smooth' }), 300);
}
...
async function initializeApp() {
    ...
    imgApiProvider = localStorage.getItem('imgApiProvider') || 'llmost';
...
function initSettingsUI() {
    ...
    // Load image settings
    document.getElementById('img-provider-select').value = imgApiProvider;
    document.getElementById('img-model-input').value = imgModelId;
    document.getElementById('img-key-input').value = imgApiKey;
...
function saveSettings() {
    ...
    // Save image settings
    imgApiProvider = document.getElementById('img-provider-select')?.value || 'llmost';
    ...
    if (player && player.currentJourney) {
        ...
        if ((player.currentCombat && player.currentCombat.isActive) || (player.currentJourney && player.currentJourney.isPausedForCheck)) {
            ...
            if (userInput) {
                ...
                userInput.placeholder = "Вы в пути... Нажмите 'Вперед', чтобы продолжить.";
            }
        }
    } else {
...
function addLogMessage(message, type = "gm-message", isRestoring = false, imagePrompt = "", savedImageBase64 = null) {
    ...
    if (!isRestoring && player) {
        ...
        player.gameLogHistory.push({ message, type, imagePrompt, savedImageBase64 });
    ...
    if (textToSpeak && textToSpeak.trim() !== '' && category !== 'system') {
        if (!bubbleElement.querySelector('.tts-controls-wrapper')) {
            ...
            if (savedImageBase64) {
                const imgContainer = document.createElement('div');
                imgContainer.className = 'chat-illustration-container';
                imgContainer.innerHTML = `<img src="${savedImageBase64}" alt="Эфирное видение" title="Нажми, чтобы открыть оригинал" onclick="const w = window.open(''); w.document.write('<style>body{margin:0;background:#000;display:flex;justify-content:center;align-items:center;height:100vh;}img{max-width:100%;max-height:100vh;object-fit:contain;}</style><img src=\'${savedImageBase64}\'>');"><div class="img-watermark">Эфирный Слепок</div>`;
                bubbleElement.appendChild(imgContainer);
                ...
                imgBtn.title = 'Визуализировать сцену';
                imgBtn.onclick = (e) => {
                    e.stopPropagation();
                    window.generateVisionImage(imagePrompt, imgBtn, bubbleElement);
                };
...
function handleUserInput() {
    ...
    if (DEBUG_MODE && text.startsWith('/dev ')) {
        ...
        if (cmd === 'turn') {
            ...
            addLogMessage(`[DEV] Пропущено ходов: ${skipAmount}. Текущий ход: ${player.stats.turnCount}`, "system-message");
            updateCharacterSheet();
        }
        else if (cmd === 'test') {
            runUnitTests();
...
// --- ГЕНЕРАЦИЯ ИЗОБРАЖЕНИЙ (ФИКС) ---
async function generateVisionImage(prompt, btnElement, bubbleElement) {
    if (!imgApiKey) {
        showCustomAlert('Введите API ключ для генерации изображений в настройках!');
        return;
    }
    const originalHtml = btnElement.innerHTML;
    btnElement.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
    ...
    let targetUrl = '';
    let headers = { 'Content-Type': 'application/json', 'Authorization': `Bearer ${imgApiKey}` };
    let body = (imgApiProvider === 'llmost') ? { model: imgModelId, messages: [{ role: 'user', content: prompt }], modalities: ['text', 'image'] } : { model: imgModelId, prompt: prompt, n: 1, size: "1024x1024" };

    if (imgApiProvider === 'llmost') targetUrl = 'https://llmost.ru/api/v1/chat/completions';
    else if (imgApiProvider === 'openrouter') targetUrl = 'https://openrouter.ai/api/v1/images/generations';
    else targetUrl = 'https://api.openai.com/v1/images/generations';

    try {
        const response = await fetch(targetUrl, { method: 'POST', headers: headers, body: JSON.stringify(body) });
        ...
        if (data.data && data.data[0] && data.data[0].url) {
            ...
            reader.onloadend = function() {
                ...
                const imgContainer = document.createElement('div');
                imgContainer.className = 'chat-illustration-container';
                imgContainer.innerHTML = `<img src="${base64data}" alt="Видение" onclick="const w = window.open(''); w.document.write('<img src=\'${base64data}\'>');"><div class="img-watermark">Эфирный Слепок</div>`;
                bubbleElement.appendChild(imgContainer);
                btnElement.style.display = 'none';
                setTimeout(() => gameLog.scrollTo({ top: gameLog.scrollHeight, behavior: 'smooth' }), 300);
            };
