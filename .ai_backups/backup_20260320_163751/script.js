...
function updateCharacterSheet() {
    ...
    const hpLine = document.querySelector('#character-stats .stat-line:has(#stat-hp)');
    if (hpLine) hpLine.title = `Базовое: ${calculateMaxHp(player.stats.con)}\n` + breakdown['hp'].map(b => `${b.name}: ${b.change > 0 ? '+' : ''}${b.change}`).join('\n');
    ...
    // Обновление основных характеристик с тултипами
    ...
    statsToUpdate.forEach(statKey => {
        ...
        let tooltip = `Базовое значение: ${base}`;
        if (breakdown[statKey].length > 0) {
            tooltip += "\n" + breakdown[statKey].map(b => `${b.name}: ${b.change > 0 ? '+' : ''}${b.change}`).join('\n');
        }
        statLine.title = tooltip;
