...
async function initMapTiles() {
    const promises = [];
    const tilePath = 'assets/assets/map/Forest/'; // Твой путь к тайлам
    for (let i = 1; i <= 30; i++) {
        promises.push(new Promise((resolve) => {
            const img = new Image();
            img.src = `${tilePath}${i}.png`;
            img.onload = () => { 
                mapTiles[i] = img; 
                resolve(); 
            };
            img.onerror = () => {
                console.warn(`Не удалось загрузить тайл: ${img.src}`);
                resolve(); 
            };
        }));
    }
    ...
    tilesLoaded = true;
    console.log("Система тайлов готова. Загружено: ", Object.keys(mapTiles).length);
}
...
function renderVisualMap() {
    ...
    const hexSize = TILE_SIZE * mapState.zoom;
    const hexW = hexSize * 2;
    const hexH = Math.sqrt(3) * hexSize;
    
    // Отрисовка лесного массива
    for (let row = -2; row < (height / (hexH * 0.75)) + 4; row++) {
        for (let col = -2; col < (width / hexW) + 4; col++) {
            ...
            const tileIdx = ((Math.abs(row * 13 + col * 19)) % 15) + 1;
            if (mapTiles[tileIdx]) {
    ...
    allPoints.forEach(p1 => {
        ...
        neighbors.forEach(n => {
            const startX = Number(p1.x) * mapState.zoom + mapState.offsetX;
            const startY = Number(p1.y) * mapState.zoom + mapState.offsetY;
            const endX = Number(n.point.x) * mapState.zoom + mapState.offsetX;
            const endY = Number(n.point.y) * mapState.zoom + mapState.offsetY;
            
            const steps = Math.max(5, Math.floor(n.dist / (TILE_SIZE * 0.5)));
            for (let i = 0; i <= steps; i++) {
                const tx = startX + (endX - startX) * (i / steps);
                const ty = startY + (endY - startY) * (i / steps);
                const roadIdx = 16 + (Math.abs(Math.floor(tx * 0.1 + ty * 0.1)) % 15);
                if (mapTiles[roadIdx]) {
    ...
    allPoints.forEach(point => {
        ...
        const fontSize = Math.max(8, 12 * mapState.zoom);
