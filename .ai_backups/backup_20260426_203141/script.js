...
let enableWorldSim = localStorage.getItem('enableWorldSim') === 'true';
...
function initWorldSimulator() {
    console.log("[WorldSim] Инициализация нового мира (Stage 4: Ecology & Disasters)...");
    let newWorld = {
        time: { accumulatedMinutes: 0 },
        regions: {
            "capital_aquilon": { name: "Аквилон", owner: "aquilon_empire", climate: "temperate", population: 50000, resources: { food: 1000, wood: 500, ore: 200, weapons: 100 }, facilities: { farms: 5, lumbermills: 2, mines: 1, forges: 4 }, markets: {}, caravans: [] },
            "silver_haven": { name: "Серебряная Гавань", owner: "free_cities", climate: "tropical", population: 20000, resources: { food: 800, wood: 200, ore: 50, weapons: 50 }, facilities: { farms: 3, lumbermills: 1, mines: 0, forges: 1 }, markets: {}, caravans: [] },
            "thunder_citadel": { name: "Громовая Цитадель", owner: "dwarven_clans", climate: "cold", population: 15000, resources: { food: 200, wood: 100, ore: 2000, weapons: 500 }, facilities: { farms: 1, lumbermills: 1, mines: 8, forges: 6 }, markets: {}, caravans: [] },
            "whispering_woods": { name: "Шепчущий Лес", owner: "elven_circle", climate: "temperate", population: 5000, resources: { food: 1500, wood: 2000, ore: 10, weapons: 20 }, facilities: { farms: 4, lumbermills: 6, mines: 0, forges: 0 }, markets: {}, caravans: [] }
        },
        factions: {
            "aquilon_empire": { name: "Аквилонская Империя", resources: { gold: 10000, food: 5000, manpower: 5000 }, stability: 80, relations: { "free_cities": 0, "dwarven_clans": 20, "elven_circle": -20 }, diplomacy: {}, armies: [] },
            "free_cities": { name: "Свободные Города", resources: { gold: 15000, food: 2000, manpower: 2000 }, stability: 60, relations: { "aquilon_empire": 0, "dwarven_clans": 50, "elven_circle": 10 }, diplomacy: {}, armies: [] },
            "dwarven_clans": { name: "Горные Кланы", resources: { gold: 8000, food: 1000, manpower: 3000 }, stability: 90, relations: { "aquilon_empire": 20, "free_cities": 50, "elven_circle": -50 }, diplomacy: {}, armies: [] },
            "elven_circle": { name: "Дети Леса", resources: { gold: 2000, food: 8000, manpower: 1500 }, stability: 70, relations: { "aquilon_empire": -20, "free_cities": 10, "dwarven_clans": -50 }, diplomacy: {}, armies: [] }
        },
        npcs: {},
        news: [],
        animals: {
            "whispering_woods": { herbivores: 1200, carnivores: 150 },
            "thunder_citadel": { herbivores: 300, carnivores: 50 },
            "capital_aquilon": { herbivores: 600, carnivores: 20 },
            "silver_haven": { herbivores: 400, carnivores: 10 }
        },
        weather: {
            "capital_aquilon": "Ясно",
            "silver_haven": "Легкий бриз",
            "thunder_citadel": "Снег",
            "whispering_woods": "Туман"
        }
    };

    const basePrices = { food: 1, wood: 2, ore: 5, weapons: 20 };
    for (let rId in newWorld.regions) {
        let r = newWorld.regions[rId];
        r.markets = { food: 1, wood: 2, ore: 5, weapons: 20 };
    }
    ...
    for(let i=0; i<40; i++) {
        ...
        newWorld.npcs[id] = {
            id: id,
            name: names[Math.floor(Math.random() * names.length)] + " " + prof,
            type: "npc",
            profession: prof,
            homeLocation: home,
...
function updateWorldSimulation(pulses) {
    if (!enableWorldSim || !World) return;
...
function buildFullPlayerSnapshot() {
    ...
    if (enableWorldSim && World) {
        ...
        worldContextString = "\n=== СИМУЛЯЦИЯ МИРА (АКТУАЛЬНО) ===\n";
        if (playerRegion) {
            let r = World.regions[playerRegion];
            let ownerName = World.factions[r.owner] ? World.factions[r.owner].name : "Ничья земля";
            worldContextString += `Регион: ${r.name} (Владелец: ${ownerName})\nПогода: ${World.weather[playerRegion] || "Нормальная"}\n`;
            worldContextString += `Цены на рынке: Еда(${r.markets.food}), Дерево(${r.markets.wood}), Руда(${r.markets.ore}), Оружие(${r.markets.weapons})\n`;
        } else {
            worldContextString += "Игрок находится вне крупных симулируемых регионов.\n";
        }
        
        let activeWars = [];
        for (let fId in World.factions) {
            for (let target in World.factions[fId].diplomacy) {
                if (World.factions[fId].diplomacy[target] === "war") {
                    let warStr = `${World.factions[fId].name} воюет с ${World.factions[target].name}`;
                    if (!activeWars.includes(`${World.factions[target].name} воюет с ${World.factions[fId].name}`)) {
                        activeWars.push(warStr);
                    }
                }
            }
        }
        if (activeWars.length > 0) {
            worldContextString += "Активные войны в мире:\n" + activeWars.map(w => `- ${w}`).join("\n") + "\n";
        }
        
        let recentNews = World.news.filter(n => n.daysOld <= 2);
        if (recentNews.length > 0) {
            worldContextString += "Свежие слухи в мире:\n" + recentNews.map(n => `- ${n.text}`).join("\n") + "\n";
        }
        worldContextString += "==================================================\n";
    }
...
function initSettingsUI() {
    ...
    const worldSimCheckbox = document.getElementById('enable-world-sim-checkbox');
    if (worldSimCheckbox) worldSimCheckbox.checked = enableWorldSim;
...
function saveSettings() {
    ...
    if (worldSimCheckbox) {
        enableWorldSim = worldSimCheckbox.checked;
        localStorage.setItem('enableWorldSim', enableWorldSim);
    }
...
async function startGameWithNarrator() {
    ...
    const startPrompt = initialPromptTemplate.replace(/{start_mode_instruction}/g, startModeInstruction)
        ...
        .replace(/{map_coordinates_list}/g, mapCoordsString).replace(/{worldEventsData}/g, (enableWorldSim ? "АКТИВИРУЙ ПРОТОКОЛ СИМУЛЯЦИИ МИРА: Создай 10 Весомых Мировых Событий (ID: event_1...event_10) через nexusDefine. ОБЯЗАТЕЛЬНО укажи category: 'World_Event', displayType: 'text', value: 'зреет'. КРИТИЧЕСКИ ВАЖНО: Поле 'description' должно быть глубоким и подробным (2-3 предложения). Опиши суть надвигающейся угрозы, политического кризиса или природного катаклизма, кто в этом замешан и чем это грозит миру." : "Симуляция мира отключена."))
...
Игровое время: День ${player.gameTime.day}, ${player.gameTime.hour < 10 ? '0'+player.gameTime.hour : player.gameTime.hour}:${player.gameTime.minute < 10 ? '0'+player.gameTime.minute : player.gameTime.minute} (${player.timeOfDay})`, "system-message");
    ...
    if (enableWorldSim) runWorldSimulationTick();
...
async function prepareUnifiedPrompt(expiredEffects, promptTextForAI) {
    try {
        ...
        const worldEventsData = (enableWorldSim && player.worldEvents) ? JSON.stringify(player.worldEvents, null, 2) : "Симуляция отключена.";
...
async function runWorldSimulationTick() {
    if (!enableWorldSim || isSimulatingWorld || player.stats.turnCount % 10 !== 0 || player.stats.turnCount === 0) return;
