...
async function generateVisionImage(prompt, btnElement, bubbleElement) {
    ...
    if (imgApiProvider === 'llmost') {
        // LLMost использует эндпоинт чата для мультимодальных моделей
        ...
        // Стандартный формат OpenAI / OpenRouter
        targetUrl = imgApiProvider === 'openrouter' 
            ? 'https://openrouter.ai/api/v1/images/generations' 
            : 'https://api.openai.com/v1/images/generations';
        body = { model: imgModelId, prompt: prompt, n: 1, size: "1024x1024" };
    }
    ...
    try {
        ...
        // Обработка мультимодального ответа (LLMost / GPT-4o)
        if (data.choices && data.choices[0]?.message?.content) {
            const content = data.choices[0].message.content;
            if (Array.isArray(content)) {
                const imgPart = content.find(p => p.type === 'image');
                if (imgPart && imgPart.image) base64data = `data:image/png;base64,${imgPart.image.data}`;
            }
        }
        
        // Fallback для стандартного формата DALL-E
        ...
        if (base64data) {
            ...
            throw new Error('Изображение не найдено в ответе. Проверьте поддержку модели.');
        }
    } catch (e) {
