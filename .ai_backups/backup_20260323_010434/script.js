...
function initializeGameInterface() {
    ...
    setupMapControls();
        ...
        // --- [ПАТЧ] ДИНАМИЧЕСКАЯ ПЕРЕСБОРКА ИНТЕРФЕЙСА (ИСПРАВЛЕННАЯ) ---
    if (!window.uiRestructured) {
        const leftSidebar = document.querySelector('.left-sidebar');
        const gameContent = document.querySelector('.game-content');

        // 1. Окружение: переносим наверх чата
        const envPanel = document.querySelector('.environment-panel');
        if (envPanel && gameContent) {
            gameContent.insertBefore(envPanel, document.getElementById('game-log'));
            envPanel.classList.add('env-horizontal');
        }

        // 2. Умения: прилепляем СНИЗУ к Персонажу (визуальное слияние)
        const charPanel = document.querySelector('.character-sheet');
        const skillsPanel = document.querySelector('.skills-panel');
        if (charPanel && skillsPanel && leftSidebar) {
            leftSidebar.insertBefore(skillsPanel, charPanel.nextSibling);
            charPanel.classList.add('merged-top-panel');
            skillsPanel.classList.add('merged-bottom-panel');
        }
        
        // 3. Инвентарь: прилепляем СНИЗУ к Экипировке
        const equipPanel = document.querySelector('.equipment-panel-v2');
        const invPanel = document.querySelector('.inventory');
        if (equipPanel && invPanel && leftSidebar) {
            leftSidebar.insertBefore(invPanel, equipPanel.nextSibling);
            equipPanel.classList.add('merged-top-panel');
            invPanel.classList.add('merged-bottom-panel');
        }

        // 4. Карта: делаем большим оверлеем
        const mapPanel = document.querySelector('.map-panel');
        const gameInterfaceUI = document.getElementById('game-interface');
        if (mapPanel && gameInterfaceUI) {
            mapPanel.classList.add('map-overlay-panel');
            gameInterfaceUI.appendChild(mapPanel);

            const mapBtn = document.createElement('button');
            mapBtn.id = 'toggle-map-btn';
            mapBtn.title = 'Глобальная Карта';
            mapBtn.innerHTML = '<i class="fas fa-map-marked-alt"></i>';
            mapBtn.onclick = () => {
                mapPanel.classList.toggle('visible');
                // ФИКС КАНВАСА: Ждем конца анимации, задаем жесткие размеры и перерисовываем
                setTimeout(() => {
                    const canvas = document.getElementById('visual-map');
                    if (canvas) {
                        canvas.width = canvas.parentElement.clientWidth;
                        canvas.height = canvas.parentElement.clientHeight || 500;
                        isMapInitialized = false; // Центрируем на игроке
                        renderVisualMap();
                    }
                }, 550); 
            };
            document.querySelector('.header-controls').insertBefore(mapBtn, document.getElementById('in-game-menu-button'));
            
            const mapClose = document.createElement('button');
            mapClose.className = 'map-close-btn';
            mapClose.innerHTML = '<i class="fas fa-times"></i> Закрыть';
            mapClose.onclick = () => mapPanel.classList.remove('visible');
            mapPanel.appendChild(mapClose);
        }

        // 5. Расчеты ГМ: Выдвижной терминал
        const calcPanel = document.querySelector('.calculation-log-panel');
        if (calcPanel) {
            calcPanel.classList.add('gm-terminal-panel');
            document.body.appendChild(calcPanel);
            
            const calcBtn = document.createElement('button');
            calcBtn.id = 'toggle-calc-btn';
            calcBtn.title = 'Системные Логи ГМ';
            calcBtn.innerHTML = '<i class="fas fa-terminal"></i>';
            calcBtn.onclick = () => calcPanel.classList.toggle('visible');
            document.querySelector('.header-controls').insertBefore(calcBtn, document.getElementById('in-game-menu-button'));
        }

        window.uiRestructured = true;
    }

    
    // Увеличиваем разрешение холста карты аппаратно
    const mapCanvasPatch = document.getElementById('visual-map');
    if (mapCanvasPatch) {
        mapCanvasPatch.width = 900;
        mapCanvasPatch.height = 500;
    }
...
function addLogMessage(message, type = "gm-message") {
    ...
    try {
        if (type === 'gm-message') {
            ...
            // Используем DOMPurify, чтобы разрешить отрисовку HTML (плашки кубиков)
            cleanHtml = `<p class="${type}">${DOMPurify.sanitize(message, { USE_PROFILES: { html: true } })}</p>`;
            textToSpeak = message.replace(/<[^>]*>?/gm, ''); // Вырезаем теги для озвучки TTS
        }
    } catch (e) {
...
function handleUserInput() {
    ...
    if (rollsContainer && rollsContainer.children.length > 0) {
        ...
        rollsContainer.querySelectorAll('.roll-badge').forEach(badge => {
            ...
            rollsHtml += `<span class="chat-roll-badge ${extraClass}"><i class="fas fa-dice-d20"></i> ${badgeText}</span>`;
        });
        rollsHtml += '</div>';
    ...
    if (player.currentCombat && player.currentCombat.isActive) {
        ...
        if (activeEnemies.length === 0) {
            ...
            enemyRollsText += rollDetails.join(", ") + ". Используй эти базовые значения (добавив к ним модификаторы врагов) для описания их успехов или провалов в этом ходу.]";
    ...
    try {
        ...
        if (isInitialPrompt) {
            ...
            if (!isSummarizationRequest) {
                ...
                const poetInput = `[ДЕЙСТВИЕ ИГРОКА И БРОСКИ]: ${promptTextForAI}\n\n[ФАКТЫ И РАСЧЕТЫ ОТ СЧЕТОВОДА (ПРЕВРАТИ ЭТО В ХУДОЖЕСТВЕННЫЙ ТЕКСТ)]: ${logicSummaryForPoet}`;
                ...
                if (args.aiIdentifier && args.name && args.type) {
                    ...
                    if (existingKey) {
                        ...
                        player.visibleEntities[newId] = {
                            ...
                            // Умный фолбэк: если ИИ забыл прописать опыт, считаем его из ХП и Силы монстра
