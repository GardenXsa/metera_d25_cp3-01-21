...
function simulateOneDay() {
    ...
    for (let fId of fKeys) {
        ...
        for (let targetF of fKeys) {
            ...
            // ОБЪЯВЛЕНИЕ ВОЙНЫ
            let canFight = f.globalWeapons > 500 || (f.globalFood < 1000 && f.resources.manpower.amount > 2000);
            if (rel < -50 && currentStatus !== "war" && canFight) {
                f.diplomacy[targetF] = "war";
                targetFaction.diplomacy[fId] = "war";
                let reason = f.globalFood < 1000 ? "из-за острой нехватки продовольствия (набег отчаяния)" : "из-за давней вражды и амбиций";
