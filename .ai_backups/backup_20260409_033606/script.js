...
const TILE_SPRITE_MAP = {
    ...
    'dirt':          { x: 1, y: 0 },    // грязь
    'stone_floor':   { x: 2, y: 0 },    // каменный пол
    'wood_floor':    { x: 3, y: 0 },    // деревянный пол
    'water_deep':    { x: 4, y: 0 },    // вода
    'sand':          { x: 5, y: 0 },    // песок

    // Стены
    'wall_stone':    { x: 0, y: 1 },    // каменная стена
    'wall_wood':     { x: 1, y: 1 },    // деревянная стена
    'wall_cave':     { x: 2, y: 1 },    // стена пещеры
    'door_wood':     { x: 3, y: 1 },    // деревянная дверь

    // Объекты
    'chest':         { x: 0, y: 2 },    // сундук
    'campfire':      { x: 1, y: 2 },    // костёр
    'torch':         { x: 2, y: 2 },    // факел
    'table':         { x: 3, y: 2 },    // стол
    'bones':         { x: 4, y: 2 },    // кости
    'blood':         { x: 5, y: 2 },    // кровь
    ...
    // Деревья / растения
    'tree_pine_dark':{ x: 0, y: 3 },    // сосна
    'tree_dead':     { x: 1, y: 3 },    // мёртвое дерево
    'bush_dry':      { x: 2, y: 3 },    // куст

    // Городские / интерьер
    'c_floor_cobble':{ x: 0, y: 4 },    // брусчатка
    'c_wall_brick':  { x: 1, y: 4 },    // кирпичная стена
    'c_door_front':  { x: 2, y: 4 },    // входная дверь
    'c_bed':         { x: 3, y: 4 },    // кровать
    'c_bookshelf':   { x: 4, y: 4 },    // книжный шкаф

    // Подземелье (dungeon)
    'd_floor':       { x: 0, y: 5 },    // пол подземелья
    'd_wall':        { x: 1, y: 5 },    // стена подземелья
    'd_door':        { x: 2, y: 5 },    // дверь подземелья
    'd_barrel':      { x: 3, y: 5 },    // бочка
    'd_pillar':      { x: 4, y: 5 },    // колонна

    // Ад / лава
    'h_floor_lava':  { x: 0, y: 6 },    // лава
    'h_wall_obsidian':{ x: 1, y: 6 },   // обсидиановая стена
    'h_portal':      { x: 2, y: 6 },    // портал

    // Зима / лёд
    's_snow_ground': { x: 0, y: 7 },    // снег
    's_wall_ice':    { x: 1, y: 7 },    // ледяная стена
    's_tree_pine':   { x: 2, y: 7 },    // заснеженная сосна

    // Магия / арканум
    'm_floor_stars': { x: 0, y: 8 },    // звёздный пол
    'm_portal_blue': { x: 1, y: 8 },    // синий портал
    'm_crystal_blue':{ x: 2, y: 8 },    // магический кристалл

    // Дефолт (если тип не найден)
    'void':          { x: 0, y: 0 }     // пустота = трава
};
