...
async function generateLocalMapFromAI(description, size) {
    ...
    try {
        ...
        const rawResponse = await performAiFetch(finalPrompt, [], modelId, "Сгенерируй JSON план локации согласно инструкции.");
        const result = parseAIResponse(rawResponse);
        
        let plots = null;
        if (result.actions) {
            const mapAction = result.actions.find(a => a.command === 'renderLocation');
            if (mapAction && mapAction.args && mapAction.args.plots) plots = mapAction.args.plots;
        }
