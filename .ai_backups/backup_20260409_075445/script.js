...
function renderCanvasMap(plots, width, height, canvas) {
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            ...
            const rotation = plot && plot.rotation ? plot.rotation : 0; // Получаем угол поворота
            ...
            // Смещаем центр координат в центр текущего тайла
            ctx.translate(x * RENDER_TILE_SIZE + RENDER_TILE_SIZE / 2, y * RENDER_TILE_SIZE + RENDER_TILE_SIZE / 2);
            // Поворачиваем (переводим градусы в радианы)
            ctx.rotate(rotation * Math.PI / 180);
            
            // Отрисовываем картинку, сместив ее назад на половину размера, так как центр теперь в (0,0)
            ctx.drawImage(
                TILESET_IMAGE, 
            ...
            ctx.restore(); // Возвращаем контекст в нормальное состояние
            
            // Отрисовка легкой сетки поверх тайлов (без поворота)
