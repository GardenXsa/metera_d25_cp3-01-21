...
function initWorldSimulator() {
    ...
    for (let fId in fConfig) {
        ...
        newWorld.factions[fId] = {
            name: bf.name,
            resources: {
                food: { amount: isElf ? 10000 : 5000, quality: 1.0 },
                wood: { amount: isElf ? 10000 : 5000, quality: 1.0 },
                ore: { amount: isDwarf ? 15000 : 3000, quality: 1.0 },
                herbs: { amount: isElf ? 5000 : 1000, quality: 1.0 },
                steel: { amount: isDwarf ? 2000 : 500, quality: 1.0 },
                cloth: { amount: 1000, quality: 1.0 },
                weapons: { amount: 1000, quality: 1.0 },
                potions: { amount: 200, quality: 1.0 },
                luxury: { amount: 300, quality: 1.0 },
    ...
    for (let key in globalLocations) {
        ...
        newWorld.regions[key] = {
            ...
            resources: {
                food: { amount: isElf ? 10000 : Math.floor(Math.random() * 5000), quality: 1.0 },
                wood: { amount: isElf ? 10000 : Math.floor(Math.random() * 5000), quality: 1.0 },
                ore: { amount: isDwarf ? 15000 : Math.floor(Math.random() * 3000), quality: 1.0 },
                herbs: { amount: isElf ? 5000 : Math.floor(Math.random() * 1000), quality: 1.0 },
                steel: { amount: isDwarf ? 2000 : Math.floor(Math.random() * 500), quality: 1.0 },
                cloth: { amount: Math.floor(Math.random() * 1000), quality: 1.0 },
                weapons: { amount: isDwarf || ownerId === 'aquilon' ? 5000 : Math.floor(Math.random() * 1000), quality: 1.0 },
                potions: { amount: isElf ? 1000 : Math.floor(Math.random() * 200), quality: 1.0 },
                luxury: { amount: ownerId === 'consortium' ? 2000 : Math.floor(Math.random() * 300), quality: 1.0 }
            },
...
function simulateOneDay() {
    ...
    for (let rId of rKeys) {
        ...
        // Обслуживание зданий (Износ)
        for (let fId in r.facilities) {
            let fac = r.facilities[fId];
            if (fac.level > 0) {
                fac.durability -= 1;
                if (fac.durability < 0) fac.durability = 0;
                // Если износ критический, производительность падает
                if (fac.durability < 20) fac.level = Math.floor(fac.level * 0.5);
            }
        }

        // Ремонт зданий (если есть ресурсы)
        for (let fId in r.facilities) {
            let fac = r.facilities[fId];
            if (fac.level > 0 && fac.durability < 50 && r.resources.wood > 5) {
                fac.durability += 20;
                r.resources.wood -= 5;
            }
        }
        ...
        let steelCapacity = Math.floor(workersPerSector * (r.facilities.forges.level / 10));
        ...
        for (let good in BASE_ECONOMY_PRICES) {
            ...
            // Формула цены: База * (Спрос / Предложение) * Качество. Ограничиваем от 0.2x до 5x
            let priceMod = Math.max(0.2, Math.min(5.0, demand / supply));
            let qualityMod = r.resources[good].quality || 1.0;
            r.markets[good] = Math.max(1, Math.floor(BASE_ECONOMY_PRICES[good] * priceMod * qualityMod));
        }
    }
    ...
    for (let fId of fKeys) {
        ...
        if (atWarWith && f.resources.gold > 1000 && f.resources.manpower > 300) {
            ...
            let isCombatActive = false; // Флаг для предотвращения преждевременного роспуска
            ...
            if (enemyFactionId) {
                ...
                if (atkPower > defPower) {
                    ...
                    isCombatActive = false; // Враг разбит, можно распускать армию
                } else {
            ...
            // Если вражеской армии нет, но регион враждебный (ОСАДА)
                ...
                if (army.siegeDays === -1) {
                    ...
                    if (atkPower > garrisonPower) {
                        ...
                        isCombatActive = false; // Город взят, армия становится гарнизоном
                    } else {
            ...
            // Очистка: если армия выжила и НЕ занята боем/осадой
...
function repeatLastAction() {
    ...
    // Вставляем чистый текст пользователя (системные теги больше не сохраняются сюда)
...
[ВРЕМЯ С ПРОШЛОГО ХОДА]: ${(() => {
    ...
    // Сохраняем ЧИСТЫЙ текст пользователя для функции "Повторить действие", ДО добавления системных тегов
...
async function _internalPerformAiFetch(systemInstruction, history, providerModel, currentInput = "") {
    ...
    while (attempts < maxAttempts) {
        ...
        if (!response.ok) {
            ...
            // Авто-отключение неподдерживаемых параметров
            if (errText.includes("prompt_caching")) {
            ...
            if (retry) {
                addLogMessage("[СИСТЕМА] Модель не поддерживает параметры ИИ (Thinking/Caching), отключаю их...", "system-message");
                attempts++;
                continue;
            }
            ...
            case 'addItem':
                if (args.aiIdentifier && args.name) {
                    ...
                    // ЛОГИКА ДЛЯ ПРЕДМЕТОВ
                        ...
                        quality: args.quality || 1, // 1 - обычное, 5 - шедевр
...
async function runWorldSimulationTick() {
    ...
    try {
        ...
        for (let fId in World.factions) {
            let f = World.factions[fId];
            worldSummary += `Фракция: ${f.name}. Стабильность: ${f.stability}/100. Золото: ${f.resources.gold}. Армий в походе СЕЙЧАС: ${f.armies.length}.\n`;
            for (let target in f.diplomacy) {
        ...
        // ПЕРЕДАЕМ ИИ ТОЧНУЮ ХРОНОЛОГИЮ ЗА ПРОПУЩЕННОЕ ВРЕМЯ
