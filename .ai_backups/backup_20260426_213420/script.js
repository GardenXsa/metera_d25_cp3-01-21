...
async function finalizeCharacterCreation() {
    ...
    const proceedToNarrator = async () => {
        ...
        tempPlayer = {
            ...
            gameTime: { year: 1, month: 1, day: 1, hour: 8, minute: 0, totalPulses: 0 },
            timeOfDay: "Утро",
            equipment: {},
            inventory: {},
...
function updateTimeDisplay() {
    ...
    if (timeInfo && player && player.gameTime) {
        ...
        timeInfo.innerHTML = `${icon} День ${gt.day} | ${hh}:${mm}`;
    }
}
...
async function runWorldSimulationTick() {
    if (isSimulatingWorld || player.stats.turnCount % 10 !== 0 || player.stats.turnCount === 0) return;
