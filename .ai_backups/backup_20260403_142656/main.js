...
ipcMain.handle('speak-text', async (event, text, voiceModel) => {
    ...
    return new Promise((resolve) => {
        // Умный поиск папки (учитывает распаковку из asar при билде)
        // Умный поиск пути для сбилженного приложения (извлекаем из asar)
        let ttsDir = path.join(__dirname, 'assets', 'tts');
        
        // В Electron при сборке ресурсы попадают в app.asar. 
        // Бинарники (piper.exe) должны запускаться из app.asar.unpacked
        if (__dirname.includes('app.asar')) {
            ttsDir = path.join(__dirname.replace('app.asar', 'app.asar.unpacked'), 'assets', 'tts');
        } else {
            // Путь для разработки (npm start)
        ...
        if (!fs.existsSync(piperExe)) {
            resolve({ success: false, error: `Движок Piper не найден: ${piperExe}` });
            return;
        }
        ...
        // Запускаем локальную генерацию речи
        // Указываем { cwd: ttsDir }, чтобы piper.exe видел свои DLL, иначе будет ошибка 3221226505
        const piper = spawn(piperExe, ['--model', modelPath, '--output_file', outputPath], { cwd: ttsDir });
        // Явно указываем кодировку для передачи текста в пайпер
        piper.stdin.setDefaultEncoding('utf-8');
        piper.stdin.write(text);
        piper.stdin.end();

        piper.on('close', (code) => {
            if (code === 0) resolve({ success: true, audioPath: `file://${outputPath}` });
            else resolve({ success: false, error: `Piper exit code: ${code}` });
        });
