...
async function generateVisionImage(prompt, btnElement, bubbleElement) {
    ...
    let body = { model: imgModelId, prompt: prompt, n: 1, size: "1024x1024" };

    if (imgApiProvider === 'llmost') targetUrl = 'https://llmost.ru/api/v1/images/generations';
    ...
    try {
        ...
        if (data.data && data.data[0] && data.data[0].url) {
            ...
            // Скачиваем картинку, пока жива ссылка, и конвертируем в Base64
            ...
            reader.onloadend = function() {
                ...
                // Вшиваем картинку прямо в историю для сохранения на диск
                ...
                imgContainer.innerHTML = `<img src="${base64data}" alt="Эфирное видение" title="Нажми, чтобы открыть оригинал" onclick="const w = window.open(''); w.document.write('<style>body{margin:0;background:#000;display:flex;justify-content:center;align-items:center;height:100vh;}img{max-width:100%;max-height:100vh;object-fit:contain;}</style><img src=\'${base64data}\'>');"><div class="img-watermark">Эфирный Слепок</div>`;
        ...
        showCustomAlert('Ошибка генерации: ' + e.message);
...
// --- ГЛОБАЛЬНАЯ ФУНКЦИЯ ГЕНЕРАЦИИ (ФИКС) ---
window.generateVisionImage = async function(prompt, btnElement, bubbleElement) {
    if (!imgApiKey) {
    ...
    // Используем эндпоинт чата для мультимодальной генерации
    let targetUrl = 'https://llmost.ru/api/v1/chat/completions';
    if (imgApiProvider !== 'llmost') {
        targetUrl = 'https://api.openai.com/v1/chat/completions';
    }

    let headers = { 'Content-Type': 'application/json', 'Authorization': `Bearer ${imgApiKey}` };
    
    // Формат для моделей, поддерживающих output_modalities: ["image"]
    let body = {
        model: imgModelId,
        messages: [{ role: "user", content: prompt }],
        modalities: ["text", "image"],
        size: "512x512" // Экономный размер
    };
    ...
    try {
        ...
        // Парсим мультимодальный ответ (ищем картинку в content)
        let base64data = null;
        ...
        if (message.content && Array.isArray(message.content)) {
            ...
            if (imagePart && imagePart.image) {
                base64data = `data:image/png;base64,${imagePart.image.data}`;
            }
        } else if (data.data && data.data[0] && data.data[0].url) {
            // Fallback для стандартных моделей
        ...
        if (base64data) {
            // Вшиваем в историю для сохранения
            if (player && player.gameLogHistory) {
                for(let i = player.gameLogHistory.length - 1; i >= 0; i--) {
                    if(player.gameLogHistory[i].type === 'gm-message') {
                        player.gameLogHistory[i].savedImageBase64 = base64data;
            ...
            const imgContainer = document.createElement('div');
            imgContainer.className = 'chat-illustration-container';
            imgContainer.innerHTML = `<img src="${base64data}" alt="Видение" onclick="const w = window.open(''); w.document.write('<style>body{margin:0;background:#000;display:flex;justify-content:center;align-items:center;height:100vh;}img{max-width:100%;max-height:100vh;object-fit:contain;}</style><img src=\'${base64data}\'>');"><div class="img-watermark">Эфирный Слепок</div>`;
            bubbleElement.appendChild(imgContainer);
            btnElement.style.display = 'none';
            setTimeout(() => gameLog.scrollTo({ top: gameLog.scrollHeight, behavior: 'smooth' }), 300);
        } else {
            throw new Error('Модель вернула текст, но не сгенерировала изображение. Проверьте поддержку модели.');
        }
    } catch (e) {
        console.error('Image Gen Error:', e);
        showCustomAlert('Ошибка: ' + e.message);
