...
const TradeSystem = {
    ...
    confirmTrade: function(tradeId) {
        ...
        for (const offer of trade.offer_items) {
            ...
            if (ItemRegistry.has(offer.id)) ItemRegistry.get(offer.id).state = "idle";
        }
        for (const req of trade.request_items) {
            ...
            if (ItemRegistry.has(req.id)) ItemRegistry.get(req.id).state = "idle";
        }
...
case 'setEntityBinding':
            ...
            case 'createContainer':
                ...
                break;
                                                            case 'initiateTrade':
                if (args.targetId && args.offerItems && args.requestItems) {
                    ...
                    args.requestItems.forEach(req => {
                        const foundId = tgtContObj.items.find(id => {
                            let it = ItemRegistry.get(id);
                            return it && it.id === req.id && it.state === "idle" && it.stack_size >= req.quantity;
                        });
                        if (foundId) {
                            if (!TradeSystem.addItemToTrade(tradeId, foundId, "request", req.quantity)) success = false;
                        } else {
                            success = false;
                        }
                    });
                    ...
                    if (success) {
                        ...
                        feedback = `[ТОРГОВЛЯ] Инициирована сделка. Предметы заблокированы. Спросите игрока: "Подтверждаете сделку?" и при согласии вызовите confirmTrade.`;
                    } else {
                        TradeSystem._rollbackTrade(tradeId, "Invalid items or insufficient quantities");
                        feedback = `[ERROR] Не удалось инициировать сделку. У торговца нет запрошенных предметов, либо у игрока не хватает товара.`;
                    }
                } else {
