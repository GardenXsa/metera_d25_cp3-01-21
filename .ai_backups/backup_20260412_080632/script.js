    ...
    for (const category of sortedCategories) {
        ...
        groupedData[category].sort((a, b) => a.name.localeCompare(b.name, currentLanguage)).forEach(item => {
            ...
            li.addEventListener('mouseenter', (e) => {
                ...
                const content = `<div style="color:#bdc3c7; font-style:italic; margin-bottom: 5px; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 3px;">Категория: ${item.category || 'Прочее'}</div>
                                 <div style="color:#ecf0f1; line-height: 1.4; font-size: 0.95em;">${desc}</div>`;
...
async function runWorldSimulationTick() {
    ...
    addCalculationMessage("[СИСТЕМА: СИМУЛЯЦИЯ] Мир начинает движение...");
    try {
        const prompt = "Ты — ДВИЖОК МИРА. Твоя задача: развить ОДНО из 10 событий в списке worldEvents. \n1. Выбери событие со статусом 'зреет'.\n2. Переведи его в 'АКТИВНО' (nexusUpdate).\n3. Запиши в setMemory (ID: World_Log) максимально подробные и драматичные последствия этого события (кто пострадал, как изменилась геополитика, какие слухи пошли по миру).\n4. ВАЖНО: Только JSON-команды, без текста!";
        const modelId = currentApiProvider === 'gemini' ? geminiModelId : (currentApiProvider === 'llmost' ? llmostModelId : openrouterModelId);
        const raw = await performAiFetch(prompt, [], modelId, `Текущий ход: ${player.stats.turnCount}. Развивай мир. События: ${JSON.stringify(player.worldEvents)}`);
        const res = parseAIResponse(raw);
        if (res.actions) {
            res.actions.forEach(a => {
                executeCommand(a.command, a.args);
                if (a.command === 'nexusUpdate') {
                    const ev = player.worldEvents.find(e => e.id === a.args.id);
                    if (ev) ev.value = a.args.value;
                }
            });
            addLogMessage("(( Эфирные потоки принесли вести из далеких земель: мир изменился. ))", "command-feedback");
        }
    } catch (e) { console.error("World Sim Error:", e); } finally { isSimulatingWorld = false; }
}