...
async function generateVisionImage(prompt, btnElement, bubbleElement) {
    ...
    try {
        const response = await fetch(targetUrl, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${imgApiKey}` }, body: JSON.stringify(body) });
        const data = await response.json();
        
        // ВЫВОД ДЛЯ ОТЛАДКИ - ЭТО ПОМОЖЕТ ПОНЯТЬ СТРУКТУРУ
        console.log("--- DEBUG: LLMOST RESPONSE ---", data);

        let base64data = null;

        // 1. Поиск в стандартном OpenAI формате (data[0].url или b64_json)
        if (data.data && data.data[0]) {
            if (data.data[0].url) {
                const imgRes = await fetch(data.data[0].url);
                const imgBlob = await imgRes.blob();
                base64data = await new Promise(r => { const rd = new FileReader(); rd.onloadend = () => r(rd.result); rd.readAsDataURL(imgBlob); });
            } else if (data.data[0].b64_json) {
                base64data = `data:image/png;base64,${data.data[0].b64_json}`;
            }
        }

        // 2. Поиск в Chat Completions (Choices -> Message -> Content)
        if (!base64data && data.choices && data.choices[0].message) {
            const content = data.choices[0].message.content;
            if (Array.isArray(content)) {
                const imgPart = content.find(p => p.type === 'image' || p.image_url || p.image);
                if (imgPart?.image?.data) base64data = `data:image/png;base64,${imgPart.image.data}`;
                else if (imgPart?.image_url?.url) {
                    const imgRes = await fetch(imgPart.image_url.url);
                    const imgBlob = await imgRes.blob();
                    base64data = await new Promise(r => { const rd = new FileReader(); rd.onloadend = () => r(rd.result); rd.readAsDataURL(imgBlob); });
                }
            } else if (typeof content === 'string' && content.startsWith('http')) {
                const imgRes = await fetch(content.trim());
                const imgBlob = await imgRes.blob();
                base64data = await new Promise(r => { const rd = new FileReader(); rd.onloadend = () => r(rd.result); rd.readAsDataURL(imgBlob); });
            }
        }
        ...
        if (base64data) {
            renderGeneratedImageInChat(base64data, bubbleElement);
            btnElement.style.display = 'none';
        } else {
            // Если не нашли, выводим JSON в окно, чтобы ты мог сфоткать
            console.error("Image not found in response fields.");
            showCustomAlert("ОШИБКА ПАРСИНГА: Скопируй это в консоли (F12) и скинь мне. JSON keys: " + Object.keys(data).join(", "));
        }
    } catch (e) {
