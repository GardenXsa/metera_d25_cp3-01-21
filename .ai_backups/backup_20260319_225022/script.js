...
function renderVisualMap() {
    ...
    // Если ширина 0 (панель скрыта), не пытаемся инициализировать, чтобы не сломать offsetX
    if (width === 0) return;
    ...
    // Умное центрирование при первой отрисовке
    if (!isMapInitialized) {
        const currentLocName = (player.location || "").toLowerCase().trim();
        // Ищем точку с максимально похожим названием
        ...
        if (playerPoint) {
            ...
            // Если текущая локация не найдена, центрируемся на первой доступной точке
    ...
    // Сетка
    ctx.strokeStyle = 'rgba(93, 74, 54, 0.1)';
