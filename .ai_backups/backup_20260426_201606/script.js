...
function initWorldSimulator() {
    console.log("[WorldSim] Инициализация нового мира (Stage 2: Economy)...");
    let newWorld = {
        time: { accumulatedMinutes: 0 },
        regions: {
            "capital_aquilon": { name: "Аквилон", climate: "temperate", population: 50000, resources: { food: 1000, wood: 500, ore: 200, weapons: 100 }, facilities: { farms: 5, lumbermills: 2, mines: 1, forges: 4 }, markets: {}, caravans: [] },
            "silver_haven": { name: "Серебряная Гавань", climate: "tropical", population: 20000, resources: { food: 800, wood: 200, ore: 50, weapons: 50 }, facilities: { farms: 3, lumbermills: 1, mines: 0, forges: 1 }, markets: {}, caravans: [] },
            "thunder_citadel": { name: "Громовая Цитадель", climate: "cold", population: 15000, resources: { food: 200, wood: 100, ore: 2000, weapons: 500 }, facilities: { farms: 1, lumbermills: 1, mines: 8, forges: 6 }, markets: {}, caravans: [] },
            "whispering_woods": { name: "Шепчущий Лес", climate: "temperate", population: 5000, resources: { food: 1500, wood: 2000, ore: 10, weapons: 20 }, facilities: { farms: 4, lumbermills: 6, mines: 0, forges: 0 }, markets: {}, caravans: [] }
        },
        factions: {
            "aquilon_empire": { name: "Аквилонская Империя", resources: { gold: 10000, food: 5000 }, stability: 80, relations: { "free_cities": 0, "dwarven_clans": 20, "elven_circle": -20 }, armies: [] },
            "free_cities": { name: "Свободные Города", resources: { gold: 15000, food: 2000 }, stability: 60, relations: { "aquilon_empire": 0, "dwarven_clans": 50, "elven_circle": 10 }, armies: [] },
            "dwarven_clans": { name: "Горные Кланы", resources: { gold: 8000, food: 1000 }, stability: 90, relations: { "aquilon_empire": 20, "free_cities": 50, "elven_circle": -50 }, armies: [] },
            "elven_circle": { name: "Дети Леса", resources: { gold: 2000, food: 8000 }, stability: 70, relations: { "aquilon_empire": -20, "free_cities": 10, "dwarven_clans": -50 }, armies: [] }
        },
        npcs: {},
        news: [],
        animals: {
...
function simulateOneDay() {
    console.log("[WorldSim] Симуляция нового дня (Stage 2: Economy)...");
    ...
    // 2. Продвинутая Экономика (Производство, Потребление, Цены)
    const basePrices = { food: 1, wood: 2, ore: 5, weapons: 20 };
    ...
    for (let rId of rKeys) {
        ...
        // Инициализация зданий, если их нет (для совместимости старых сейвов)
        if (!r.facilities) r.facilities = { farms: 1, lumbermills: 1, mines: 1, forges: 1 };

        // Производство (зависит от населения, зданий и погоды)
        ...
        for (let good in basePrices) {
            ...
            let priceMod = demand / supply;
            priceMod = Math.max(0.2, Math.min(10, priceMod)); // Ограничитель инфляции (от 20% до 1000%)
            r.markets[good] = Math.max(1, Math.floor(basePrices[good] * priceMod));
        }
    }

    // 3. Динамические Торговые Маршруты (Караваны)
    for (let rId of rKeys) {
        let r = World.regions[rId];
        for (let good in basePrices) {
            // Если цена в 2 раза выше базовой -> дефицит, нужен импорт
            if (r.markets[good] > basePrices[good] * 2) {
                ...
                for (let supId of rKeys) {
                    ...
                    // Ищем регион, где товар дешевле и его много
                ...
                if (bestSupplier) {
                    let amount = Math.floor(World.regions[bestSupplier].resources[good] * 0.2); // Отправляем 20% запасов
                    if (amount > 0) {
                        World.regions[bestSupplier].resources[good] -= amount;
                        let caravanId = "caravan_" + Date.now() + Math.floor(Math.random()*1000);
                        World.regions[bestSupplier].caravans.push({
                            id: caravanId,
                            origin: bestSupplier,
                            destination: rId,
                            goods: { [good]: amount },
                            hoursLeft: 48 + Math.floor(Math.random() * 24) // 2-3 дня пути
                        });
                        if (Math.random() < 0.3) {
                            generateWorldNews(`Торговый караван с ${good} отправился из ${World.regions[bestSupplier].name} в ${r.name}.`, "global", 1);
                        }
                    }
                }
            }
    ...
    // 4. Фракции (Политика и Войны - Заглушка для Этапа 3)
    let fKeys = Object.keys(World.factions);
    for (let fId of fKeys) {
        let f = World.factions[fId];
        f.resources.gold += 500; 
        f.resources.food -= 200; 

        let targetF = fKeys[Math.floor(Math.random() * fKeys.length)];
        if (targetF !== fId) {
            let change = Math.floor(Math.random() * 11) - 5; 
            f.relations[targetF] = Math.max(-100, Math.min(100, (f.relations[targetF] || 0) + change));
        }
    }

    // 5. Экология (Хищник-Жертва)
