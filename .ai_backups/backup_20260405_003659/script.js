...
async function generateVisionImage(prompt, btnElement, bubbleElement) {
    ...
    try {
        ...
        // ЛОГИКА ДЛЯ LLMOST (Мультимодальный ответ)
        if (data.choices && data.choices[0].message) {
            const msg = data.choices[0].message;
            if (Array.isArray(msg.content)) {
                const imgPart = msg.content.find(p => p.type === 'image');
                if (imgPart && imgPart.image) {
                    base64data = `data:image/png;base64,${imgPart.image.data}`;
                }
            } else if (typeof msg.content === 'string' && msg.content.includes('data:image')) {
                // Если вдруг прислали строкой
                const match = msg.content.match(/data:image\/[^;]+;base64,[^\s"]+/);
                if (match) base64data = match[0];
            }
        }

        // ЛОГИКА ДЛЯ ОБЫЧНОГО OpenAI (DALL-E)
        if (!base64data && data.data && data.data[0].url) {
        ...
        if (base64data) {
            ...
            throw new Error('Картинка сгенерирована, но скрипт не смог её распарсить. Проверь консоль.');
        }
    } catch (e) {
