...
function initWorldSimulator() {
    ...
    for (let key in globalLocations) {
        ...
        for (let resKey in ECONOMY_ITEMS) {
            ...
            if (isElf && (resKey === 'wood' || resKey === 'herbs' || resKey === 'wheat')) baseAmount *= 4;
            if (isDwarf && (resKey === 'iron_ore' || resKey === 'gold_ore')) baseAmount *= 4;
...
function simulateOneDay() {
    ...
    for (let rId in World.regions) {
        ...
        if (Math.random() < 0.015) {
            ...
            if (disaster === "Землетрясение") {
                ...
                if (r.facilities.farms && r.facilities.farms.level > 0) r.facilities.farms.durability -= 50;
                if (r.facilities.mines && r.facilities.mines.level > 0) r.facilities.mines.durability -= 50;
                ...
                if(r.resources.wheat) r.resources.wheat.amount = Math.floor(r.resources.wheat.amount * 0.3);
                if(r.resources.wood) r.resources.wood.amount = Math.floor(r.resources.wood.amount * 0.4);
                damageStr = "Урожай и леса уничтожены.";
            } else {
                if(r.resources.wood) r.resources.wood.amount = Math.floor(r.resources.wood.amount * 0.7);
                if(r.resources.meat) r.resources.meat.amount = Math.floor(r.resources.meat.amount * 0.8);
    ...
    for (let rId of rKeys) {
        ...
        // Сбор налогов фракцией (Подоходный налог)
        let faction = World.factions[r.owner];
        if (faction) {
            let taxRevenue = Math.floor(r.moneySupply * 0.02); // 2% налог
            r.moneySupply -= taxRevenue;
            faction.resources.gold.amount += taxRevenue;
        }
        
        // --- ПРОИЗВОДСТВЕННЫЕ ЦЕПОЧКИ (DATA-DRIVEN) ---
        ...
        // 1. Базовая добыча сырья
        if(r.facilities.farms) {
            r.resources.wheat.amount += Math.floor(workersPerSector * (r.facilities.farms.level / 10) * 2 * weatherMod);
        ...
        if (employmentRate < 0.5 && Math.random() < 0.05) {
            ...
            if(r.resources.weapons) r.resources.weapons.amount = Math.max(0, r.resources.weapons.amount - 50);
        }
        ...
        for (let good in ECONOMY_ITEMS) {
            ...
            let demand = r.population * 0.001; // Базовый спрос
            
            // Умный спрос на основе рецептов
            for(let recipe of CRAFTING_RECIPES) {
    ...
    // Микро-экономика NPC (Потребительская корзина)
    for (let id in World.npcs) {
        let npc = World.npcs[id];
        if (!npc.isAlive) continue;
        ...
        if (locId && World.regions[locId]) {
            ...
            // Богатые NPC покупают роскошь
            if (npc.economy.savings > 200 && r.resources.jewelry && r.resources.jewelry.amount > 0) {
                let luxPrice = r.markets.jewelry || 150;
                npc.economy.savings -= luxPrice;
                r.resources.jewelry.amount -= 1;
                r.moneySupply += luxPrice;
                npc.needs.social += 50;
            }
        }
    }
        ...
        for (let good in ECONOMY_ITEMS) {
            ...
            if (bestDest) {
                ...
                if (amount > 0) {
                    ...
                    if (Math.random() < 0.15) {
                        generateWorldNews(`Торговая гильдия отправила караван из ${r.name} в ${destRegion.name}. Груз: ${ECONOMY_ITEMS[good].name} (${amount} шт).`, bestDest, 2, 'trade');
                    }
                }
            }
        }
    ...
    for (let rId in World.animals) {
        ...
        if (a.herbivores > 500 && r) {
            ...
            a.herbivores -= hunted;
            if(!r.resources.meat) r.resources.meat = {amount:0, quality:1.0};
            r.resources.meat.amount += hunted * 2;
        }
    }
