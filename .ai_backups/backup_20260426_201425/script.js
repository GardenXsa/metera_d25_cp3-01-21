...
function initWorldSimulator() {
    console.log("[WorldSim] Инициализация нового мира (Stage 1: Deep NPCs)...");
    let newWorld = {
        time: { accumulatedMinutes: 0 },
        regions: {
            "capital_aquilon": { name: "Аквилон", climate: "temperate", population: 50000, resources: { food: 1000, wood: 500, ore: 200, weapons: 100 }, markets: {}, caravans: [] },
            "silver_haven": { name: "Серебряная Гавань", climate: "tropical", population: 20000, resources: { food: 800, wood: 200, ore: 50, weapons: 50 }, markets: {}, caravans: [] },
            "thunder_citadel": { name: "Громовая Цитадель", climate: "cold", population: 15000, resources: { food: 200, wood: 100, ore: 2000, weapons: 500 }, markets: {}, caravans: [] },
            "whispering_woods": { name: "Шепчущий Лес", climate: "temperate", population: 5000, resources: { food: 1500, wood: 2000, ore: 10, weapons: 20 }, markets: {}, caravans: [] }
        },
        factions: {
    ...
    for(let i=0; i<40; i++) {
        ...
        newWorld.npcs[id] = {
            ...
            relationships: {}, // { targetId: { trust: 50, fear: 0, affection: 0 } }
            memory: [], // { day, text }
    ...
    // Стартовые караваны
    newWorld.regions["silver_haven"].caravans.push({ id: "caravan_1", origin: "silver_haven", destination: "capital_aquilon", goods: { food: 500 }, hoursLeft: 24 });
    newWorld.regions["thunder_citadel"].caravans.push({ id: "caravan_2", origin: "thunder_citadel", destination: "capital_aquilon", goods: { ore: 200, weapons: 50 }, hoursLeft: 48 });
...
function simulateOneDay() {
    console.log("[WorldSim] Симуляция нового дня...");
    ...
    // 2. Экономика (Производство и Потребление)
    for (let rId in World.regions) {
        let r = World.regions[rId];
        // Потребление
        r.resources.food -= Math.floor(r.population * 0.01);
        if (r.resources.food < 0) {
            r.resources.food = 0;
            r.population -= Math.floor(Math.random() * 50); // Голод
            if (Math.random() < 0.1) generateWorldNews(`Голод в регионе ${r.name}! Люди гибнут.`, rId, 3);
        }
        // Производство (упрощенно)
        if (rId === "whispering_woods") r.resources.wood += 500;
        if (rId === "thunder_citadel") { r.resources.ore += 300; r.resources.weapons += 50; }
        if (rId === "capital_aquilon") r.resources.food += 600;

        // Пересчет цен (Спрос / Предложение)
        const basePrices = { food: 1, wood: 2, ore: 5, weapons: 20 };
        for (let good in basePrices) {
            let supply = r.resources[good] || 1;
            let demand = r.population * (good === 'food' ? 0.05 : 0.01);
            r.markets[good] = Math.max(1, Math.floor(basePrices[good] * (demand / supply)));
        }
    }

    // 3. Фракции (Политика и Войны)
    let fKeys = Object.keys(World.factions);
    for (let fId of fKeys) {
        let f = World.factions[fId];
        f.resources.gold += 500; // Налоги
        f.resources.food -= 200; // Содержание армий

        // Случайные изменения отношений
        ...
        if (targetF !== fId) {
            ...
            if (f.relations[targetF] < -80 && Math.random() < 0.05) {
                generateWorldNews(`Фракция ${f.name} объявляет войну ${World.factions[targetF].name}!`, "global", 5);
                f.relations[targetF] = -100;
            }
        }
    }

    // 4. Экология (Хищник-Жертва)
    for (let rId in World.animals) {
        let a = World.animals[rId];
        // Волки едят оленей, олени размножаются
        let eaten = Math.floor(a.wolves * 0.5);
        a.deer = Math.max(0, a.deer - eaten);
        a.deer += Math.floor(a.deer * 0.1); // Рост популяции оленей
        if (a.deer < a.wolves) {
            a.wolves -= Math.floor(a.wolves * 0.1); // Волки умирают от голода
        } else {
            a.wolves += Math.floor(a.wolves * 0.05); // Волки размножаются
        }
    }

    // 5. Устаревание новостей
