...
function setupEventListeners() {
    ...
    // Логика переключения видимости API ключа
...
async function performAiFetch(systemInstruction, history, providerModel, currentInput = "") {
    ...
    // Используем IPC мост Electron для обхода ERR_CONNECTION_CLOSED
    if (currentApiProvider === 'gemini' && window.electronAPI?.sendGeminiRequest) {
        console.log(`[AI Fetch] Запрос через системный уровень Electron: ${providerModel}`);
        ...
        const response = await window.electronAPI.sendGeminiRequest(providerModel, activeKey, contents);
    ...
    // Резервный метод для других провайдеров
    let targetUrl = `https://generativelanguage.googleapis.com/v1beta/models/${providerModel}:generateContent?key=${activeKey}`;
...
// --- ФУНКЦИЯ БЫСТРОГО СТАРТА (ГЛОБАЛЬНАЯ) ---
function handleQuickStart() {
    const races = ['human', 'elf', 'dwarf'];
    const classes = ['warrior', 'mage', 'rogue', 'bard'];
    const eras = ['rebirth', 'architects', 'sundering', 'silence'];
    ...
    // 1. Выбираем случайные параметры
    charRaceSelect.value = races[Math.floor(Math.random() * races.length)];
    charClassSelect.value = classes[Math.floor(Math.random() * classes.length)];
    charEraSelect.value = eras[Math.floor(Math.random() * eras.length)];

    // 2. Инициализируем базовые статы для выбранной расы/класса
    handleRaceOrClassChange();

    // 3. Заполняем имя и описание
    charNameInput.value = names[Math.floor(Math.random() * names.length)] + " " + (Math.floor(Math.random() * 900) + 100);
    charDescInput.value = "Авантюрист, прибывший из старой деревни на севере.";

    // 4. Распределяем 10 очков случайно между всеми характеристиками (включая Стойкость)
    const statKeys = ['str', 'dex', 'int', 'con', 'cha', 'res'];
    while (availableStatPoints > 0) {
        const randomStat = statKeys[Math.floor(Math.random() * statKeys.length)];
        currentCreationStats[randomStat]++;
    ...
    // 5. Обновляем UI и переходим к Рассказчику
    updateStatCreationDisplay();
    finalizeCharacterCreation();
}