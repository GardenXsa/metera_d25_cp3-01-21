...
function initWorldSimulator() {
    console.log("[WorldSim] Инициализация нового мира (Stage 3: Factions)...");
    let newWorld = {
        ...
        news: [],
        animals: {
            "whispering_woods": { wolves: 150, deer: 800 },
            "thunder_citadel": { wolves: 50, deer: 100 },
            "capital_aquilon": { wolves: 20, deer: 300 }
        },
        weather: {
...
function simulateOneDay() {
    console.log("[WorldSim] Симуляция нового дня (Stage 3: Factions & Armies)...");

    // 1. Погода
    const weathers = ["Ясно", "Облачно", "Дождь", "Гроза", "Туман", "Снег"];
    for (let rId in World.regions) {
        if (Math.random() < 0.3) {
            World.weather[rId] = weathers[Math.floor(Math.random() * weathers.length)];
        }
    }
    ...
    for (let rId of rKeys) {
        ...
        let weatherMod = (World.weather[rId] === "Ясно") ? 1.2 : (World.weather[rId] === "Гроза" || World.weather[rId] === "Снег") ? 0.5 : 1.0;
        ...
        if (World.weather[rId] === "Снег") woodConsumed *= 2;
    ...
    for (let fId of fKeys) {
        ...
        if (f.stability < 30 && Math.random() < 0.1) {
            ...
            f.stability += 20; // Сброс напряжения
                ...
                if (army.daysToMove <= 0) {
                    ...
                    if (regionOwner && f.diplomacy[regionOwner] === "war") {
                        ...
                        if (defArmyIndex !== -1) {
                            ...
                            let defPower = defArmy.size * (defArmy.morale / 100) * (Math.random() * 0.5 + 1.0); // Бонус защитника
    ...
    for (let rId in World.animals) {
        ...
        let eaten = Math.floor(a.wolves * 0.5);
        a.deer = Math.max(0, a.deer - eaten);
        a.deer += Math.floor(a.deer * 0.1); 
        if (a.deer < a.wolves) {
            a.wolves -= Math.floor(a.wolves * 0.1); 
        } else {
            a.wolves += Math.floor(a.wolves * 0.05); 
        }
    }
