...
window.activatePlayerSkill = function(skillId) {
    ...
    // Добавляем системную команду для GM
    queuePlayerActionForGM(`[SYSTEM: Игрок АКТИВИРОВАЛ УМЕНИЕ "${skill.name}". Заявленный эффект: "${skill.effect}". ТЫ ОБЯЗАН применить этот эффект в своих расчетах (нанести урон, наложить бафф/дебафф через addStatusEffect) и красочно описать применение умения в narrative!]`);

    // Добавляем текст в поле ввода
    userInput.value += ` *Применяет умение: ${skill.name}* `;
...
function parseAIResponse(rawResponse) {
    ...
    if (startIdx !== -1 && endIdx !== -1) {
        ...
        try {
            ...
            console.error("КРИТИЧЕСКАЯ ОШИБКА: JSON поврежден.", jsonErr);
            throw new Error("Ответ ИИ содержит невалидный JSON. Восстановление невозможно. Требуется повторный запрос.\nДетали: " + jsonErr.message);
        }
