...
function initSettingsUI() {
    ...
    const aiTtsGroup = document.getElementById('ai-tts-settings-group');
    const browserTtsGroup = document.getElementById('tts-voice-selector-group');
    const aiTtsKeyInput = document.getElementById('ai-tts-key-input');
    const aiTtsVoiceInput = document.getElementById('ai-tts-voice-input');

    if (ttsEngineSelect) ttsEngineSelect.value = ttsEngine;
    if (aiTtsKeyInput) aiTtsKeyInput.value = aiTtsKey;
    if (aiTtsVoiceInput) aiTtsVoiceInput.value = aiTtsVoice;

    const updateTtsView = () => {
        if (ttsEngineSelect.value === 'browser') {
            if (aiTtsGroup) aiTtsGroup.style.display = 'none';
            if (browserTtsGroup) browserTtsGroup.style.display = 'block';
        } else {
            if (aiTtsGroup) aiTtsGroup.style.display = 'block';
            if (browserTtsGroup) browserTtsGroup.style.display = 'none';
        }
    };
    if (ttsEngineSelect) {
        ttsEngineSelect.addEventListener('change', updateTtsView);
        updateTtsView();
    }
...
function saveSettings() {
    ...
    ttsEngine = document.getElementById('tts-engine-select')?.value || 'browser';
    aiTtsKey = document.getElementById('ai-tts-key-input')?.value.trim() || '';
    aiTtsVoice = document.getElementById('ai-tts-voice-input')?.value.trim() || 'alloy';
    
    localStorage.setItem('ttsEngine', ttsEngine);
    localStorage.setItem('aiTtsKey', aiTtsKey);
    localStorage.setItem('aiTtsVoice', aiTtsVoice);
