...
window.generateVisionImage = async function(prompt, btnElement, bubbleElement) {
    ...
    // Универсальный эндпоинт для OpenAI-совместимых провайдеров
    let targetUrl = '';
    if (imgApiProvider === 'llmost') targetUrl = 'https://llmost.ru/v1/images/generations';
    else if (imgApiProvider === 'openrouter') targetUrl = 'https://openrouter.ai/api/v1/images/generations';
    else targetUrl = 'https://api.openai.com/v1/images/generations';

    let headers = { 'Content-Type': 'application/json', 'Authorization': `Bearer ${imgApiKey}` };
    
    // СТРОГОЕ ПРАВИЛО: Экономный размер 512x512 (минимум токенов)
    let body = {
        model: imgModelId,
        prompt: prompt,
        n: 1,
    ...
    try {
        ...
        if (data.data && data.data[0] && data.data[0].url) {
            ...
            throw new Error('Ответ не содержит ссылки на изображение.');
        }
    } catch (e) {
        console.error('Image Gen Error:', e);
        showCustomAlert('Ошибка API: ' + e.message);
        btnElement.innerHTML = originalHtml;
        btnElement.style.pointerEvents = 'auto';
    }
};;
