                ...
                imgBtn.onclick = async (e) => {
                    ...
                    try {
                        ...
                        if (res && res.success && res.imageUrl) {
                            const finalSrc = res.imageUrl.startsWith('data:image') || res.imageUrl.startsWith('http') ? res.imageUrl : 'data:image/jpeg;base64,' + res.imageUrl;
                            const imgContainer = document.createElement('div');
                            imgContainer.className = 'chat-illustration-container';
                            imgContainer.innerHTML = `<img src="${finalSrc}" alt="Эфирное видение" title="Нажми, чтобы открыть оригинал" onclick="const w = window.open(''); w.document.write('<style>body{margin:0;background:#000;display:flex;justify-content:center;align-items:center;height:100vh;}img{max-width:100%;max-height:100vh;object-fit:contain;}</style><img src=\'${finalSrc}\'>');"><div class="img-watermark">Эфирный Слепок</div>`;
                            bubbleElement.appendChild(imgContainer);
                            
                            if (player && player.gameLogHistory && player.gameLogHistory.length > 0) {
                                player.gameLogHistory[player.gameLogHistory.length - 1].savedImageBase64 = finalSrc;
                            }
                            imgBtn.remove();
                        }
                    } catch (err) {
