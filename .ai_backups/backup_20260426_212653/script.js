...
function initWorldSimulator() {
    console.log("[WorldSim] Инициализация нового мира (Глубокая генерация)...");
    let newWorld = {
        time: { accumulatedMinutes: 0 },
        regions: {}, factions: {}, npcs: {}, news: [], animals: {}, weather: {}
    };

    // 1. Создаем УНИКАЛЬНЫЕ фракции с асимметричным балансом
    const baseFactions = [
        { id: "emp", name: "Империя Аквилон", g: [15000, 30000], f: [4000, 8000], m: [5000, 10000] },
        { id: "free", name: "Свободные Города", g: [25000, 50000], f: [3000, 6000], m: [2000, 5000] },
        { id: "dwf", name: "Горные Кланы", g: [10000, 20000], f: [1000, 3000], m: [4000, 8000] },
        { id: "elf", name: "Дети Леса", g: [2000, 5000], f: [8000, 15000], m: [1000, 3000] },
        { id: "cult", name: "Культ Бездны", g: [5000, 10000], f: [2000, 4000], m: [6000, 15000] }
    ];

    baseFactions.forEach(bf => {
        newWorld.factions[bf.id] = {
            name: bf.name,
            resources: {
            ...
            stability: 40 + Math.floor(Math.random() * 60),
            relations: {}, diplomacy: {}, armies: []
        };
    });

    // Дипломатия (кто кого ненавидит со старта)
    let fKeys = Object.keys(newWorld.factions);
    for (let f1 of fKeys) {
        for (let f2 of fKeys) {
            if (f1 !== f2) {
                newWorld.factions[f1].relations[f2] = Math.floor(Math.random() * 200) - 100; // от -100 до 100
                newWorld.factions[f1].diplomacy[f2] = newWorld.factions[f1].relations[f2] < -60 ? "war" : "neutral";
            }
        }
    }

    // 2. Распределяем локации по существующим фракциям и рандомизируем их
    for (let key in globalLocations) {
        if (key === 'startLocation') continue;
        let loc = globalLocations[key];
        let ownerId = fKeys[Math.floor(Math.random() * fKeys.length)];

        newWorld.regions[key] = {
            name: loc.name,
            owner: ownerId,
            climate: Math.random() > 0.5 ? "temperate" : (Math.random() > 0.5 ? "cold" : "tropical"),
            population: 1000 + Math.floor(Math.random() * 49000),
            resources: {
                food: Math.floor(Math.random() * 5000),
                wood: Math.floor(Math.random() * 5000),
                ore: Math.floor(Math.random() * 5000),
                weapons: Math.floor(Math.random() * 1000)
            },
            facilities: {
                farms: Math.floor(Math.random() * 12),
                lumbermills: Math.floor(Math.random() * 10),
                mines: Math.floor(Math.random() * 10),
                forges: Math.floor(Math.random() * 8)
            },
        ...
        newWorld.animals[key] = {
            herbivores: 100 + Math.floor(Math.random() * 4900),
            carnivores: 10 + Math.floor(Math.random() * 490)
        };
    }
