...
function processRulerDiplomacy() {
    ...
    for (let rId in World.rulers) {
        ...
        if (Math.random() < 0.15) { // 15% шанс принять решение в день
            ...
            if (ruler.personality.cruelty > 65 && power > 15000 && ruler.personality.ambition > 60) {
                ...
                World.intrigues.push({
                    ...
                    type: Math.random() > 0.5 ? "assassination" : "sabotage",
                    initiatorFactionId: ruler.factionId,
                    targetFactionId: targetF,
                    targetRulerId: World.factions[targetF].rulerId,
                    progress: 0, requiredProgress: 100, progressPerDay: Math.max(1, Math.floor(ruler.personality.paranoia / 15)),
                ...
                faction.relations[targetF] = Math.min(100, (faction.relations[targetF] || 0) + 20);
                generateWorldNews(`ДИПЛОМАТИЯ: ${ruler.name} отправляет дары во фракцию ${World.factions[targetF].name}, укрепляя отношения.`, "global", 2, 'misc');
    ...
    for (let i = World.intrigues.length - 1; i >= 0; i--) {
        ...
        if (intr.progress >= intr.requiredProgress) {
            if (intr.type === "assassination" && intr.targetRulerId && World.rulers[intr.targetRulerId]) {
                ...
                generateWorldNews(`ДИВЕРСИЯ: Экономика ${World.factions[intr.targetFactionId]?.name} пострадала от неизвестных саботажников.`, "global", 3, 'disaster');
