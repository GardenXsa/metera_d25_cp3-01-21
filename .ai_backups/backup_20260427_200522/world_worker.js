...
// Голод и Бунты (Причина: нет еды или работы)
    ...
    for (let fId of fKeys) {
        ...
        let foodPerCapita = f.globalFood / (f.totalPopulation || 1);
                ...
                else if (f.resources.gold.amount > 1000 && activeWarsCount === 0) f.stability += 1.5; // ФИКС: В мирное время стабильность уверенно растет
...
function processRulerDiplomacy() {
    ...
    for (let rId in World.rulers) {
        ...
        // ФИКС: Шанс принятия решения снижен с 20% до 2% в день. Правители не спамят действиями.
        if (Math.random() < 0.02) { 
            let targetF = fKeys[Math.floor(Math.random() * fKeys.length)];
            if (targetF === ruler.factionId) continue;

            if (ruler.personality.cruelty > 65 && power > 5000 && ruler.personality.ambition > 60) {
            ...
            } else if (ruler.personality.paranoia > 60 && Math.random() < 0.3) {
                ...
                World.intrigues.push({
                    ...
                    progress: 0, requiredProgress: selectedType === 'rebellion' ? 200 : 100, 
                    progressPerDay: Math.max(1, Math.floor(ruler.personality.paranoia / 20)),
                    discoveryChance: 5, isDiscovered: false, actors: [], gmInitiated: false, startDay: player?.stats?.turnCount || 0
                });
