...
function simulateOneDay() {
    ...
    for (let rId of rKeys) {
        ...
        if(r.facilities.farms) {
            addBatch(r.resources, 'wheat', Math.floor(workersPerSector * (r.facilities.farms.level / 5) * 5 * weatherMod * fert), currentDay, `Собрано на фермах (${r.name})`);
            addBatch(r.resources, 'cotton', Math.floor(workersPerSector * (r.facilities.farms.level / 15) * weatherMod * fert), currentDay, `Собрано на плантациях (${r.name})`);
            addBatch(r.resources, 'herbs', Math.floor(workersPerSector * (r.facilities.farms.level / 20) * weatherMod * fert), currentDay, `Собрано травниками (${r.name})`);
        }
    ...
    for (let fId of fKeys) {
        ...
        for (let targetF of fKeys) {
            ...
            // ЗАКЛЮЧЕНИЕ МИРА (С УЧЕТОМ ВОЕННОЙ УСТАЛОСТИ МИРА)
                ...
                if (f.resources.manpower.amount < 5000 || targetFaction.resources.manpower.amount < 5000 || f.stability < 30 || Math.random() < peaceChance) {
    ...
    for (let rId in World.rulers) {
        ...
        if (Math.random() < 0.20) { 
            ...
            if (ruler.personality.cruelty > 65 && power > 15000 && ruler.personality.ambition > 60) {
                ...
                if (warWeary > 75 && Math.random() < 0.8) {
