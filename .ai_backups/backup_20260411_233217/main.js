...
ipcMain.handle('speak-text', async (event, text, voiceModel) => {
    ...
    return new Promise((resolve) => {
        let ttsDir;
        if (app.isPackaged) {
            // Возвращаем твою родную иерархию: распакованная папка assets/tts
        ...
        const piper = spawn(piperExe, [
            ...
            env: {
                ...process.env,
                ESPEAK_DATA_PATH: ttsDir // КРИТИЧЕСКИ ВАЖНО: Указываем путь к словарям
            },
            windowsHide: true 
        });

        // Перехватываем текстовые ошибки от Piper
