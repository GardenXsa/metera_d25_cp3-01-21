...
// --- СИСТЕМА ТАЙЛОВ КАРТЫ ---
const mapTiles = {};
const TILE_SIZE = 40; // Размер гекса
let tilesLoaded = false;

async function initMapTiles() {
    const promises = [];
    for (let i = 1; i <= 30; i++) {
        promises.append(new Promise((resolve) => {
            const img = new Image();
            img.src = `assets/map/tiles/${i}.png`;
            img.onload = () => { mapTiles[i] = img; resolve(); };
            img.onerror = () => resolve(); // Пропускаем битые
        }));
    }
    await Promise.all(promises);
    tilesLoaded = true;
    console.log("Map tiles loaded: ", Object.keys(mapTiles).length);
}
...
function renderVisualMap() {
    if (!mapContext || !mapCanvas || !player || !tilesLoaded) return;
    ...
    // Центрирование (оставляем твою логику)
    if (!isMapInitialized) {
        const currentLocName = (player.location || "").toLowerCase().trim();
        const playerPoint = allPoints.find(p => p.name.toLowerCase().trim().includes(currentLocName));
        if (playerPoint) {
    ...
    // 1. ФОН (ГЕНЕРАЦИЯ ЛЕСА)
    ctx.fillStyle = '#1a2530'; // Темная подложка
    ctx.fillRect(0, 0, width, height);

    const hexW = TILE_SIZE * 2 * mapState.zoom;
    const hexH = Math.sqrt(3) * TILE_SIZE * mapState.zoom;
    
    // Рисуем сетку леса, покрывающую весь видимый экран
    for (let row = -5; row < (height / (hexH * 0.75)) + 5; row++) {
        for (let col = -5; col < (width / hexW) + 5; col++) {
            let x = col * hexW + (row % 2 === 0 ? 0 : hexW / 2) + (mapState.offsetX % hexW);
            let y = row * (hexH * 0.75) + (mapState.offsetY % (hexH * 0.75));
            
            // Псевдослучайный выбор лесного тайла (1-15)
            const tileIdx = ((Math.abs(row * 31 + col * 17)) % 15) + 1;
            if (mapTiles[tileIdx]) {
                ctx.drawImage(mapTiles[tileIdx], x - hexW/2, y - hexH/2, hexW, hexH);
            }
        }
    }

    // 2. ДОРОГИ (ИСПОЛЬЗОВАНИЕ АССЕТОВ 16-30)
    allPoints.forEach(p1 => {
        ...
        neighbors.forEach(n => {
            const start = { x: Number(p1.x) * mapState.zoom + mapState.offsetX, y: Number(p1.y) * mapState.zoom + mapState.offsetY };
            const end = { x: Number(n.point.x) * mapState.zoom + mapState.offsetX, y: Number(n.point.y) * mapState.zoom + mapState.offsetY };
            
            // Рисуем тайлы дорог вдоль линии
            const steps = Math.floor(n.dist / (TILE_SIZE * 0.8));
            for (let i = 0; i <= steps; i++) {
                const tx = start.x + (end.x - start.x) * (i / steps);
                const ty = start.y + (end.y - start.y) * (i / steps);
                // Случайный дорожный тайл (16-30)
                const roadIdx = 16 + (Math.abs(Math.floor(tx + ty)) % 15);
                if (mapTiles[roadIdx]) {
                    ctx.drawImage(mapTiles[roadIdx], tx - hexW/2, ty - hexH/2, hexW, hexH);
                }
            }
        });
    });

    // 3. МАРКЕРЫ (ПОВЕРХ ВСЕГО)
    allPoints.forEach(point => {
        const pos = { x: Number(point.x) * mapState.zoom + mapState.offsetX, y: Number(point.y) * mapState.zoom + mapState.offsetY };
        const isPlayer = player.location && player.location.toLowerCase().includes(point.name.toLowerCase());
        drawMapMarker(ctx, pos, getLocationType(point.name), isPlayer);
        
        ctx.font = `bold ${10 * mapState.zoom}px 'MedievalSharp'`;
        ctx.fillStyle = '#fff';
        ctx.textAlign = 'center';
        ctx.shadowBlur = 4; ctx.shadowColor = "black";
        ctx.fillText(point.name.split('(')[0].trim(), pos.x, pos.y + 20);
        ctx.shadowBlur = 0;
    });
