...
console.log('[DEBUG] Raw saves from main:', rawSaves);
            ...
            return rawSaves.map(save => {
                ...
                return {
                    slotType: save.slotType || type,
                    slotId: save.slotId || id,
                    timestamp: save.timestamp,
                    playerData: save.playerData,
                    fileName: save.filename
                };
            }).filter(s => s.slotType && s.slotId !== undefined); // Отфильтровываем некорректные имена
...
async function populateLoadGameScreen() {
    console.log("[DEBUG] populateLoadGameScreen ЗАПУЩЕНА");
    try {
        if (!manualSaveSlotsList || !autoSaveSlotsList) {
            console.error("[CRITICAL] Элементы списка сохранений не найдены в DOM!");
            return;
        }
        ...
        console.log("[DEBUG] Запрашиваю список из Electron API...");
    ...
    if (window.electronAPI && window.electronAPI.isElectron) {
        try {
            ...
            console.log("[DEBUG] Получено файлов от Electron:", fsSaves.length);
...
function populateLoadGameScreen() {
    ...
    if (autoSaves.length > 0) {
        ...
        console.error("[DEBUG] ОШИБКА В populateLoadGameScreen:", err);
    }
}
