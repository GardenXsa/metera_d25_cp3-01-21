        ...
        for (let i = f.armies.length - 1; i >= 0; i--) {
            ...
            if (army.destination && army.location !== army.destination) {
                ...
                if (army.daysToMove <= 0) {
                    ...
                    let regionOwner = World.regions[army.location].owner;
                    if (regionOwner && f.diplomacy[regionOwner] === "war") {
                        let defender = World.factions[regionOwner];
                        let defArmyIndex = defender.armies.findIndex(a => a.location === army.location);
                        ...
                        if (defArmyIndex !== -1) {
                            ...
                            if (atkPower > defPower) {
                                ...
                                generateWorldNews(`Битва за ${World.regions[army.location].name}: Армия ${f.name} разбила войска ${defender.name}! Потери защитников: ${casualties}.`, army.location, 5);
                                defender.armies.splice(defArmyIndex, 1);
                            } else {
                                let casualties = Math.floor(army.size * 0.8);
                                defArmy.size -= Math.floor(defArmy.size * 0.2);
                                generateWorldNews(`Битва за ${World.regions[army.location].name}: Войска ${defender.name} отбили атаку ${f.name}! Потери нападавших: ${casualties}.`, army.location, 5);
                                f.armies.splice(i, 1);
                            }
                        } else {
                            World.regions[army.location].owner = fId;
                            generateWorldNews(`Регион ${World.regions[army.location].name} захвачен фракцией ${f.name}!`, army.location, 5);
                        }
                    }
                }
            }
...
function buildFullPlayerSnapshot() {
    ...
    if (World) {
        ...
        // 4. Слухи
        let recentNews = World.news.filter(n => n.daysOld <= 2);
        if (recentNews.length > 0) {
            worldContextString += "[СВЕЖИЕ МИРОВЫЕ СЛУХИ]\n" + recentNews.map(n => `- ${n.text}`).join("\n") + "\n";
        }
        worldContextString += "==================================================\n";
    }
