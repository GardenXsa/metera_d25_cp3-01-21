...
function setupEventListeners() {
    ...
    if (communityButton) communityButton.addEventListener('click', () => window.open('https://discord.com/invite/kDnTx2HAvT', '_blank'));

    // --- Настройки ---
    if (saveSettingsButton) saveSettingsButton.addEventListener('click', saveSettings);
    if (settingsBackButton) settingsBackButton.addEventListener('click', () => setActiveScreen(settingsReturnScreen));

    // --- Игра ---
    if (rollDiceButton) rollDiceButton.addEventListener('click', handleRollDiceButtonClick);
    ...
    if (inGameSettingsButton) inGameSettingsButton.addEventListener('click', openSettingsFromGame);
    ...
    bodySlots.forEach(slot => {
        ...
        if (el) {
            el.addEventListener('click', (e) => {
                if (e.currentTarget.classList.contains('equipped')) {
                    unequipItem(e.currentTarget.dataset.slot);
                }
            });
