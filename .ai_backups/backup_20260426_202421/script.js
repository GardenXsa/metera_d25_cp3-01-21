...
async function runWorldSimulationTick() {
    ...
    loaderDiv.innerHTML = `
        ...
        <div class="ether-text-container">
            ...
            <span class="ether-text-subtitle">Шестерни мира совершают оборот</span>
        </div>
    `;
    gameLog.appendChild(loaderDiv);
    gameLog.scrollTo({ top: gameLog.scrollHeight, behavior: 'smooth' });
    ...
    try {
        const prompt = "Ты — ДВИЖОК МИРА (WORLD SIMULATOR). Твоя задача: взять ОДНО событие со статусом 'зреет' (или 'brewing') и перевести его в активную фазу.\n\nПРИКАЗ:\n1. Верни СТРОГО JSON. Никакого текста до или после.\n2. В массиве actions используй `nexusUpdate`, чтобы изменить `value` выбранного события на 'АКТИВНО'.\n3. В поле `narrative` напиши ЭПИЧНЫЙ ХУДОЖЕСТВЕННЫЙ ТЕКСТ (2-3 абзаца), описывающий катастрофу, начало войны или политический сдвиг. Опиши масштабы, страх людей и последствия.\n4. Опционально: используй `setMemory` для записи факта для GM.";
        
        const modelId = currentApiProvider === 'gemini' ? geminiModelId : (currentApiProvider === 'llmost' ? llmostModelId : openrouterModelId);
        const raw = await performAiFetch(prompt, [], modelId, `Текущий ход: ${player.stats.turnCount}
Игровое время: День ${player.gameTime.day}, ${player.gameTime.hour < 10 ? '0'+player.gameTime.hour : player.gameTime.hour}:${player.gameTime.minute < 10 ? '0'+player.gameTime.minute : player.gameTime.minute} (${player.timeOfDay}). Доступные события: ${JSON.stringify(events)}`);
        ...
        if (loaderDiv) loaderDiv.remove();
        addLogMessage("(( Сбой симуляции мира. ))", "system-message");
    } finally { 
