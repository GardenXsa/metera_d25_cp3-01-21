...
async function _internalPerformAiFetch(systemInstruction, history, providerModel, currentInput = "") {
    ...
    while (attempts < maxAttempts) {
        ...
        if (!response.ok) {
            ...
            // Авто-отключение кэширования, если модель его не поддерживает
            if (response.status === 400 && errText.includes("prompt_caching")) {
                console.warn('[API] Модель не поддерживает prompt_caching, отключаю для этого запроса...');
                addLogMessage("[СИСТЕМА] Модель не поддерживает Prompt Caching, отключаю...", "system-message");
