...
async function initializeApp() {
    ...
    mapDetailLevel = localStorage.getItem('mapDetailLevel') || 'medium';
...
function initSettingsUI() {
    ...
    const mapDetailSelect = document.getElementById('map-detail-select');
    if (mapDetailSelect) mapDetailSelect.value = mapDetailLevel;
...
function saveSettings() {
    ...
    mapDetailLevel = document.getElementById('map-detail-select')?.value || 'medium';
    localStorage.setItem('mapDetailLevel', mapDetailLevel);
    }
...
async function generateLocalMapFromAI(description, size) {
    console.log(`[CityGen] Запрос генерации карты: ${size}, Детализация: ${mapDetailLevel}`);
    try {
        ...
        const detailInstructions = {
            low: "Focus only on essential walls and clear floor. Keep it simple.",
            medium: "Add standard furniture and decorations logically.",
            high: "Rich layering. Use z:1 for scattering debris/blood/carpets. Ensure varied objects.",
            ultra: "MASTERPIECE MODE. You are a Senior Level Designer. Use all 3 layers densely. Mix ground textures (e.g. mix grass with dirt for natural look). Add atmospheric details. Every tile must feel handcrafted. SURPRISE THE PLAYER WITH YOUR CREATIVITY!"
        };

        const finalPrompt = promptTemplate
            .replace('{size}', size)
            .replace('{description}', description)
            .replace('{detail_level_instruction}', detailInstructions[mapDetailLevel] || detailInstructions.medium)
        ...
        const rawResponse = await performAiFetch(finalPrompt, [], modelId, "Сгенерируй JSON план локации согласно инструкции. ТЫ ОБЯЗАН ЗАПОЛНИТЬ ВСЕ КЛЕТКИ СЕТКИ!");
        ...
        if (result.actions && Array.isArray(result.actions)) {
            ...
            if (mapAction && mapAction.args && mapAction.args.plots) {
                plots = mapAction.args.plots;
            }
        }

        if (!plots && result.plots) plots = result.plots;
...
function renderCanvasMap(plots, width, height, canvas) {
    ...
    // 1. Сортируем тайлы по слою (Z-index), чтобы земля рисовалась первой, а объекты поверх нее
    const sortedPlots = [...plots].sort((a, b) => (a.z || 0) - (b.z || 0));

    // 2. Отрисовываем все слои
    for (const plot of sortedPlots) {
        const x = plot.x;
        const y = plot.y;
        if (x < 0 || x >= width || y < 0 || y >= height) continue;

        const type = plot.type || 'void';
        ...
        ctx.save();
        ctx.translate(x * RENDER_TILE_SIZE + RENDER_TILE_SIZE / 2, y * RENDER_TILE_SIZE + RENDER_TILE_SIZE / 2);
        ctx.rotate(rotation * Math.PI / 180);
        
        ctx.drawImage(
            TILESET_IMAGE, 
            srcX, srcY, SOURCE_TILE_SIZE, SOURCE_TILE_SIZE, 
            -RENDER_TILE_SIZE / 2, -RENDER_TILE_SIZE / 2, RENDER_TILE_SIZE, RENDER_TILE_SIZE
        );
        
        ctx.restore();
    }

    // 3. Отрисовка сетки поверх всего
