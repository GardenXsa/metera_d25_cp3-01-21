...
async function prepareLogicPrompt(expiredEffects, promptTextForAI, isInitial = false) {
    try {
        ...
        const eraContext = activeEraSpecialLore || "Данные по конкретной эпохе не предоставлены.";
...
### СТРОГИЙ РЕЕСТР ТЕКУЩЕЙ ЭПОХИ (ФАУНА, ФЛОРА, ИЕРАРХИЯ):
${eraContext}
...
### ЛОР МИРА:
### СПЕЦИФИКА ТЕКУЩЕЙ ЭПОХИ (ИСПОЛЬЗОВАТЬ ДЛЯ АТМОСФЕРЫ):
${eraContextNarrative}
