...
ИНВЕНТАРЬ (ПОЛНЫЙ):
${JSON.stringify(player.container_backpack ? ContainerRegistry.get(player.container_backpack).items.map(id => { let it = ItemRegistry.get(id); return it ? { id: it.prototype_id, name: it.custom_props.name, quantity: it.stack_size } : null; }).filter(Boolean) : {}, null, 2)}

ЭКИПИРОВКА (ПО СЛОТАМ):
${JSON.stringify(player.container_equipment ? ContainerRegistry.get(player.container_equipment).items.map(id => { let it = ItemRegistry.get(id); return it ? { slot: it.slot_index, id: it.prototype_id, name: it.custom_props.name } : null; }).filter(Boolean) : {}, null, 2)}
...
case 'setEntityBinding':
            ...
            case 'initiateTrade':
                if (args.targetId) {
                    feedback = `[ТОРГОВЛЯ] Инициирована торговля с ${args.targetId}. (Требует подтверждения)`;
                }
                break;
            case 'confirmTrade':
                if (args.tradeId) {
