...
worldWorker.onmessage = function(e) {
    ...
    else if (data.type === 'TICK_DONE') {
        World = data.World;
        isSimulatingWorld = false;
        updateWorldChroniclesDisplay();
        updateTradeJournalDisplay();
        
        // Если есть новые новости, выводим последнюю
        if (data.news && data.news.length > 0) {
            const latestNews = data.news[data.news.length - 1];
            if (latestNews.daysOld === 0) {
                addLogMessage(latestNews.text, "world-event");
            }
        }
        
        const loaderDiv = document.getElementById('world-sim-loader');
...
function initWorldSimulator() {
    worldWorker.postMessage({ action: 'init', ECONOMY_ITEMS, CRAFTING_RECIPES });
    // Возвращаем пустую болванку, воркер сам ее наполнит
    return { time: { accumulatedMinutes: 0, lastEventPulse: 0 }, regions: {}, factions: {}, npcs: {}, news: [], animals: {}, weather: {}, rulers: {}, intrigues: [] };
}
...
async function runWorldSimulationTick() {
    ...
    worldWorker.postMessage({ action: 'runTick', World, player, globalLocations });
}
// ======================================================================
