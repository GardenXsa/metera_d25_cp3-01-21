...
function executeCommand(command, args) {
    ...
    try {
        switch (command) {
            ...
            case 'renderLocation':
                if (args.size && args.plots && Array.isArray(args.plots)) {
                    player.localMap = args;
                    buildLocalMap(args);
                    feedback = `[СИСТЕМА КАРТЫ] Сгенерирована локальная зона: ${args.locationId || 'Новая локация'}.`;
                    addCalculationMessage(feedback);
                } else {
                    feedback = `[ERROR] 'renderLocation' требует 'size' (например "8x8") и массив 'plots'.`;
                }
                break;
...
function buildLocalMap(args) {
    ...
    if (!localMapPanel || !gridContainer || !inspector) return;

    localMapPanel.style.display = 'flex'; // Показываем панель
    if (!localMapPanel.classList.contains('expanded')) {
        localMapPanel.querySelector('.panel-toggle').click(); // Автоматически разворачиваем
    }
    ...
    inspector.innerHTML = '<p style="color: #7f8c8d; font-style: italic; text-align: center;">Нажмите на клетку для осмотра</p>';
}
