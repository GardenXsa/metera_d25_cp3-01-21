...
function simulateOneDay() {
    ...
    for (let rId of rKeys) {
        ...
        if(r.facilities.farms) {
            addBatch(r.resources, 'wheat', Math.floor(workersPerSector * (r.facilities.farms.level / 5) * 5 * weatherMod), currentDay, `Собрано на фермах (${r.name})`);
            addBatch(r.resources, 'cotton', Math.floor(workersPerSector * (r.facilities.farms.level / 15) * weatherMod), currentDay, `Собрано на плантациях (${r.name})`);
            addBatch(r.resources, 'herbs', Math.floor(workersPerSector * (r.facilities.farms.level / 20) * weatherMod), currentDay, `Собрано травниками (${r.name})`);
        }
        if(r.facilities.lumbermills) addBatch(r.resources, 'wood', Math.floor(workersPerSector * (r.facilities.lumbermills.level / 10) * weatherMod), currentDay, `Вырублено на лесопилке (${r.name})`);
        if(r.facilities.mines) {
            addBatch(r.resources, 'iron_ore', Math.floor(workersPerSector * (r.facilities.mines.level / 10)), currentDay, `Добыто в шахтах (${r.name})`);
            addBatch(r.resources, 'gold_ore', Math.floor(workersPerSector * (r.facilities.mines.level / 30)), currentDay, `Добыто в золотых жилах (${r.name})`);
        }
        ...
        for (let recipe of CRAFTING_RECIPES) {
            ...
            if (maxCrafts > 0) {
                ...
                for (let outRes in recipe.outputs) {
                    ...
                    addBatch(r.resources, outRes, produced, currentDay, `Произведено в ${r.name} (Здание: ${fac.level} ур.)`);
                }
            }
        }
