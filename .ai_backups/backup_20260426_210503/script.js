...
function buildFullPlayerSnapshot() {
    ...
    if (World) {
        ...
        worldContextString += "\n--- ГЛОБАЛЬНЫЙ БАЛАНС СИЛ (РЕАЛЬНЫЕ ДАННЫЕ ДВИЖКА) ---\n";
        let activeWars = [];
        for (let fId in World.factions) {
            let f = World.factions[fId];
            worldContextString += `Фракция: ${f.name} | Казна: ${f.resources.gold} | Еда: ${f.resources.food} | Армия: ${f.resources.manpower} | Стабильность: ${f.stability}%\n`;
            
            for (let target in f.diplomacy) {
                if (f.diplomacy[target] === "war") {
                    let warStr = `${f.name} воюет с ${World.factions[target].name}`;
                    if (!activeWars.includes(`${World.factions[target].name} воюет с ${f.name}`)) {
