...
function initWorldSimulator() {
    ...
    for (let key in globalLocations) {
        ...
        newWorld.regions[key] = {
            ...
            markets: { food: 1, wood: 2, ore: 5, weapons: 20 },
            caravans: []
        };
    ...
    if (rKeys.length > 0) {
        for(let i=0; i<40; i++) {
            ...
            newWorld.npcs[id] = {
                ...
                relationships: {}, memory: [], inventory: { gold: Math.floor(Math.random()*100), items: {} },
...
function simulateOneDay() {
    ...
    for (let rId in World.regions) {
        ...
        if (Math.random() < 0.015) {
            ...
            generateWorldNews(`СТИХИЙНОЕ БЕДСТВИЕ! В регионе ${r.name} произошло событие: ${disaster}. ${damageStr}`, rId, 5);
        }
    }
    ...
    for (let rId of rKeys) {
        ...
        if (r.resources.food < 0) {
            ...
            r.population -= deaths;
            if (Math.random() < 0.2) generateWorldNews(`Ужасный голод в ${r.name}! Погибло ${deaths} человек.`, rId, 4);
        }
    ...
    for (let fId of fKeys) {
        ...
        for (let targetF of fKeys) {
            ...
            if (rel < -40 && currentStatus !== "war") {
                ...
                generateWorldNews(`Фракция ${f.name} объявляет войну ${World.factions[targetF].name}!`, "global", 5);
            }
                ...
                generateWorldNews(`Фракции ${f.name} и ${World.factions[targetF].name} заключили военный союз.`, "global", 4);
            }
            else if (rel > -20 && currentStatus === "war") {
                f.diplomacy[targetF] = "neutral";
                World.factions[targetF].diplomacy[fId] = "neutral";
                generateWorldNews(`Фракции ${f.name} и ${World.factions[targetF].name} подписали мирный договор.`, "global", 5);
            }
        }
        ...
        if (atWarWith && f.resources.gold > 1000 && f.resources.manpower > 300) {
            ...
            f.armies.push({ id: armyId, size: 500, morale: 100, location: homeRegion, destination: targetRegion, daysToMove: 3 });
            generateWorldNews(`Армия фракции ${f.name} (500 воинов) выступила в поход на ${World.regions[targetRegion].name}.`, homeRegion, 3);
        }

        for (let i = f.armies.length - 1; i >= 0; i--) {
            let army = f.armies[i];
            if (army.destination && army.location !== army.destination) {
                army.daysToMove--;
                if (army.daysToMove <= 0) {
                    ...
                    if (regionOwner && f.diplomacy[regionOwner] === "war") {
                        let defender = World.factions[regionOwner];
                        let defArmyIndex = defender.armies.findIndex(a => a.location === targetLoc);
                        ...
                        if (defArmyIndex !== -1) {
                            ...
                            if (atkPower > defPower) {
                                ...
                                generateWorldNews(`Битва за ${World.regions[targetLoc].name}: Армия ${f.name} разбила войска ${defender.name}! Потери защитников: ${casualties}.`, targetLoc, 5);
                                defender.armies.splice(defArmyIndex, 1);
                            } else {
                                let casualties = Math.floor(army.size * 0.8);
                                defArmy.size -= Math.floor(defArmy.size * 0.2);
                                generateWorldNews(`Битва за ${World.regions[targetLoc].name}: Войска ${defender.name} отбили атаку ${f.name}! Потери нападавших: ${casualties}.`, targetLoc, 5);
                                f.armies.splice(i, 1);
                                armySurvived = false;
                            }
                        } else {
                            World.regions[targetLoc].owner = fId;
                            generateWorldNews(`Регион ${World.regions[targetLoc].name} захвачен фракцией ${f.name}!`, targetLoc, 5);
                        }
                    }
                    
                    // ИСПРАВЛЕНИЕ: Если армия выжила (победила или захватила пустой регион),
                    // она распускается, а выжившие солдаты возвращаются в пул рекрутов фракции.
                    if (armySurvived && f.armies[i]) {
...
function generateWorldNews(text, location, importance) {
    World.news.push({
    ...
    console.log("[WorldSim NEWS]: " + text);
}
...
function updateWorldChroniclesDisplay() {
    ...
    // 1. Получаем глобальные события из Nexus (от ИИ)
    const events = Object.values(player.nexusData || {}).filter(item => 
        item.category === 'World_Event' || item.category === 'Мировое Событие' || item.id.startsWith('event_')
    );

    // 2. Получаем фоновые новости из симулятора движка (от JS)
    let simNews = [];
    ...
    if (events.length === 0 && simNews.length === 0) {
        listEl.innerHTML = `<li data-i18n="gameInterface.worldChroniclesPanel.empty">${t('gameInterface.worldChroniclesPanel.empty', 'Мир пока спокоен...')}</li>`;
        return;
    }
    ...
    simNews.sort((a, b) => a.daysOld - b.daysOld).forEach(news => {
        ...
        li.innerHTML = `
            <div style="display:flex; justify-content: space-between; align-items: flex-start;">
                <span class="chronicle-title" style="color: #f39c12; font-size: 0.9em;">Слухи (${news.daysOld} дн. назад)</span>
                <span class="chronicle-status-brewing"><i class="fas fa-newspaper"></i> Фон</span>
            </div>
            <div class="chronicle-desc" style="color: #aeb6bf;">${news.text}</div>
        `;
        listEl.appendChild(li);
    });
}
    ...
    updateWorldSimDebugDisplay();
            ...
            case 'endJourney':
                ...
                break;
                        ...
                        case 'advanceTime':
                if (args.pulses !== undefined) {
                    let p = parseInt(args.pulses, 10);
                    if (!isNaN(p) && p > 0) {
                        advanceTime(p);
                        feedback = `[ВРЕМЯ] Прошло времени: ${p * 5} минут.`;
                        addCalculationMessage(feedback);
                    }
                } else {
                    feedback = `[ERROR] 'advanceTime' требует аргумент 'pulses'.`;
                }
                break;
        ...
        if (res.narrative) {
            ...
            const cardHtml = `
                <div class="world-event-card">
                    ...
                    <div class="world-event-body">${DOMPurify.sanitize(marked.parse(res.narrative))}</div>
...
function updateWorldSimDebugDisplay() {
    ...
    if (DEBUG_MODE && typeof World !== 'undefined' && World) {
        ...
        // 1. ФРАКЦИИ
        html += '<h4>ФРАКЦИИ И ДИПЛОМАТИЯ</h4><ul>';
        for (let fId in World.factions) {
            let f = World.factions[fId];
            let wars = [];
            for (let t in f.diplomacy) { if (f.diplomacy[t] === "war" && World.factions[t]) wars.push(World.factions[t].name); }
            let warText = wars.length > 0 ? `<br><span style="color:#e74c3c; font-size:0.9em;">⚔️ Воюет с: ${wars.join(', ')}</span>` : '';
            
            html += `<li><b style="color:#3498db">${f.name}</b><br>
                     <span class="sim-icon">💰</span>${Math.floor(f.resources.gold)} | 
                     <span class="sim-icon">🍖</span>${Math.floor(f.resources.food)} | 
                     <span class="sim-icon">🛡️</span>${Math.floor(f.resources.manpower)} | 
                     <span class="sim-icon">⚖️</span>${Math.floor(f.stability)}%${warText}</li>`;
        }
        html += '</ul>';

        // 2. РЕГИОНЫ
        html += '<h4>РЕГИОНЫ (ЭКОНОМИКА)</h4><ul>';
        let activeCaravans = 0;
        for (let rId in World.regions) {
            let r = World.regions[rId];
            let an = World.animals[rId] || {herbivores: 0, carnivores: 0};
            let owner = World.factions[r.owner] ? World.factions[r.owner].name : "Нейтралы";
            activeCaravans += r.caravans.length;
            
            html += `<li><b style="color:#2ecc71">${r.name}</b> <span style="font-size:0.8em; color:#7f8c8d;">(${owner})</span><br>
                     Поп: ${Math.floor(r.population)} | Погода: ${World.weather[rId]}<br>
                     <span style="color:#f39c12; font-size:0.9em;">Здания: 🌾${r.facilities.farms} 🪓${r.facilities.lumbermills} ⛏️${r.facilities.mines} ⚒️${r.facilities.forges}</span><br>
                     <span style="color:#bdc3c7; font-size:0.9em;">Склад: 🍖${Math.floor(r.resources.food)} 🪵${Math.floor(r.resources.wood)} 🪨${Math.floor(r.resources.ore)} 🗡️${Math.floor(r.resources.weapons)}</span><br>
                     <span style="color:#e67e22; font-size:0.9em;">Цены: 🍖${r.markets.food} 🪵${r.markets.wood} 🪨${r.markets.ore} 🗡️${r.markets.weapons}</span><br>
                     <span style="color:#9b59b6; font-size:0.9em;">Фауна: 🦌${Math.floor(an.herbivores)} 🐺${Math.floor(an.carnivores)}</span></li>`;
        }
        html += '</ul>';

        // 3. ЛОГИСТИКА И ВОЙНА
        html += '<h4>АКТИВНОСТЬ НА КАРТЕ</h4><ul>';
        html += `<li><span style="color:#f1c40f">Караванов в пути: ${activeCaravans}</span></li>`;
        
        let armiesExist = false;
        for (let fId in World.factions) {
            World.factions[fId].armies.forEach(a => {
                armiesExist = true;
                let loc = World.regions[a.location] ? World.regions[a.location].name : a.location;
                let dest = World.regions[a.destination] ? World.regions[a.destination].name : a.destination;
                html += `<li><b style="color:#e74c3c">${World.factions[fId].name}</b>: Армия (${a.size} ед.)<br>Марш: ${loc} ➔ ${dest} (Осталось: ${a.daysToMove} дн.)</li>`;
            });
        }
        if (!armiesExist) html += '<li style="color:#7f8c8d; font-style:italic;">Нет активных передвижений войск</li>';
        html += '</ul>';
