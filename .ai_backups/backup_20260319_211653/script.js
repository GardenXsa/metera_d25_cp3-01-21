...
function saveGame(slotType, slotId) {
    if (!player) {
    ...
    try {
        ...
        saveData = {
            ...
            historyData: structuredClone(conversationHistory.slice(-MAX_HISTORY * 2)) // Сохраняем последние MAX_HISTORY пар сообщений
        };
    } catch (e) {
...
function loadGame(slotType, slotId) {
    const allSaves = getAllSaves();
    const targetArray = allSaves[slotType];
