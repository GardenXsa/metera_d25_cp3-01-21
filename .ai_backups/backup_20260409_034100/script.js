...
const TILE_SPRITE_MAP = {
    // --- 0: Природа (Ground) ---
    'n_grass':       { x: 0, y: 0 },    // Трава
    'dirt':          { x: 1, y: 0 },    // Земля
    'stone_floor':   { x: 2, y: 0 },    // Камень
    'mud':           { x: 3, y: 0 },    // Грязь
    'water_deep':    { x: 4, y: 0 },    // Вода (Глубокая)
    'sand':          { x: 6, y: 0 },    // Песок

    // --- 1: Растительность ---
    'tree_pine_dark':{ x: 0, y: 1 },    // Ель
    'tree_oak':      { x: 1, y: 1 },    // Лиственное дерево
    'tree_dead':     { x: 3, y: 1 },    // Мертвое дерево
    'bush_dry':      { x: 5, y: 1 },    // Куст
    'n_flower_red':  { x: 11, y: 1 },   // Красный цветок

    // --- 2: Архитектура (Стены/Двери) ---
    'wall_stone':    { x: 0, y: 2 },    // Каменная стена
    'wall_wood':     { x: 2, y: 2 },    // Деревянная стена
    'wall_cave':     { x: 5, y: 2 },    // Пещерная стена
    'door_wood':     { x: 9, y: 2 },    // Деревянная дверь
    'd_door':        { x: 10, y: 2 },   // Железная дверь

    // --- 3: Интерьер и Объекты ---
    'table':         { x: 15, y: 3 },   // Стол
    'c_bed':         { x: 16, y: 3 },   // Кровать
    'chest':         { x: 20, y: 3 },   // Сундук
    'campfire':      { x: 25, y: 3 },   // Костер
    'torch':         { x: 26, y: 3 },   // Факел на стене
    'bones':         { x: 29, y: 3 },   // Кости
    'blood':         { x: 31, y: 3 },   // Пятно крови

    // --- 4: Городские полы ---
    'c_floor_cobble':{ x: 0, y: 4 },    // Брусчатка
    'wood_floor':    { x: 1, y: 4 },    // Доски
    'c_floor_carpet':{ x: 2, y: 4 },    // Ковер

    // --- 5: Dungeon (Подземелье) ---
    'd_wall':        { x: 0, y: 5 },    // Стена тюрьмы
    'd_floor':       { x: 1, y: 5 },    // Пол кирпичный
    'd_barrel':      { x: 5, y: 5 },    // Бочка
    'd_crate':       { x: 6, y: 5 },    // Ящик
    'd_pillar':      { x: 9, y: 5 },    // Колонна

    // --- 6: Аномалии и Магия ---
    'h_floor_lava':  { x: 0, y: 6 },    // Лава
    'm_portal_blue': { x: 1, y: 6 },    // Портал
    'm_crystal_blue':{ x: 3, y: 6 },    // Кристалл маны
    'h_wall_obsidian':{ x: 5, y: 6 },   // Темная стена

    // --- 7: Зима ---
    's_snow_ground': { x: 0, y: 7 },    // Снег
    's_wall_ice':    { x: 2, y: 7 },    // Лед
    's_tree_pine':   { x: 4, y: 7 },    // Заснеженная ель

    // Системные
    'void':          { x: 12, y: 0 }    // Пустота (черный тайл)
};

function getSpriteCoords(type) {
    return TILE_SPRITE_MAP[type] || TILE_SPRITE_MAP['void'];
}
...
async function loadTileSet() {
    return new Promise((resolve) => {
        ...
        // Путь к файлу согласно архитектуре проекта
