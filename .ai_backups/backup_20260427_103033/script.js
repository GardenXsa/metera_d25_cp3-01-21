    ...
    for (let fId in fConfig) {
        ...
        newWorld.factions[fId] = {
            ...
            resources: {
                ...
                food: { amount: bf.f[0] + Math.floor(Math.random() * (bf.f[1] - bf.f[0])), quality: 1.0 },
...
function simulateOneHour() {
    ...
    for (let rId in World.regions) {
        ...
        for (let i = region.caravans.length - 1; i >= 0; i--) {
            ...
            if (destRegion && Math.random() < 0.005) {
            ...
            if (caravan.hoursLeft <= 0) {
                if (destRegion) {
                    for (let good in caravan.goods) {
                        ...
                        // ИСПРАВЛЕНО: Обращение к .amount
                        destRegion.resources[good].amount = (destRegion.resources[good].amount || 0) + amount;
...
function simulateOneDay() {
    ...
    for (let rId in World.regions) {
        ...
        if (Math.random() < 0.015) {
            ...
            if (disaster === "Землетрясение") {
                ...
                if (r.facilities.farms.level > 0) r.facilities.farms.level--;
                if (r.facilities.mines.level > 0) r.facilities.mines.level--;
                ...
                // ИСПРАВЛЕНО: Обращение к .amount
                r.resources.food.amount = Math.floor(r.resources.food.amount * 0.3);
                r.resources.wood.amount = Math.floor(r.resources.wood.amount * 0.4);
                damageStr = "Урожай и леса уничтожены.";
            } else {
                r.resources.wood.amount = Math.floor(r.resources.wood.amount * 0.7);
                r.resources.food.amount = Math.floor(r.resources.food.amount * 0.8);
    ...
    for (let rId of rKeys) {
        ...
        // ИСПРАВЛЕНО: Обращение к .level
        let totalJobs = (r.facilities.farms.level + r.facilities.lumbermills.level + r.facilities.mines.level + r.facilities.forges.level + r.facilities.weavers.level + r.facilities.alchemists.level) * 200;
        ...
        let avgWage = Math.max(1, Math.floor((r.moneySupply / totalWorkforce) * 0.01));
        if (unemployed > totalWorkforce * 0.2) avgWage = Math.max(1, avgWage - 1);
        ...
        let workersPerSector = activeWorkers / 6;

        // ИСПРАВЛЕНО: Обращение к .amount и .level
        r.resources.food.amount += Math.floor(workersPerSector * (r.facilities.farms.level / 10) * 2 * weatherMod);
        r.resources.wood.amount += Math.floor(workersPerSector * (r.facilities.lumbermills.level / 10) * weatherMod);
        r.resources.ore.amount += Math.floor(workersPerSector * (r.facilities.mines.level / 10));
        r.resources.herbs.amount += Math.floor(workersPerSector * (r.facilities.farms.level / 20) * weatherMod);
        ...
        // Ткань
        let clothCapacity = Math.floor(workersPerSector * (r.facilities.weavers.level / 10));
        let fibersToCloth = Math.min(r.resources.food.amount, clothCapacity);
        if (fibersToCloth > 0) {
            r.resources.food.amount -= Math.floor(fibersToCloth * 0.2);
            r.resources.cloth.amount += Math.floor(fibersToCloth * 0.5);
        }

        // Оружие
        let weaponCapacity = Math.floor(workersPerSector * (r.facilities.forges.level / 20));
        let steelToWeapons = Math.min(r.resources.steel.amount, weaponCapacity);
        if (steelToWeapons > 0) {
            r.resources.steel.amount -= steelToWeapons;
            r.resources.wood.amount -= Math.floor(steelToWeapons * 0.2);
            r.resources.weapons.amount += Math.floor(steelToWeapons * 0.3);
        }

        // Зелья
        let potionCapacity = Math.floor(workersPerSector * (r.facilities.alchemists.level / 10));
        let herbsToPotions = Math.min(r.resources.herbs.amount, potionCapacity);
        if (herbsToPotions > 0) {
            r.resources.herbs.amount -= herbsToPotions;
            r.resources.potions.amount += Math.floor(herbsToPotions * 0.5);
        }

        // Роскошь
        let luxuryCapacity = Math.floor(workersPerSector * (r.facilities.weavers.level / 20));
        let matsToLuxury = Math.min(r.resources.cloth.amount, Math.min(r.resources.steel.amount, luxuryCapacity));
        if (matsToLuxury > 0) {
            r.resources.cloth.amount -= matsToLuxury;
            r.resources.steel.amount -= Math.floor(matsToLuxury * 0.5);
            r.resources.luxury.amount += Math.floor(matsToLuxury * 0.2);
        }
        ...
        r.resources.food.amount -= foodConsumed;
        r.resources.wood.amount -= woodConsumed;
        r.resources.cloth.amount -= clothConsumed;

        if (r.resources.food.amount < 0) {
            r.resources.food.amount = 0;
        ...
        if (r.resources.wood.amount < 0) r.resources.wood.amount = 0;
        if (r.resources.cloth.amount < 0) r.resources.cloth.amount = 0;
        if (r.resources.weapons.amount < 0) r.resources.weapons.amount = 0;
        ...
        if (employmentRate < 0.5 && Math.random() < 0.05) {
            ...
            r.resources.weapons.amount = Math.max(0, r.resources.weapons.amount - 50);
        }

        // --- ДИНАМИЧЕСКОЕ ЦЕНООБРАЗОВАНИЕ ---
        for (let good in BASE_ECONOMY_PRICES) {
            let supply = r.resources[good].amount || 1; // ИСПРАВЛЕНО
            let demand = 0;
            ...
            else demand = r.population * 0.001;
    ...
    for (let id in World.npcs) {
        ...
        if (locId && World.regions[locId]) {
            ...
            if (!npc.economy.isEmployed) {
                if (Math.random() < (r.facilities.farms.level + r.facilities.mines.level) / 100) {
    ...
    for (let rId of rKeys) {
        ...
        for (let good in BASE_ECONOMY_PRICES) {
            ...
            for (let destId of rKeys) {
                ...
                if (profitMargin > localPrice * 0.3 && r.resources[good].amount > 50) { // ИСПРАВЛЕНО
            ...
            if (bestDest) {
                ...
                let amount = Math.floor(r.resources[good].amount * 0.2); // ИСПРАВЛЕНО
                let cost = amount * localPrice;

                if (amount > 0) {
                    r.resources[good].amount -= amount; // ИСПРАВЛЕНО
                    r.moneySupply += cost;
                    ...
                    r.caravans.push({
                        ...
                        buyPrice: localPrice,
                        investment: cost,
                        hoursLeft: 24 + Math.floor(Math.random() * 48)
                    });
    ...
    for (let fId of fKeys) {
        ...
        // ИСПРАВЛЕНО: Обращение к .amount
        f.resources.gold.amount += 500 + Math.floor(f.stability * 10); 
        f.resources.manpower.amount += 50;
        
        if (f.resources.food.amount < 1000) f.stability -= 2;
        else f.stability += 1;
        ...
        if (f.stability < 30 && Math.random() < 0.1) {
            ...
            f.stability += 20;
            f.resources.manpower.amount -= 300;
        }
        ...
        for (let targetF of fKeys) {
            ...
            if (f.resources.food.amount < 2000) change -= 5;
            if (f.resources.gold.amount < 5000) change -= 3;
        ...
        if (atWarWith && f.resources.gold.amount > 1000 && f.resources.manpower.amount > 300) {
        ...
        for (let i = f.armies.length - 1; i >= 0; i--) {
            ...
            if (enemyFactionId) {
                ...
                if (army.siegeDays === -1) {
                    ...
                    army.siegeDays--;
                    World.regions[targetLoc].population -= Math.floor(Math.random() * 100);
                } else if (army.siegeDays === 0) {
                    let defender = World.factions[regionOwner];
                    // ИСПРАВЛЕНО: Обращение к .level
                    let garrisonPower = (World.regions[targetLoc].population / 100) + (World.regions[targetLoc].facilities.farms.level * 10);
    ...
    for (let rId in World.animals) {
        ...
        if (a.herbivores > 500 && r) {
            ...
            a.herbivores -= hunted;
            r.resources.food.amount += hunted * 2; // ИСПРАВЛЕНО
        }
    }
...
async function runWorldSimulationTick() {
    ...
    try {
        ...
        for (let rId in World.regions) {
            ...
            // ИСПРАВЛЕНО: Обращение к .amount
            worldSummary += `Регион: ${r.name} (Владелец: ${ownerName}). Население: ${r.population}. Погода: ${World.weather[rId]}. Еда: ${r.resources.food.amount}. Дерево: ${r.resources.wood.amount}. Оружие: ${r.resources.weapons.amount}.\n`;
        }
        ...
        for (let fId in World.factions) {
            let f = World.factions[fId];
            // ИСПРАВЛЕНО: Обращение к .amount
            worldSummary += `Фракция: ${f.name}. Стабильность: ${f.stability}/100. Золото: ${f.resources.gold.amount}. Армий в походе СЕЙЧАС: ${f.armies.length}.\n`;
            for (let target in f.diplomacy) {
    ...
    if (DEBUG_MODE && typeof World !== 'undefined' && World) {
        ...
        for (let fId in World.factions) {
            ...
            html += `<div class="debug-item">
                     ...
                     💰${Math.floor(f.resources.gold.amount)} | 🛡️${Math.floor(f.resources.manpower.amount)}
        ...
        for (let rId in World.regions) {
            ...
            html += `<div class="debug-item">
                     ...
                     Склад: 🍖${Math.floor(r.resources.food.amount)} 🪨${Math.floor(r.resources.ore.amount)} ⛓️${Math.floor(r.resources.steel.amount)} 🧪${Math.floor(r.resources.potions.amount)}
                     </div>`;
        }
        html += '</div></div>';
