            ...
            if (caravan.hoursLeft <= 0) {
                ...
                if (destRegion) {
                    for (let good in caravan.goods) {
                        destRegion.resources[good] = (destRegion.resources[good] || 0) + caravan.goods[good];
                    }
                    generateWorldNews(`Караван из ${region.name} прибыл в ${destRegion.name} с товарами.`, caravan.destination, 1);
                }
...
function simulateOneDay() {
    ...
    const basePrices = { food: 1, wood: 2, ore: 4, herbs: 6, steel: 12, cloth: 10, weapons: 35, potions: 30, luxury: 80 };
    ...
    for (let rId of rKeys) {
        ...
        for (let good in basePrices) {
            ...
            r.markets[good] = Math.max(1, Math.floor(basePrices[good] * priceMod));
        }
    }
    ...
    for (let rId of rKeys) {
        let r = World.regions[rId];
        for (let good in basePrices) {
            if (r.markets[good] > basePrices[good] * 2) {
                let bestSupplier = null;
                let bestPrice = r.markets[good];
                for (let supId of rKeys) {
                    if (supId === rId) continue;
                    let sup = World.regions[supId];
                    if (sup.markets[good] < bestPrice && sup.resources[good] > 100) {
                        bestSupplier = supId;
                        bestPrice = sup.markets[good];
                    }
                }
                if (bestSupplier) {
                    let amount = Math.floor(World.regions[bestSupplier].resources[good] * 0.2);
                    if (amount > 0) {
                        World.regions[bestSupplier].resources[good] -= amount;
                        World.regions[bestSupplier].caravans.push({
                            id: "caravan_" + Date.now() + Math.floor(Math.random()*1000),
                            origin: bestSupplier,
                            destination: rId,
                            goods: { [good]: amount },
                            hoursLeft: 48 + Math.floor(Math.random() * 24)
                        });
                        if (Math.random() < 0.1) generateWorldNews(`Торговый караван вышел из ${World.regions[bestSupplier].name} в ${r.name}.`, rId, 2, 'trade');
                    }
                }
            }
        }
