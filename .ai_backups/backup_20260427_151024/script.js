...
function addBatch(resources, key, amount, currentDay) {
    if (!resources[key]) resources[key] = { amount: 0, quality: 1.0, batches: [] };
    if (!resources[key].batches) resources[key].batches = [{amount: resources[key].amount, day: currentDay}];
    if (amount > 0) {
        resources[key].batches.push({ amount: amount, day: currentDay });
...
function consumeBatch(resources, key, amount) {
    ...
    if (!resources[key].batches) resources[key].batches = [{amount: resources[key].amount, day: 0}];
...
function extractBatches(resources, key, amount) {
    ...
    for (let i = 0; i < resources[key].batches.length && remaining > 0; i++) {
        ...
        b.amount -= take;
        remaining -= take;
        if (take > 0) extracted.push({ amount: take, day: b.day });
    }
    resources[key].batches = resources[key].batches.filter(b => b.amount > 0);
    resources[key].amount = resources[key].batches.reduce((sum, b) => sum + b.amount, 0);
    return extracted;
            ...
            if (caravan.hoursLeft <= 0) {
                if (destRegion) {
                    ...
                    for (let good in caravan.goods) {
                        ...
                        let batches = caravan.goodsBatches ? caravan.goodsBatches[good] : [{amount: amount, day: player?.gameTime?.year * 360 || 0}];
...
function simulateOneDay() {
    ...
    for (let rId of rKeys) {
        ...
        if(r.facilities.farms) {
            addBatch(r.resources, 'wheat', Math.floor(workersPerSector * (r.facilities.farms.level / 5) * 5 * weatherMod), currentDay);
            addBatch(r.resources, 'cotton', Math.floor(workersPerSector * (r.facilities.farms.level / 15) * weatherMod), currentDay);
            addBatch(r.resources, 'herbs', Math.floor(workersPerSector * (r.facilities.farms.level / 20) * weatherMod), currentDay);
        }
        if(r.facilities.lumbermills) addBatch(r.resources, 'wood', Math.floor(workersPerSector * (r.facilities.lumbermills.level / 10) * weatherMod), currentDay);
        if(r.facilities.mines) {
            addBatch(r.resources, 'iron_ore', Math.floor(workersPerSector * (r.facilities.mines.level / 10)), currentDay);
            addBatch(r.resources, 'gold_ore', Math.floor(workersPerSector * (r.facilities.mines.level / 30)), currentDay);
        }
        ...
        for (let recipe of CRAFTING_RECIPES) {
            ...
            if (maxCrafts > 0) {
                ...
                for (let outRes in recipe.outputs) {
                    ...
                    addBatch(r.resources, outRes, produced, currentDay);
                }
            }
        }
    ...
    for (let rId of rKeys) {
        ...
        for (let good in ECONOMY_ITEMS) {
            ...
            if (bestDest) {
                ...
                if (amount > 0) {
                    let extractedBatches = extractBatches(r.resources, good, amount);
                    r.moneySupply += cost;
                    r.caravans.push({
