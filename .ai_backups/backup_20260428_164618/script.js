...
const CoreInventorySystem = {
    ...
    createItem: function(prototypeId, quantity, containerId, customProps = {}) {
        ...
        const item = {
            ...
            slot_index: null,
            state: "idle", // idle, equipped, in_trade, destroyed
            ...
            created_at: player ? player.stats.turnCount : 0,
            last_moved_at: player ? player.stats.turnCount : 0
        };
        ItemRegistry.set(id, item);

        if (containerId && ContainerRegistry.has(containerId)) {
            ContainerRegistry.get(containerId).items.push(id);
        }
        return id;
    },
        ...
        if (targetContainerId && ContainerRegistry.has(targetContainerId)) {
            const tgt = ContainerRegistry.get(targetContainerId);
            if (tgt.items.length >= tgt.max_slots) return { success: false, error: "Target container is full" };
        }
        ...
        if (targetContainerId && ContainerRegistry.has(targetContainerId)) {
            ContainerRegistry.get(targetContainerId).items.push(item.id);
...
const TradeSystem = {
    ...
    initiateTrade: function(initiatorId, targetId, initiatorContainerId, targetContainerId) {
        ...
        this.activeTrades.set(tradeId, {
            ...
            offer_items: [], // Массив UUID предметов
            request_items: [], // Массив UUID предметов
            status: "pending"
        });
        return tradeId;
    },
