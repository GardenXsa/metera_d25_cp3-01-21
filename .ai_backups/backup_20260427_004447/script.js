...
async function runWorldSimulationTick() {
    ...
    try {
        // Собираем СЫРЫЕ ДАННЫЕ из JS-симуляции для нейросети
        ...
        worldSummary += `\nПоследние системные события: ${recentNews || "Нет свежих данных"}\n`;

        const fantasyMonths = ["Утренней Звезды", "Ледолома", "Ветровея", "Цветеня", "Солнцеворота", "Знойника", "Жатвеня", "Листопада", "Хладника", "Мертвой Луны", "Темного Рубежа", "Конца Года"];
        let currentDateStr = `${player.gameTime.day} ${fantasyMonths[player.gameTime.month - 1]}, ${player.gameTime.year} года`;
        
        // ЖЕСТКИЙ СЧЕТЧИК ДНЕЙ
        let daysPassed = World.time.daysSinceLastEvent || 1;
        World.time.daysSinceLastEvent = 0; // Сбрасываем счетчик

        const prompt = `### ДИРЕКТИВА: ДВИЖОК МИРА (WORLD SIMULATOR) v4.0
Ты — аналитический модуль и летописец. Твоя задача: взять СЫРЫЕ ДАННЫЕ симуляции, проанализировать изменения за ПРОШЕДШЕЕ ВРЕМЯ и выдать точную, историческую сводку.
...
ПРИКАЗЫ (ЛОГИКА И МАТЕМАТИКА):
1. ТЫ ОБЯЗАН учитывать цифру прошедшего времени (${daysPassed} дней). Если прошло 14 дней, события должны развиваться в масштабе двух недель (долгие осады, марши армий, истощение ресурсов). Если прошел 1 день — события локальны и стремительны.
2. Опиши конкретные факты из сводки: кто кого осаждает, где упала стабильность, где дефицит. Не лей воду, оперируй фактами.
3. Начни текст с четкого обозначения прошедшего времени и текущей даты. Игрок должен понимать хронологию. (Например: "За минувшие ${daysPassed} дней...", "К ${currentDateStr} ситуация в мире...").
4. Твой ответ ДОЛЖЕН БЫТЬ СТРОГО ВАЛИДНЫМ JSON ОБЪЕКТОМ. Массив actions оставляй ПУСТЫМ [], так как всю математику мира уже сделал движок. Исключение: можешь использовать nexusDefine для создания глобальных эпохальных флагов.
Формат:
{
  "narrative": "Твоя точная и атмосферная хроника событий...",
  "actions": []
}`;
