...
function initializeGameInterface() {
    ...
    if (localMapPanel) localMapPanel.style.display = 'flex'; // Всегда показываем панель
...
async function generateLocalMapFromAI(description, size) {
    ...
    try {
        ...
        const rawResponse = await performAiFetch(finalPrompt, [], modelId, "Сгенерируй JSON план локации.");
        const result = parseAIResponse(rawResponse);
        
        // Ищем команду renderLocation в ответе специализированной нейросети
        const mapAction = result.actions?.find(a => a.command === 'renderLocation');
        if (mapAction && mapAction.args && mapAction.args.plots) {
            return mapAction.args.plots;
        }
        throw new Error("Нейросеть не вернула массив plots.");
    } catch (e) {
    ...
    try {
        ...
        if (isInitialPrompt) {
            ...
            if (mapAction && enableLocalMap) {
                ...
                const generatedPlots = await generateLocalMapFromAI(mapAction.args.description || "Неизвестная локация", mapAction.args.size || "6x6");
...
function openFullscreenMap(args) {
    ...
    inspector.innerHTML = '<p style="color: #7f8c8d; font-style: italic; text-align: center; margin-top: 50px;">Кликните по клетке на плане, чтобы изучить её содержимое.</p>';

    const [width, height] = args.size.split('x').map(Number);
    // Делаем тайлы крупными (48px)
    ...
    for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
            ...
            cell.onclick = () => selectLocalPlot(cell, tileData);
...
function selectLocalPlot(element, data) {
    if (selectedLocalTile) selectedLocalTile.classList.remove('selected');
    element.classList.add('selected');
    selectedLocalTile = element;

    const inspectorName = document.getElementById('large-map-info-name');
    const inspectorDetails = document.getElementById('large-map-info-details');
    if (!inspectorName || !inspectorDetails) return;

    inspectorName.textContent = `[ ${data.name || "Неизвестно"} ]`;

    let detailsHtml = `
        <div class="info-row"><span class="info-label">Объект:</span> ${tileTypeDictionary[data.type] || data.type}</div>
        <div class="info-row"><span class="info-label">Позиция:</span> X:${data.x} | Y:${data.y}</div>
    `;

    if (data.desc) detailsHtml += `<div class="info-row" style="margin-top: 10px; font-style: italic; color: #bdc3c7;">"${data.desc}"</div>`;
    if (data.loot) detailsHtml += `<div class="info-row" style="margin-top: 10px; color: #2ecc71;"><span class="info-label">Замечено:</span> ${data.loot}</div>`;
    if (data.owner) detailsHtml += `<div class="info-row" style="margin-top: 10px; color: #9b59b6;"><span class="info-label">Владелец:</span> ${data.owner}</div>`;

    inspectorDetails.innerHTML = detailsHtml;
}
...
function selectLocalPlot(element, data) {
    ...
    if (data.desc) detailsHtml += `<div class="info-row" style="margin-top: 6px; font-style: italic; color: #bdc3c7;">"${data.desc}"</div>`;
    if (data.loot) detailsHtml += `<div class="info-row" style="margin-top: 6px; color: #2ecc71;"><span class="info-label">Замечено:</span> ${data.loot}</div>`;
    if (data.owner) detailsHtml += `<div class="info-row" style="margin-top: 6px; color: #9b59b6;"><span class="info-label">Владелец:</span> ${data.owner}</div>`;
