...
function handleUserInput() {
    ...
    // --- АНТИЧИТ: Удаляем вручную вписанные броски из текста игрока ---
    // Игрок больше не сможет скопировать [ROLL_RESULT: 20...] в чат
    ...
    if (turn > 0 && turn % MEMORY_SUMMARY_TURN === 0) {
        ...
        turnRollMemory = {}; // Сброс памяти бросков
        return;
    }
    ...
    userInput.value = '';
    if (rollsContainer) rollsContainer.innerHTML = '';
    turnRollMemory = {}; // Сброс памяти бросков после отправки хода
}
