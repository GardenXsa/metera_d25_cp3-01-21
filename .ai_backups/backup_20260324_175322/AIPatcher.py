...
class AIPatcherPro(ctk.CTk):
    ...
    def analyze_json(self):
        ...
        raw_input = self.txt_json.get("1.0", "end").strip()
        if not raw_input:
            self.lbl_status.configure(text="Ошибка: Пустой ввод", text_color="#e74c3c")
            return

        # --- УМНЫЙ ЗАХВАТ JSON ---
        # Игнорируем markdown-разметку и лишний текст, ищем границы объекта {}
        start_idx = raw_input.find('{')
        end_idx = raw_input.rfind('}')

        if start_idx == -1 or end_idx == -1:
            self.log_diff("ОШИБКА: JSON-объект не найден (проверьте наличие { и })", "error")
            self.lbl_status.configure(text="JSON не найден", text_color="#e74c3c")
            return
        ...
        try:
            ...
            # Попытка исправить типичные ошибки (лишние запятые)
            try:
                import re
                # Удаляем запятые перед закрывающими скобками
                fixed_json = re.sub(r',\s*([\}\]])', r'\1', json_str)
                patch_data = json.loads(fixed_json)
            except:
                self.log_diff(f"ОШИБКА ПАРСИНГА JSON:\n{e}\n\nПроверьте, что в коде экранированы кавычки и слэши.", "error")
        ...
        for op in operations:
            ...
            if not os.path.exists(abs_path):
                self.log_diff(f"ОШИБКА: Файл не найден: {path}", "error")
                has_errors = True
                continue
            ...
            if action == "replace":
                if search not in original_content:
                    self.log_diff(f"ОШИБКА: Текст для замены не найден!", "error")
                    has_errors = True
                    continue
                ...
                if search not in original_content:
                    self.log_diff(f"ОШИБКА: Якорь не найден!", "error")
                    has_errors = True
                    continue
                ...
                start_idx_block, end_idx_block = self.extract_brace_block(original_content, start_marker)
                
                if start_idx_block == -1:
                    self.log_diff(f"ОШИБКА: Маркер блока не найден: {start_marker}", "error")
                    has_errors = True
                    continue
                
                if action == "delete_js_block":
                    new_content = original_content[:start_idx_block] + original_content[end_idx_block:]
                else:
                    new_content = original_content[:start_idx_block] + content_to_insert + "\n" + original_content[end_idx_block:]

            # Diff логика
            orig_lines = original_content.splitlines()
            new_lines = new_content.splitlines()
            diff = list(difflib.unified_diff(orig_lines, new_lines, lineterm='', n=1))
            
            if not diff:
                self.log_diff("Изменений нет.", "info")
            else:
                for line in diff[2:]:
                    if line.startswith('+'): self.log_diff(line, "add")
                    elif line.startswith('-'): self.log_diff(line, "del")
                    else: self.log_diff(line)
