        ...
        if (selectedEvent.type === 'combat') {
            ...
            if (selectedEvent.enemies && selectedEvent.enemies.length > 0) {
                selectedEvent.enemies.forEach((en, idx) => {
                    ...
                    // Находим внутренний ID только что созданного врага
                    const internalId = Object.keys(player.visibleEntities).find(key => player.visibleEntities[key].aiIdentifier === eId);
                    if (internalId) participants.push(internalId);
                });
            } else {
...
function updateCharacterSheet() {
    ...
    if (player && player.currentJourney) {
        ...
        if (player.currentCombat && player.currentCombat.isActive) {
            // Бой во время путешествия - возвращаем управление
...
function updateEnvironmentVisibility() {
    ...
    Object.values(player.allKnownEntities).forEach(ent => {
        ...
        if ((isHere || isFollowing) && ent.stats.hp > 0) {
            newVisible[nextId] = { ...ent, id: nextId };
            nextId++;
        }
    });
