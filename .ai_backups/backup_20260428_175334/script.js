...
function is_sellable(containerId, itemId) {
    ...
    if (item.flags.bound_to_owner && item.flags.bound_to_owner !== ContainerRegistry.get(containerId).owner_id) return false;
...
const CoreInventorySystem = {
    createContainer: function(type, ownerId, maxWeight, maxSlots, locationData = null) {
        ...
        const container = {
            ...
            location: locationData || { world_coords: [0,0,0], parent_entity: null, parent_container: null, region_id: null },
    ...
    createItem: function(prototypeId, quantity, containerId, customProps = {}) {
        ...
        const item = {
            ...
            custom_props: { weight_per_unit: baseWeight, ...customProps },
    ...
    findItemByPrototype: function(containerId, protoId) {
        const cont = ContainerRegistry.get(containerId);
        if (!cont) return null;
        return cont.items.find(id => ItemRegistry.get(id)?.prototype_id === protoId);
    },
    moveItem: function(itemId, sourceContainerId, targetContainerId, quantity = null) {
        ...
        if (item.state === "in_trade") return { success: false, error: "Item state locks movement" };
        if (item.flags.bound_to_owner && targetContainerId && ContainerRegistry.get(targetContainerId)?.owner_id !== item.flags.bound_to_owner) return { success: false, error: "Item is soulbound" };
        ...
        if (!actualTargetId) {
            let groundPile = Array.from(ContainerRegistry.values()).find(c => c.type === 'ground_pile' && c.location.region_id === player.location);
            if (!groundPile) actualTargetId = this.createContainer('ground_pile', null, 9999, 999, { world_coords: [0,0,0], parent_entity: null, parent_container: null, region_id: player.location });
        ...
        if (ContainerRegistry.has(actualTargetId)) {
            ...
            if (srcCont && srcCont.type === 'faction_vault' && actualTargetId === player.container_backpack) {
                ...
                const regionId = srcCont.location?.region_id;
    ...
    destroyContainer: function(containerId) {
        const cont = ContainerRegistry.get(containerId);
        if (!cont) return false;
        let groundPileId = Array.from(ContainerRegistry.values()).find(c => c.type === 'ground_pile' && c.location.region_id === cont.location.region_id)?.id;
...
const TradeSystem = {
    ...
    initiateTrade: function(initiatorId, targetId, initiatorContainerId, targetContainerId) {
        ...
        let tradeValueGold = 0;

        for (const offer of trade.offer_items) {
            if (ItemRegistry.has(offer.id)) {
                let it = ItemRegistry.get(offer.id);
                it.state = "idle";
                if (it.prototype_id === 'gold') tradeValueGold += offer.quantity;
            }
            const res = CoreInventorySystem.moveItem(offer.id, trade.initiator_container, trade.target_container, offer.quantity);
            if (!res.success) return this._rollbackTrade(tradeId, "Failed to move offer item");
        }
        for (const req of trade.request_items) {
            if (ItemRegistry.has(req.id)) {
                let it = ItemRegistry.get(req.id);
                it.state = "idle";
                if (it.prototype_id === 'gold') tradeValueGold += req.quantity;
            }
            const res = CoreInventorySystem.moveItem(req.id, trade.target_container, trade.initiator_container, req.quantity);
            if (!res.success) return this._rollbackTrade(tradeId, "Failed to move request item");
        }
        
        // Пост-обработка (Репутация и Экономика)
        if (trade.target !== 'player' && typeof World !== 'undefined' && World && World.npcs[trade.target]) {
            const npc = World.npcs[trade.target];
            const regionId = npc.homeLocation || (ContainerRegistry.get(trade.target_container)?.location?.region_id);
            ...
            if (regionId && World.regions[regionId]) {
                ...
                if (tradeValueGold > 0) {
                    ...
                    if (factionId && World.factions[factionId]) {
        ...
        if (player && player.container_backpack) {
            const bp = ContainerRegistry.get(player.container_backpack);
            if (bp) {
                player.stats.gold = bp.items.filter(id => ItemRegistry.get(id)?.prototype_id === 'gold').reduce((sum, id) => sum + ItemRegistry.get(id).stack_size, 0);
            }
        }
        return { success: true };
    },
    _rollbackTrade: function(tradeId, reason) {
        const trade = this.activeTrades.get(tradeId);
        [...trade.offer_items, ...trade.request_items].forEach(obj => {
            if (ItemRegistry.has(obj.id)) ItemRegistry.get(obj.id).state = "idle";
        });
        trade.status = "failed";
        this.activeTrades.delete(tradeId);
        const btn = document.getElementById('trade-confirm-btn');
    ...
    negotiateTrade: function(tradeId, newOfferItems, newRequestItems) {
        if (!this.activeTrades.has(tradeId)) return false;
        const trade = this.activeTrades.get(tradeId);
        [...trade.offer_items, ...trade.request_items].forEach(obj => {
            if (ItemRegistry.has(obj.id)) ItemRegistry.get(obj.id).state = "idle";
        ...
        trade.request_items = [];
        let success = true;
        newOfferItems.forEach(obj => { if(!this.addItemToTrade(tradeId, obj.id, "offer", obj.quantity)) success = false; });
        newRequestItems.forEach(obj => { if(!this.addItemToTrade(tradeId, obj.id, "request", obj.quantity)) success = false; });
        return success;
    }
};
...
function buildFullPlayerSnapshot() {
    ...
    const nearbyContainers = Array.from(ContainerRegistry.values()).filter(c => c.location?.region_id === player.location && c.owner_id !== 'player').map(c => ({
        id: c.id, type: c.type, locked: c.lock_data.is_locked, owner: c.owner_id, items: c.items.map(id => ItemRegistry.get(id)?.prototype_id)
    }));

    const nearbyNpcs = Object.values(player.visibleEntities).filter(e => e.type === 'npc').map(npc => {
        const wNpc = World?.npcs[npc.aiIdentifier];
        return {
            id: npc.aiIdentifier, name: npc.name, inventory_id: wNpc?.inventory_id, gold: wNpc?.inventory?.gold || 0
        };
    });
...
{
  "player": {
    "id": "player", "name": "${player.name}", "race": "${player.race}", "class": "${player.class}", "level": ${player.stats.level},
  ...
  "inventory": ${JSON.stringify(player.container_backpack ? ContainerRegistry.get(player.container_backpack).items.map(id => { let it = ItemRegistry.get(id); return it ? { instance_id: it.id, prototype_id: it.prototype_id, name: it.custom_props.name, quantity: it.stack_size, flags: it.flags } : null; }).filter(Boolean) : [])},
  "equipment": ${JSON.stringify(player.container_equipment ? ContainerRegistry.get(player.container_equipment).items.map(id => { let it = ItemRegistry.get(id); return it ? { slot: it.slot_index, instance_id: it.id, prototype_id: it.prototype_id, name: it.custom_props.name } : null; }).filter(Boolean) : [])},
...
case 'calculationLog':
            ...
            // --- ИНВЕНТАРЬ ---
                ...
                if (args.aiIdentifier && args.name) {
                    ...
                    const targetContId = args.containerId || player.container_backpack;
                    ...
                    if (aiId.toLowerCase() === 'gold' && targetContId === player.container_backpack) {
                        player.stats.gold = (player.stats.gold || 0) + quantity;
            ...
            case 'moveItem':
                if (args.itemId && args.sourceContainerId && args.targetContainerId) {
                    const res = CoreInventorySystem.moveItem(args.itemId, args.sourceContainerId, args.targetContainerId, args.quantity || null);
                    feedback = res.success ? `[СИСТЕМА] Предмет перемещен.` : `[ERROR] Ошибка перемещения: ${res.error}`;
                    updateInventoryDisplay();
                } else {
                    feedback = `[ERROR] 'moveItem' требует 'itemId', 'sourceContainerId', 'targetContainerId'.`;
                }
                break;
                        case 'lockContainer':
                if (args.containerId) {
...
case 'setEntityBinding':
            ...
            case 'createContainer':
                ...
                break;
                                                                                    case 'initiateTrade':
                if (args.initiatorId && args.targetId && args.initiatorContainerId && args.targetContainerId && args.offerItems && args.requestItems) {
                    const initCont = ContainerRegistry.get(args.initiatorContainerId);
                    const tgtCont = ContainerRegistry.get(args.targetContainerId);
                    if (!initCont || !tgtCont) {
                        feedback = `[ERROR] Один из контейнеров не найден.`;
                        break;
                    }
                    if (!checkDistance(initCont.location, tgtCont.location)) {
                        feedback = `[ERROR] Контейнеры слишком далеко друг от друга для торговли.`;
                        break;
                    }
                    
                    const tradeId = TradeSystem.initiateTrade(args.initiatorId, args.targetId, args.initiatorContainerId, args.targetContainerId);
                    let success = true;
                    
                    args.offerItems.forEach(offer => {
                        if (!is_sellable(args.initiatorContainerId, offer.id)) success = false;
                        if (!TradeSystem.addItemToTrade(tradeId, offer.id, "offer", offer.quantity)) success = false;
                    });
                    
                    args.requestItems.forEach(req => {
                        if (!is_sellable(args.targetContainerId, req.id)) success = false;
                        if (!TradeSystem.addItemToTrade(tradeId, req.id, "request", req.quantity)) success = false;
                    });
                    
                    if (success) {
                        if (args.initiatorId === 'player') player.active_trade_id = tradeId;
                        feedback = `[ТОРГОВЛЯ] Инициирована сделка между ${args.initiatorId} и ${args.targetId}. Предметы заблокированы. Ожидание подтверждения.`;
                    } else {
                        TradeSystem._rollbackTrade(tradeId, "Invalid items, insufficient quantities, or ownership mismatch");
                        feedback = `[ERROR] Не удалось инициировать сделку. Проверьте наличие предметов (instance_id), их количество и права владения (is_sellable).`;
                    }
                } else {
                    feedback = `[ERROR] 'initiateTrade' требует 'initiatorId', 'targetId', 'initiatorContainerId', 'targetContainerId', 'offerItems' [{id, quantity}] и 'requestItems' [{id, quantity}].`;
                }
                break; if (false) {
                    feedback = `[ERROR] 'initiateTrade' требует 'initiatorId', 'targetId', 'initiatorContainerId', 'targetContainerId', 'offerItems' [{id, quantity}] и 'requestItems' [{id, quantity}].`;
                }
                break; if (false) {
                    feedback = `[ERROR] 'initiateTrade' требует 'targetId', 'offerItems' [{id, quantity}] и 'requestItems' [{id, quantity}].`;
                }
                break; if (false) {
            ...
            case 'buildContainer':
                ...
                if (args.location && args.damage) {
                    let destroyed = 0;
                    ContainerRegistry.forEach(cont => {
                        if (cont.location?.region_id === args.location && cont.physical_props) {
                            cont.physical_props.health -= args.damage;
                            cont.items.forEach(itemId => {
                                let it = ItemRegistry.get(itemId);
                                if (it) it.durability -= Math.floor(args.damage / 2);
                            });
            ...
            case 'castMagicalPocket':
                ...
                if (args.containerId) {
                    const cont = ContainerRegistry.get(args.containerId);
                    if (cont) {
                        cont.location = { world_coords: [0,0,0], parent_entity: null, parent_container: null, region_id: "unknown_wilderness" };
