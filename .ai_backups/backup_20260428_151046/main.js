    ...
    try {
        ...
        for (const file of files) {
            try {
                ...
                // Читаем только первые 250 КБ файла (ОЧЕНЬ ВАЖНО для предотвращения вылета из-за нехватки памяти)
                const fd = fs.openSync(filePath, 'r');
                const buffer = Buffer.alloc(250 * 1024);
                const bytesRead = fs.readSync(fd, buffer, 0, 250 * 1024, 0);
                fs.closeSync(fd);
                
                const chunk = buffer.toString('utf-8', 0, bytesRead);
                
                // Извлекаем данные регулярками, не загружая весь гигантский JSON в память
