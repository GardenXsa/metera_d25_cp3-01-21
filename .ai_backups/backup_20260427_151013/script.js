...
// Голод и Бунты (Причина: нет еды или работы)
    ...
    for (let fId of fKeys) {
        ...
        let armyUpkeep = Math.floor(f.resources.manpower.amount * 0.5); // 1 золото на 2 солдат
        let stateUpkeep = f.regionCount * 2000; // Содержание каждого региона
        ...
        if (f.resources.gold.amount >= totalExpenses) {
            ...
            f.stability -= 2; // Недовольство элит
            f.resources.manpower.amount = Math.floor(f.resources.manpower.amount * 0.95); // Дезертирство из-за невыплаты жалованья
        }
        ...
        if (foodPerCapita < 0.5) f.stability -= 3;
        else if (foodPerCapita > 1.5 && f.resources.gold.amount > 0) f.stability += 1;
