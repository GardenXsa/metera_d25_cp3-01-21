...
const WorkerInventorySystem = {
    createContainer: function(type, ownerId, maxWeight, maxSlots, locationData = null) {
        ...
        const container = { id: id, type: type, max_weight_kg: maxWeight, max_slots: maxSlots, owner_id: ownerId, location: locationData, items: [] };
    ...
    createItem: function(prototypeId, quantity, containerId, customProps = {}) {
        ...
        const item = { id: id, prototype_id: prototypeId, stack_size: quantity, container_id: containerId, state: "idle", flags: {}, custom_props: customProps };
...
function syncAbstractToPhysical() {
    ...
    for (let rId in World.regions) {
        ...
        for (let res in r.resources) {
            let absAmt = Math.floor(r.resources[res].amount);
            let existingItemId = vault.items.find(id => ItemRegistry.get(id)?.prototype_id === res);
            if (absAmt > 0) {
                if (existingItemId) ItemRegistry.get(existingItemId).stack_size = absAmt;
                else WorkerInventorySystem.createItem(res, absAmt, r.vault_id, { name: getItemName(res, player ? player.era : 'rebirth') });
            } else if (existingItemId) {
                ItemRegistry.delete(existingItemId);
                vault.items = vault.items.filter(id => id !== existingItemId);
            }
        }
    }
}
