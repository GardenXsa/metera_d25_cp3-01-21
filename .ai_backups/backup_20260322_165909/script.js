...
async function loadActiveEraLore(eraId) {
    if (!eraId) return;
    var filePath = "assets/promts/epo_DC/" + eraId + ".txt";
    try {
        var response = await fetch(filePath + "?t=" + Date.now());
        if (response.ok) {
            activeEraSpecialLore = await response.text();
            console.log("[System] База данных эпохи " + eraId + " интегрирована.");
        }
    } catch (e) {
        console.error("[Error] Не удалось загрузить лор эпохи:", e);
        activeEraSpecialLore = "";
    }
}
let activeEraSpecialLore = ""; // Глобальная переменная для данных из epo_DC
...
async function loadActiveEraLore(eraId) {
    if (!eraId) return;
    var filePath = "assets/promts/epo_DC/" + eraId + ".txt";
    console.log("[System] Загрузка лора эпохи: " + filePath);
    try {
        var response = await fetch(filePath + "?t=" + Date.now());
        if (!response.ok) throw new Error("File not found");
        activeEraSpecialLore = await response.text();
        console.log("[System] Лор эпохи " + eraId + " интегрирован.");
    } catch (e) {
        console.warn("[System] Доп. лор эпохи не найден.");
        activeEraSpecialLore = "// Данные по эпохе отсутствуют.";
    }
}
async function loadActiveEraLore(eraId) {
    if (!eraId) return;
...
### ИНСТРУКЦИЯ:
${isInitial ? "ЭТО ПЕРВЫЙ ХОД. Инициализируй мир согласно реестру эпохи." : "Выполни расчеты и верни JSON."}`;
    } catch (error) {
