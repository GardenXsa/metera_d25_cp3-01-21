...
function updateWorldSimulation(pulses) {
    ...
    while (World.time.accumulatedMinutes >= 60) {
        ...
        // Если наступила полночь по игровому времени
        if (player && player.gameTime && player.gameTime.hour === 0) {
            simulateOneDay();
        }
    }
    syncWorldWithPlayer();
...
Игровое время: День ${player.gameTime.day}, ${player.gameTime.hour < 10 ? '0'+player.gameTime.hour : player.gameTime.hour}:${player.gameTime.minute < 10 ? '0'+player.gameTime.minute : player.gameTime.minute} (${player.timeOfDay})`, "system-message");
    ...
    if (enableWorldSim) await runWorldSimulationTick();
...
async function runWorldSimulationTick() {
    ...
    // Триггер события каждые 24 игровых часа (288 пульсов по 5 минут)
    if (isSimulatingWorld || (player.gameTime.totalPulses - World.time.lastEventPulse < 288)) return;
