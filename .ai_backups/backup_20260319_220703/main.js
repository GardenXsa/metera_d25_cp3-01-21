...
ipcMain.handle('get-save-path', async () => { return SAVES_DIR; });
    console.log('[DEBUG MAIN] Читаю папку сейвов:', SAVES_DIR);
    try {
        const files = fs.readdirSync(SAVES_DIR).filter(f => f.endsWith('.json'));
    console.log('[DEBUG MAIN] Найдено файлов:', files.length);
        const result = [];
        for (const file of files) {
            try {
                const content = fs.readFileSync(path.join(SAVES_DIR, file), 'utf-8');
                const data = JSON.parse(content);
                result.push({ filename: file, timestamp: data.timestamp, playerData: data.playerData, slotType: data.slotType, slotId: data.slotId });
            } catch (err) {
                console.log(`[Main Error] Ошибка чтения файла ${file}:`, err);
            }
        }
        return result;
    } catch (e) { console.error("[Main Error] Ошибка чтения папки сейвов:", e); return []; }
});
