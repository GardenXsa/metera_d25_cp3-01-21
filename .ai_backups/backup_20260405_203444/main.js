...
ipcMain.handle('speak-text', async (event, text, voiceModel) => {
    ...
    return new Promise((resolve) => {
        // Определяем правильный путь к папке TTS в зависимости от режима (Dev / Prod)
        let ttsDir;
        if (app.isPackaged) {
            // После билда папка tts будет лежать физически в папке resources/tts
            ttsDir = path.join(process.resourcesPath, 'tts');
        } else {
            // В режиме разработки папка лежит в корне проекта
        ...
        // ЗАПУСК: cwd: ttsDir — это база. Без этого он не подцепит DLL
