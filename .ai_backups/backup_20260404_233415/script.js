...
async function generateVisionImage(prompt, btnElement) {
    if (!imgApiKey) {
    ...
    else targetUrl = 'https://api.openai.com/v1/images/generations'; // Default OpenAI format
    ...
    try {
        ...
        if (data.data && data.data[0] && data.data[0].url) {
            showCinematicVision(data.data[0].url);
        } else {
            throw new Error('Неверный формат ответа от нейросети.');
        }
    } catch (e) {
        console.error('Image Gen Error:', e);
        showCustomAlert('Ошибка генерации изображения: ' + e.message);
    } finally {
...
function showCinematicVision(imageUrl) {
    let modal = document.getElementById('vision-modal');
    if (!modal) {
        modal = document.createElement('div');
        modal.id = 'vision-modal';
        modal.className = 'vision-overlay';
        modal.innerHTML = `
            <div class="vision-content">
                <div class="vision-frame">
                    <img id="vision-image" src="" alt="Видение">
                </div>
                <button id="vision-close" class="back-button">Вернуться в реальность</button>
            </div>
        `;
        document.body.appendChild(modal);
        document.getElementById('vision-close').onclick = () => {
            modal.classList.remove('visible');
            setTimeout(() => modal.style.display = 'none', 500);
        };
    }
    const imgEl = document.getElementById('vision-image');
    imgEl.src = imageUrl;
    modal.style.display = 'flex';
    // Force reflow
...
// --- Лог и Ввод ---
function addLogMessage(message, type = "gm-message", isRestoring = false, imagePrompt = "") {
    if (!gameLog) return;
    ...
    if (textToSpeak && textToSpeak.trim() !== '' && category !== 'system') {
        if (!bubbleElement.querySelector('.tts-controls-wrapper')) {
            ...
            if (imagePrompt && imagePrompt.trim() !== '') {
                ...
                imgBtn.onclick = (e) => {
                    e.stopPropagation();
                    generateVisionImage(imagePrompt, imgBtn);
                };
