...
function initializeGameInterface() {
    ...
    // --- [ПАТЧ] ДИНАМИЧЕСКАЯ ПЕРЕСБОРКА ИНТЕРФЕЙСА --- 
    if (!window.uiRestructured) {
        ...
        // 2. Умения: встраиваем в Персонажа
        const charContent = document.querySelector('.character-sheet .panel-content');
        const skillsPanel = document.querySelector('.skills-panel');
        if (charContent && skillsPanel) {
            skillsPanel.classList.add('merged-panel');
            charContent.appendChild(skillsPanel);
        }

        // 3. Карта: делаем большим оверлеем
        ...
        if (mapPanel && gameInterfaceUI) {
            ...
            mapBtn.onclick = () => {
                ...
                setTimeout(() => renderVisualMap(), 300);
            };
            headerControls.insertBefore(mapBtn, document.getElementById('in-game-menu-button'));
            
            // Кнопка закрытия карты
            const mapClose = document.createElement('button');
            mapClose.className = 'map-close-btn';
            mapClose.innerHTML = '<i class="fas fa-times"></i> Закрыть карту';
            mapClose.onclick = () => mapPanel.classList.remove('visible');
            mapPanel.appendChild(mapClose);
        }

        // 4. Расчеты ГМ: Выдвижной терминал
        ...
        if (calcPanel) {
            ...
            const headerControls = document.querySelector('.header-controls');
            headerControls.insertBefore(calcBtn, document.getElementById('in-game-menu-button'));
        }
        
        // 5. Экипировка и Инвентарь: бесшовное слияние
        const equipPanel = document.querySelector('.equipment-panel-v2');
        const invPanel = document.querySelector('.inventory');
        if (equipPanel && invPanel) {
            equipPanel.classList.add('merged-top');
            invPanel.classList.add('merged-bottom');
        }
