...
case 'setEntityBinding':
            ...
            case 'gmDirectResourceInjection':
                if (typeof World !== 'undefined' && World) {
                    if (World.gmInterventionPoints === undefined) World.gmInterventionPoints = 5;
                    if (World.gmInterventionPoints <= 0) { feedback = "[ERROR] Очки вмешательства ГМ исчерпаны."; break; }
                    let regInj = World.regions[args.regionId];
                    if (!regInj) { feedback = "[ERROR] Регион не найден."; break; }
                    World.gmInterventionPoints -= 1;
                    ...
                    feedback = `[Божественное вмешательство] В ${regInj.name} чудесным образом появилось ${qtyInj} ${args.goodType}. Осталось очков ГМ: ${World.gmInterventionPoints}.`;
                    addCalculationMessage(feedback);
                }
                break;
