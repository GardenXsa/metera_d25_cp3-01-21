...
ipcMain.handle('list-saves', async () => {
    try {
        ...
        for (const file of files) {
            try {
                // Асинхронное чтение файла, чтобы не вешать главный поток (убирает фриз на 5 секунд)
                const content = await fs.promises.readFile(path.join(SAVES_DIR, file), 'utf-8');
                const data = JSON.parse(content);
                results.push({ filename: file, timestamp: data.timestamp, playerData: data.playerData });
            } catch (err) {
                console.error(`[Save] Пропущен поврежденный файл ${file}:`, err.message);
            }
        }
        return results;
    } catch (e) { 
        console.error("[Save] Ошибка чтения папки сохранений:", e);
        return []; 
    }
});
