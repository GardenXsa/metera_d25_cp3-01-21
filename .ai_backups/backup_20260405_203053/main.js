...
ipcMain.handle('speak-text', async (event, text, voiceModel) => {
    ...
    return new Promise((resolve) => {
        // Получаем путь к папке ресурсов. В билде это будет путь к app.asar.
        // Заменяем app.asar на app.asar.unpacked, чтобы попасть к распакованным бинарникам.
        const rootPath = app.getAppPath().replace('app.asar', 'app.asar.unpacked');
        const ttsDir = path.join(rootPath, 'assets', 'tts');
