...
const TradeSystem = {
    ...
    confirmTrade: function(tradeId) {
        ...
        if (trade.target !== 'player' && World && World.npcs[trade.target]) {
            const npc = World.npcs[trade.target];
            const factionId = npc.homeLocation ? World.regions[npc.homeLocation]?.owner : null;
            if (factionId) {
                player.stats.reputation[factionId] = (player.stats.reputation[factionId] || 0) + 1;
                if (World.factions[factionId]) World.factions[factionId].resources.gold.amount += 5;
            }
        }
        ...
        // Синхронизация золота для UI
        player.stats.gold = CoreInventorySystem.getContainerItems ? CoreInventorySystem.getContainerItems(player.container_backpack).filter(i=>i.prototype_id==='gold').reduce((sum,i)=>sum+i.stack_size,0) : player.stats.gold;
        return { success: true };
    },
            ...
            case 'createContainer':
                ...
                if (args.initiatorId && args.targetId && args.initiatorContainerId && args.targetContainerId && args.offerItems && args.requestItems) {
                    ...
                    if (success) {
                        ...
                        feedback = `[ERROR] Не удалось инициировать сделку. Проверьте наличие предметов и права владения.`;
                    }
                } else {
