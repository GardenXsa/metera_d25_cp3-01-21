...
function loadTTSVoices() {
    ...
    ttsVoices = [
        ...
        { name: "Дмитрий (Русский, Мужской)", file: "ru_RU-dmitry-medium.onnx", lang: "ru" },
...
function handleTTSVoiceChange(event) {
    ...
    if (voice) {
        ...
        if (isTTSEnabled) speakText(t('tts.voiceTest', 'Тест голоса'));
    }
}
...
async function speakText(text) {
    if (!isTTSEnabled || !text || text.trim() === '' || !selectedTTSVoice) return;
...
function initializeGameInterface() {
    ...
    // Синхронизируем кнопку музыки в интерфейсе с реальным состоянием плеера
    isMusicPlaying = !audioPlayer.paused;
    updateMusicToggleButton(isMusicPlaying);
