...
function displaySavedChatHistory() {
    if (!gameLog || !conversationHistory || conversationHistory.length === 0) {
        return;
    }
    console.log(`Отображение сохраненной истории чата: ${conversationHistory.length} сообщений.`);
    // initializeGameInterface уже очистил лог.

    conversationHistory.forEach(messageEntry => {
        if (messageEntry.role && messageEntry.parts && messageEntry.parts[0].text) {
            const type = messageEntry.role === 'user' ? 'user-message' : 'gm-message';
            let textToShow = messageEntry.parts[0].text;

            // --- НАЧАЛО ИСПРАВЛЕНИЙ ---
            if (type === 'gm-message') {
                try {
                    // Пытаемся распарсить JSON
                    const startIndex = textToShow.indexOf('{');
                    const endIndex = textToShow.lastIndexOf('}');
                    if (startIndex === -1 || endIndex === -1) throw new Error("Not a JSON object");
                    
                    const jsonString = textToShow.substring(startIndex, endIndex + 1);
                    const messageObject = JSON.parse(jsonString);
                    
                    // Если успешно, берем только нарратив
                    textToShow = messageObject.narrative || "";

                } catch (e) {
                    // Если не получилось (старый формат или ошибка), оставляем как есть, но чистим старые команды
                    textToShow = textToShow.replace(/\[COMMAND:.+?\]/g, '').trim();
                }

                // Проверяем, не является ли это системным маркером ошибки
                const isBlockedMarker = `[${t('error.gmBlockedMarker')}`;
                if (textToShow.startsWith(isBlockedMarker)) {
                    return; // Не отображаем маркеры ошибок из истории
                }
            }
            // --- КОНЕЦ ИСПРАВЛЕНИЙ ---

            if (textToShow) { // Добавляем сообщение, только если есть что показывать
...
async function saveGame(slotType, slotId) {
    ...
    const saveData = {
        ...
        historyData: structuredClone(conversationHistory.slice(-MAX_HISTORY * 2))
    };
...
async function deleteSave(slotType, slotId) {
    ...
    showCustomConfirm(
        ...
        async () => {
            ...
            // --- ПЛАН АБАНГ: СНАЧАЛА ПЫТАЕМСЯ УДАЛИТЬ ФАЙЛ ---
            if (window.electronAPI && window.electronAPI.isElectron) {
                 const fileName = getSaveFileName(slotType, slotId);
                 success = await deleteFileFromFSA(fileName);
            }

            // --- ВСЕГДА ПРОВЕРЯЕМ И ЧИСТИМ LOCAL STORAGE ---
            // Это нужно, чтобы удалить старые "призрачные" сохранения.
            ...
            if (Array.isArray(targetArrayLS)) {
                ...
                if (allSavesLS[slotType].length < initialLengthLS) {
                    ...
                    success = true; // Считаем успехом, даже если удалили только из LS
                }
            }

            if (success) {
                console.log(`Удаление слота ${slotType} #${slotId} завершено.`);
                await populateLoadGameScreen();
            } else {
                showCustomAlert("Не удалось удалить сохранение (файл не найден).");
            }
        }
    );
}
...
async function populateLoadGameScreen() {
    ...
    console.log("[PopulateSaves] Запрос списка из Electron...");

    // --- ПЛАН АБАНГ: ОБЪЕДИНЯЕМ ДАННЫЕ ИЗ ДВУХ ИСТОЧНИКОВ ---
    // Шаг 1: Загружаем все из файлов, если это Electron. Они имеют приоритет.
    if (window.electronAPI && window.electronAPI.isElectron) {
        try {
            ...
            console.error("Ошибка чтения списка сохранений из файловой системы:", e);
        }
    }

    // Шаг 2: Загружаем все из Local Storage. Добавляем, только если такого слота еще нет.
    try {
        ...
        for (const save of allLsSaves) {
            const key = `${save.slotType}_${save.slotId}`;
            if (!allSavesMap.has(key)) {
                allSavesMap.set(key, save);
            }
        }
    } catch (e) {
        console.error("Ошибка чтения сохранений из Local Storage:", e);
    }
    
    const finalSavesList = Array.from(allSavesMap.values());

    if (finalSavesList.length === 0) {
        manualSaveSlotsList.innerHTML = `<li>${t('loadGame.noManualSaves')} (${primarySourceMessage})</li>`;
        autoSaveSlotsList.innerHTML = `<li>${t('loadGame.noAutoSaves')} (${primarySourceMessage})</li>`;
        updateDynamicUIText();
        return;
    }
    ...
    const displayedManualIds = new Set(manualSaves.map(s => s.slotId));
    for (let i = 1; i <= MAX_MANUAL_SAVES; i++) {
        if (!displayedManualIds.has(i)) {
            const li = document.createElement('li');
            li.classList.add('empty-slot');
            li.innerHTML = `
               <div class="save-info">
                   <span class="slot-name">${t('loadGame.manualSlot', {id: i})} - ${t('loadGame.emptySlot')}</span>
                </div>
                <div class="save-actions"></div>`;
            manualSaveSlotsList.appendChild(li);
        }
    }
    Array.from(manualSaveSlotsList.children).sort((a, b) => {
        const getId = (liElem) => {
            const nameSpan = liElem.querySelector('.slot-name');
            if (!nameSpan) return 999;
            const match = nameSpan.textContent.match(/#(\d+)/);
            return match ? parseInt(match[1], 10) : 999;
        };
        return getId(a) - getId(b);
    }).forEach(li => manualSaveSlotsList.appendChild(li));
...
function renderVisualMap() {
    ...
    // Очистка и фон "старого пергамента"
    ctx.fillStyle = '#e8dbbf';
    ...
    // Рисуем границы карты в стиле чернил
    ...
    // --- ГЕНЕРАЦИЯ ТИРРЕЙНА (ПСЕВДОСЛУЧАЙНАЯ) ---
    allPoints.forEach(point => {
        const pos = transform(point.worldX / COORD_MULT, point.worldY / COORD_MULT);
        // Отрисовываем тиррейн даже чуть за краем, чтобы не обрезался резко
        if (pos.x < -150 || pos.x > width + 150 || pos.y < -150 || pos.y > height + 150) return;

        const type = getLocationType(point.name);
        
        // Детерминированный генератор псевдослучайных чисел на основе координат
        let seed = Math.abs(Math.floor(point.x * 1337 + point.y * 9973));
        const random = () => { seed = (seed * 9301 + 49297) % 233280; return seed / 233280; };

        ctx.strokeStyle = 'rgba(44, 30, 20, 0.4)'; // Полупрозрачные чернила для фона
        ctx.fillStyle = '#e8dbbf'; // Цвет пергамента для перекрытия (создает эффект наброска)

        // Расширенная генерация тиррейна (пятна биомов)
        if (type === 'mountain') {
            for (let i = 0; i < 25; i++) {
                let mx = pos.x + (random() - 0.5) * 220 * mapState.zoom;
                let my = pos.y + (random() - 0.5) * 220 * mapState.zoom;
                let size = (15 + random() * 25) * mapState.zoom;
                drawMountain(ctx, mx, my, size);
            }
        } else if (type === 'natural') {
            for (let i = 0; i < 45; i++) {
                let tx = pos.x + (random() - 0.5) * 250 * mapState.zoom;
                let ty = pos.y + (random() - 0.5) * 250 * mapState.zoom;
                let size = (8 + random() * 12) * mapState.zoom;
                drawTree(ctx, tx, ty, size);
            }
        } else if (type === 'city') {
             for (let i = 0; i < 10; i++) {
                let hx = pos.x + (random() - 0.5) * 180 * mapState.zoom;
                let hy = pos.y + (random() - 0.5) * 180 * mapState.zoom;
                ctx.beginPath();
                ctx.arc(hx, hy, (20 + random()*15) * mapState.zoom, Math.PI, Math.PI * 2);
                ctx.stroke();
             }
        }
    });

    // Отрисовка маркеров поверх тиррейна
    allPoints.forEach(point => {
        const pos = transform(point.worldX / COORD_MULT, point.worldY / COORD_MULT);
        ...
        drawMapMarker(ctx, pos, type, isPlayerLocation, isHovered);

        // Текст чернильный
        const fontSize = Math.max(9, Math.min(24, 12 * mapState.zoom));
        ctx.font = (isPlayerLocation || isHovered ? 'bold' : 'normal') + ' ' + fontSize + 'px Georgia, serif';
        
        // Белая/пергаментная обводка для читаемости поверх тиррейна
        ctx.lineWidth = 3;
        ctx.strokeStyle = '#e8dbbf';
        ctx.textAlign = 'center';
        const textY = pos.y + 14 + fontSize * 0.5;
        const nameToDraw = point.name.split('(')[0].trim();
        
        ctx.strokeText(nameToDraw, pos.x, textY);
        
        ctx.fillStyle = isHovered ? '#8b0000' : '#2c1e14'; // Темные чернила или красные (при наведении)
        ctx.fillText(nameToDraw, pos.x, textY);
    });

    drawCompassRose(ctx, width - 40, 40, 20);
}
...
function saveGame_OLD_UNUSED(slotType, slotId) {
    if (!player) {
        console.error("Не удается сохранить игру: данные игрока отсутствуют.");
        return false;
    }
    ...
    let saveData;
    try {
        const playerDataToSave = structuredClone(player);
        saveData = {
            slotType: slotType,
            slotId: slotId,
            timestamp: new Date().toISOString(),
            playerData: playerDataToSave,
            historyData: structuredClone(conversationHistory) // ИСПРАВЛЕНО: Теперь история сохраняется полностью
        };
    } catch (e) {
         console.warn("structuredClone не удалось во время сохранения, используется JSON fallback:", e);
         const playerDataToSave = JSON.parse(JSON.stringify(player)); // Fallback
         saveData = {
            slotType: slotType,
            slotId: slotId,
            timestamp: new Date().toISOString(),
            playerData: playerDataToSave,
            historyData: JSON.parse(JSON.stringify(conversationHistory.slice(-MAX_HISTORY * 2)))
        };
    }

    let targetArray = allSaves[slotType];
    let maxSlots = (slotType === 'manual') ? MAX_MANUAL_SAVES : MAX_AUTO_SAVES;

    if (!Array.isArray(targetArray)) {
        console.error("Неверный массив слотов сохранения:", slotType);
        allSaves[slotType] = [];
        targetArray = allSaves[slotType];
    }
    ...
    if (existingSlotIndex !== -1) { // Перезапись существующего слота
        targetArray[existingSlotIndex] = saveData;
        console.log(`Игра сохранена в существующий ${slotType} слот ${slotId}`);
    } else { // Добавление нового слота
        if (targetArray.length < maxSlots) {
            targetArray.push(saveData);
            console.log(`Игра сохранена в новый ${slotType} слот ${slotId}`);
        } else if (slotType === 'auto') { // Если автосохранения переполнены, удаляем старейшее
            targetArray.sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp)); // Сортируем по возрастанию даты
            const removedSlot = targetArray.shift(); // Удаляем первый (самый старый)
            console.log(`Автосохранение удаляет самый старый слот: #${removedSlot.slotId}`);
            targetArray.push(saveData); // Добавляем новый
            console.log(`Автосохранение добавило новый слот ${slotId}`);
        } else { // Ручные слоты переполнены
            console.error(`Не удается сохранить в ${slotType} слот ${slotId}: Все ${maxSlots} слотов заняты.`);
            alert(t('loadGame.errorSlotFull', { slotId: slotId, max: maxSlots }));
            return false;
        }
    }
    ...
    if (slotType === 'manual') {
        targetArray.sort((a, b) => a.slotId - b.slotId); // Ручные по ID
    } else {
        targetArray.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)); // Авто по убыванию даты
    }

    storeAllSaves(allSaves);
    return true;
}

function loadGame_OLD_UNUSED(slotType, slotId) {
    const allSaves = getAllSaves();
    const targetArray = allSaves[slotType];

    if (!Array.isArray(targetArray)) {
         console.error(`Неверный массив слотов сохранения для загрузки: ${slotType}`);
         alert(t('loadGame.errorInvalidSlotType', { slotType: slotType }));
         return;
    }
    const saveData = targetArray.find(save => save.slotId === slotId);

    if (saveData) {
        try {
            if (!saveData.playerData || !saveData.historyData || !saveData.timestamp) {
                throw new Error(t("loadGame.errorSaveCorrupted", "Данные сохранения неполные или повреждены."));
            }
            ...
            // ОБНОВЛЕНИЕ: Миграция старой системы репутации на новую
            if (typeof player.stats.reputation === 'number' || player.stats.reputation === undefined) {
                const oldRep = player.stats.reputation || 0;
                player.stats.reputation = { global: oldRep };
                console.log(`[loadGame] Старая система репутации (${oldRep}) перенесена в новую:`, player.stats.reputation);
            } else if (typeof player.stats.reputation !== 'object' || player.stats.reputation === null) {
                player.stats.reputation = { global: 0 }; // Fallback для некорректных данных
            }
            ...
            player.statusEffects = player.statusEffects || {};
            player.visibleEntities = player.visibleEntities || {};
            player.race = player.race || 'human';
            player.class = player.class || 'warrior';

            if (player.quests && Object.keys(player.quests).length > 0) {
                const maxId = Object.keys(player.quests)
                                  .map(id => parseInt(id, 10))
                                  .filter(id => !isNaN(id))
                                  .reduce((max, current) => Math.max(max, current), 0);
                nextInternalQuestId = maxId + 1;
            } else {
                nextInternalQuestId = 1;
            }
            // Аналогично нужно восстановить nextInternalItemId, nextInternalEntityId и т.д.
            // если они не сохраняются/восстанавливаются где-то еще.
            // Для простоты, если они не критичны для загрузки, можно их не трогать здесь,
            // но для полной консистентности их тоже нужно обрабатывать.
            // Пока оставим как есть, предполагая, что новые ID будут генерироваться корректно.
            const maxItemId = Object.keys(player.inventory || {}).map(id => parseInt(id, 10)).filter(id => !isNaN(id)).reduce((max, current) => Math.max(max, current), 0);
            nextInternalItemId = (maxItemId || 0) + 1;

            const maxEntityId = Object.keys(player.visibleEntities || {}).map(id => parseInt(id, 10)).filter(id => !isNaN(id)).reduce((max, current) => Math.max(max, current), 0);
            nextInternalEntityId = (maxEntityId || 0) + 1;

            const maxSkillIdInPlayer = Object.values(player.skills || {}).map(s => parseInt(s.id, 10)).filter(id => !isNaN(id)).reduce((max, current) => Math.max(max, current), 0);
            // Если SkillAI_ID не числовой, то эта логика не сработает для nextInternalSkillId.
            // Если SkillAI_ID это строка, то nextInternalSkillId не нужен или должен управляться иначе.
            // Предположим, что SkillAI_ID может быть строкой, и nextInternalSkillId не используется для генерации.

            const maxMapMarkerId = Object.keys(player.mapMarkers || {}).map(id => parseInt(id, 10)).filter(id => !isNaN(id)).reduce((max, current) => Math.max(max, current), 0);
            nextInternalMapMarkerId = (maxMapMarkerId || 0) + 1;


            if (player.class === 'mage') {
                player.stats.int = player.stats.int || BASE_CLASS_STATS.mage.int + (RACE_MODIFIERS[player.race]?.int || 0);
                player.stats.maxMana = calculateMaxMana(player.stats.int, player.stats.level);
                player.stats.mana = Math.min(player.stats.mana ?? player.stats.maxMana, player.stats.maxMana);
            } else {
                 player.stats.mana = 0;
            ...
            currentSaveSlot = { type: slotType, id: slotId };
            console.log(`Игра загружена из ${slotType} слота ${slotId}`);
            ...
            addLogMessage(t('gameInterface.log.gameLoaded', { slotType: slotTypeLocalized, slotId: slotId }), "system-message");
             ...
             if (conversationHistory.length > 0 && conversationHistory[conversationHistory.length - 1].role === 'model') {
                 const lastGmMessage = conversationHistory[conversationHistory.length - 1].parts[0].text;
                 const processedText = lastGmMessage.replace(/\[COMMAND:.+?\]/g, '').trim();
                 if (!processedText.startsWith(`[${t('error.gmBlockedMarker')}`)) {
                    addLogMessage(processedText, "gm-message");
                 } else {
                     addLogMessage(t('gameInterface.log.continueAdventure', 'Продолжайте ваше приключение!'), "gm-message");
                 }
             } else {
                 addLogMessage(t('gameInterface.log.continueAdventure', 'Продолжайте ваше приключение!'), "gm-message");
             }
            ...
            console.error(`Ошибка загрузки игры из ${slotType} слота ${slotId}:`, e);
            const errorMessage = e instanceof Error ? e.message : String(e);
            alert(t('loadGame.errorLoad', { slotId: slotId, message: errorMessage }));
            player = null;
            conversationHistory = [];
            nextInternalQuestId = 1;
            nextInternalItemId = 1;
...
function deleteSave(slotType, slotId) {
    const slotTypeLocalized = t(slotType === 'manual' ? 'loadGame.manualSlotType' : 'loadGame.autoSlotType', null, slotType);
    if (!confirm(t('loadGame.confirmDelete', { slotType: slotTypeLocalized, slotId: slotId }))) {
        return;
    }
    ...
    allSaves[slotType] = targetArray.filter(save => save.slotId !== slotId);

    if (allSaves[slotType].length < initialLength) {
        storeAllSaves(allSaves);
        console.log(`Слот сохранения ${slotType} #${slotId} удален.`);
        populateLoadGameScreen(); // Обновить список на экране загрузки
    } else {
        console.warn(`Слот сохранения ${slotType} #${slotId} не найден для удаления.`);
    }
}
...
function populateLoadGameScreen() {
    const allSaves = getAllSaves();

    manualSaveSlotsList.innerHTML = '';
    const manualSaves = allSaves.manual || [];
    // Сначала добавляем существующие сохранения
    manualSaves.sort((a, b) => a.slotId - b.slotId);
    manualSaves.forEach(save => {
        manualSaveSlotsList.appendChild(createSaveListItem(save));
    });
    // Затем добавляем пустые слоты, если их меньше MAX_MANUAL_SAVES
     for (let i = 1; i <= MAX_MANUAL_SAVES; i++) {
     ...
     // Финальная сортировка всех элементов (и существующих, и пустых) по ID
     Array.from(manualSaveSlotsList.children)
        .sort((a, b) => {
             const getId = (li) => {
                 const button = li.querySelector('.load-button'); // У загруженных слотов есть кнопка
                 if (button) return parseInt(button.dataset.id, 10);
                 // У пустых слотов ID из текста
                 const nameSpan = li.querySelector('.slot-name');
                 const match = nameSpan ? nameSpan.textContent.match(/#(\d+)/) : null; // Ищем #ID
                 return match ? parseInt(match[1], 10) : 999; // Пустые без ID в конец
             };
            return getId(a) - getId(b);
        })
        .forEach(li => manualSaveSlotsList.appendChild(li)); // Перевставляем в отсортированном порядке


    autoSaveSlotsList.innerHTML = '';
    const autoSaves = allSaves.auto || [];
    if (autoSaves.length > 0) {
         autoSaves.sort((a, b) => new Date(b.timestamp) - new Date(a.timestamp)); // Сначала самые новые
        autoSaves.forEach(save => {
            autoSaveSlotsList.appendChild(createSaveListItem(save));
        });
    } else {
        autoSaveSlotsList.innerHTML = `<li>${t('loadGame.noAutoSaves')}</li>`;
    }

    updateDynamicUIText(); // Обновить заголовки с количеством слотов
}
...
function createSaveListItem(saveData) {
    ...
    const playerName = saveData.playerData?.name || t('characterCreation.unknownName', '???');
    const playerRaceKey = saveData.playerData?.race
        ? `characterCreation.race${saveData.playerData.race.charAt(0).toUpperCase() + saveData.playerData.race.slice(1)}`
        : 'characterCreation.raceNotSelected';
    const playerRace = t(playerRaceKey, null, saveData.playerData?.race || 'N/A');

    const playerClassKey = saveData.playerData?.class
        ? `characterCreation.class${saveData.playerData.class.charAt(0).toUpperCase() + saveData.playerData.class.slice(1)}`
        : 'characterCreation.classNotSelected';
    const playerClass = t(playerClassKey, null, saveData.playerData?.class || 'N/A');

    const playerLevel = saveData.playerData?.stats?.level || '?';
    const playerLocation = saveData.playerData?.location || t('gameInterface.characterPanel.locationUnknown');
    const slotTypeKey = saveData.slotType === 'manual' ? 'loadGame.manualSlot' : 'loadGame.autoSlot';
    const slotDesc = t(slotTypeKey, { id: saveData.slotId });
    const levelAbbr = t('loadGame.levelAbbr', 'Ур.');

    // --- ИСПРАВЛЕННАЯ ЛОГИКА ОТОБРАЖЕНИЯ ИСТОЧНИКА ---
    let sourceInfo = "";
    // Свойство `fileName` добавляется только для сохранений из файловой системы в Electron.
    // Его наличие — надежный признак.
    if (saveData.fileName) {
        sourceInfo = `<span class="save-source-info">(${t('loadGame.sourceFileSystemShort', 'ФС')})</span>`;
    } else {
        // Если `fileName` нет, значит, это сохранение из Local Storage (для веб-версии).
        sourceInfo = `<span class="save-source-info">(${t('loadGame.sourceLocalStorageShort', 'LS')})</span>`;
    }

    li.innerHTML = `
        <div class="save-info">
            <span class="slot-name">${slotDesc} - ${playerName} (${playerRace}, ${playerClass} ${levelAbbr}${playerLevel}) ${sourceInfo}</span>
            <span class="save-time">${t('loadGame.saveInfo', { location: playerLocation, date: formattedDate })}</span>
        </div>
    ...
    li.querySelector('.load-button').addEventListener('click', async (e) => {
        const type = e.target.getAttribute('data-type');
        const id = parseInt(e.target.getAttribute('data-id'), 10);
        await loadGame(type, id);
    });
    li.querySelector('.delete-button').addEventListener('click', async (e) => {
        const type = e.target.getAttribute('data-type');
        const id = parseInt(e.target.getAttribute('data-id'), 10);
        await deleteSave(type, id);
    });
...
function drawCompassRose(ctx, x, y, radius) {
    ctx.strokeStyle = '#2c1e14'; // Чернила
    ctx.fillStyle = '#2c1e14';
    ctx.lineWidth = 1.5;

    // Линии
    ctx.beginPath();
    ctx.moveTo(x, y - radius); ctx.lineTo(x, y + radius); // N-S
    ctx.moveTo(x - radius, y); ctx.lineTo(x + radius, y); // W-E
    ctx.stroke();
...
function drawMapMarker(ctx, pos, type, isPlayer) {
    ctx.lineWidth = 1.5;
    ...
    if (isPlayer) {
        ...
        for (let i = 0; i < 10; i++) {
            ...
            ctx.lineTo(pos.x + radius * Math.cos(i * Math.PI / 5), pos.y + radius * Math.sin(i * Math.PI / 5));
        }
        ctx.closePath();
    } else {
        switch (type) {
