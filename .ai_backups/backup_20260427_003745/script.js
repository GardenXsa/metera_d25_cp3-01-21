...
async function runWorldSimulationTick() {
    ...
    try {
        ...
        // БЕРЕМ ЖЕСТКИЙ СЧЕТЧИК ДНЕЙ ИЗ ДВИЖКА (Никакой путаницы с пульсами)
        let daysPassed = World.time.daysSinceLastEvent || 1;
        let timePassedStr = daysPassed === 1 ? "1 день" : `${daysPassed} дн.`;
        World.time.daysSinceLastEvent = 0; // Сбрасываем счетчик после прочтения

        const prompt = `### ДИРЕКТИВА: ДВИЖОК МИРА (WORLD SIMULATOR) v3.0\nТвоя задача: проанализировать экономику, политику и слухи, чтобы написать сводку новостей.\n\n[КРИТИЧЕСКИЙ ТАЙМИНГ]: С момента прошлых новостей прошло ровно: ${timePassedStr}.\nТекущая дата: ${currentDateStr}\n\n${worldSummary}\n\nПРИКАЗЫ:\n1. Напиши атмосферную сводку новостей ("Вести из Эфира") на основе данных выше.\n2. ТЫ ОБЯЗАН логически учитывать прошедшее время (${timePassedStr}). Если прошло 14 дней — опиши, как долго длилась осада, как медленно распространялась чума или как долго шли армии. Если прошел 1 день — описывай события как стремительные.\n3. Если в сводке есть войны, осады или бедствия — сделай акцент на них. Если мир спокоен — опиши экономику (дефицит/цены) или перемещения войск.\n4. Пиши в стиле мрачной исторической хроники. Текст должен быть эпичным, но реалистичным.\n5. Твой ответ ДОЛЖЕН БЫТЬ СТРОГО ВАЛИДНЫМ JSON ОБЪЕКТОМ. Массив actions оставляй ПУСТЫМ [], так как всю математику мира уже сделал движок. Исключение: можешь использовать nexusDefine для создания глобальных эпохальных флагов. Формат:\n{\n  "narrative": "Твой художественный текст о событиях в мире...",\n  "actions": []\n}`;
