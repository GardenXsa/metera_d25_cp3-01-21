...
// AI TTS
let ttsEngine = 'browser'; // 'browser', 'openai', 'elevenlabs'
let aiTtsKey = '';
let aiTtsVoice = 'alloy';
let currentAudioObj = null; // Для остановки AI аудио
...
function setupTTS() {
    // Теперь TTS поддерживает внешние API (OpenAI, ElevenLabs), поэтому не отключаем его полностью в Electron.
    
    if (!speechSynthesis && ttsEngine === 'browser') {
        console.warn("Web Speech API не поддерживается. Выберите AI TTS в настройках.");
    }

    if (toggleTTSButton) {
        toggleTTSButton.style.display = 'inline-block';
...
async function speakText(text, btnElement = null) {
    ...
    // Сброс иконки у предыдущей кнопки, если нажали на другую
    if (window.currentTtsBtn && window.currentTtsBtn !== btnElement) {
        window.currentTtsBtn.innerHTML = '<i class="fas fa-play"></i>';
        window.currentTtsBtn.classList.remove('playing', 'loading');
    }
    window.currentTtsBtn = btnElement;

    // Если нажали на кнопку, которая СЕЙЧАС играет - останавливаем
    ...
    // Остановка любого текущего аудио перед новым запуском
    if (currentAudioObj) {
        currentAudioObj.pause();
        currentAudioObj.currentTime = 0;
    }
    ...
    if (ttsEngine === 'openai') {
        if (!aiTtsKey) {
            console.warn("OpenAI TTS: Ключ не указан.");
            onPlayEnd();
            return;
        }
        try {
            const response = await fetch('https://api.openai.com/v1/audio/speech', {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${aiTtsKey}`,
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    model: 'tts-1',
                    input: text,
                    voice: aiTtsVoice || 'alloy'
                })
            });
            ...
            currentAudioObj.onplay = onPlayStart;
            currentAudioObj.onended = onPlayEnd;
            currentAudioObj.play();
        } catch (e) {
            console.error("Ошибка OpenAI TTS:", e);
            onPlayEnd();
        }
    } else if (ttsEngine === 'elevenlabs') {
        if (!aiTtsKey) {
            console.warn("ElevenLabs TTS: Ключ не указан.");
            onPlayEnd();
            return;
        }
        try {
            const voiceId = aiTtsVoice || '21m00Tcm4TlvDq8ikWAM';
            ...
            console.error("Ошибка ElevenLabs TTS:", e);
            onPlayEnd();
        }
    } else {
        if (!speechSynthesis) {
        ...
        if (selectedTTSVoice) ttsUtterance.voice = selectedTTSVoice;
        ttsUtterance.pitch = 1;
        ttsUtterance.rate = 1;
        ttsUtterance.volume = 0.8;

        ttsUtterance.onstart = onPlayStart;
        ttsUtterance.onend = onPlayEnd;
...
async function initializeApp() {
    ...
    ttsEngine = localStorage.getItem('ttsEngine') || 'browser';
    aiTtsKey = localStorage.getItem('aiTtsKey') || '';
    aiTtsVoice = localStorage.getItem('aiTtsVoice') || 'alloy';
...
function initSettingsUI() {
    ...
    document.querySelectorAll('.settings-tab-btn').forEach(btn => {
        btn.onclick = () => {
            ...
            document.getElementById(`settings-tab-${btn.dataset.tab}`).classList.add('active');
        };
    });

    // 2. Провайдеры
    const switchProviderView = (provider) => {
        document.querySelectorAll('.provider-settings-group').forEach(g => g.style.display = 'none');
        const group = document.getElementById(`${provider}-settings-group`);
        if (group) group.style.display = 'block';
        
        const modelMap = { gemini: geminiModelId, llmost: llmostModelId, openrouter: openrouterModelId, deepseek: deepseekModelId, local: localModelId };
        if (modelIdInput) modelIdInput.value = modelMap[provider] || '';
    };

    if (providerSelect) {
        providerSelect.value = currentApiProvider;
        providerSelect.onchange = () => { currentApiProvider = providerSelect.value; switchProviderView(currentApiProvider); };
    ...
    // 4. Глаз (API Key)
    const tBtn = document.getElementById('toggle-key-view');
    const kInp = document.getElementById('gemini-api-key-input');
    if (tBtn && kInp) {
        tBtn.onclick = (e) => {
            e.preventDefault();
            const isPass = kInp.type === 'password';
            kInp.type = isPass ? 'text' : 'password';
            tBtn.innerHTML = isPass ? '<i class="fas fa-eye-slash"></i>' : '<i class="fas fa-eye"></i>';
        };
    }

    // 5. Звук
    const mS = document.getElementById('music-volume-slider');
    const sS = document.getElementById('sfx-volume-slider');
    if (mS) {
        mS.value = localStorage.getItem('musicVolume') || 0.2;
        mS.oninput = () => {
            const v = parseFloat(mS.value);
            if (audioPlayer) audioPlayer.volume = v;
            if (menuMusicPlayer) menuMusicPlayer.volume = v * 0.8;
        };
    }
    if (sS) {
        sS.value = localStorage.getItem('sfxVolume') || 0.3;
        sS.oninput = () => {
            const v = parseFloat(sS.value);
            hoverSfx.volume = v * 0.3; clickSfx.volume = v;
        };
    }
}
...
function saveSettings() {
    ...
    const modelKeyMap = { gemini: 'geminiModelId', llmost: 'llmostModelId', openrouter: 'openrouterModelId', deepseek: 'deepseekModelId', local: 'localModelId' };
    if (modelKeyMap[provider]) {
        localStorage.setItem(modelKeyMap[provider], modelId);
        window[modelKeyMap[provider]] = modelId;
    }

    const gKey = document.getElementById('gemini-api-key-input')?.value.trim();
    if (gKey) { geminiApiKeys = [gKey]; geminiApiKey = gKey; localStorage.setItem('geminiApiKeys', JSON.stringify([gKey])); }

    const mVol = document.getElementById('music-volume-slider')?.value || 0.2;
    const sVol = document.getElementById('sfx-volume-slider')?.value || 0.3;
    localStorage.setItem('musicVolume', mVol);
    localStorage.setItem('sfxVolume', sVol);

    updateApiKeyStatus();
    showCustomAlert('Настройки сохранены!');
    if (document.activeElement) document.activeElement.blur();
}
...
function setupEventListeners() {
    ...
    if (helpButton) helpButton.addEventListener('click', () => setActiveScreen('help-screen'));
    if (communityButton) communityButton.addEventListener('click', () => {
    ...
    const setBackBtn = document.getElementById('settings-back-button');
    if (setBackBtn) {
    ...
    if (inGameSettingsButton) {
        inGameSettingsButton.addEventListener('click', () => {
            closeInGameMenu();
            settingsReturnScreen = 'game-interface';
            setActiveScreen('settings-menu');
        });
    }
    if (inGameExitButton) inGameExitButton.addEventListener('click', () => exitToMainMenu());

    statIncreaseButtons.forEach(button => button.addEventListener('click', handleStatIncrease));
...
function addLogMessage(message, type = "gm-message") {
    ...
    try {
        if (type === 'gm-message') {
            ...
            cleanHtml = `<p class="${type}">${tempDiv.innerHTML}</p>`;
            textToSpeak = message;
        }
    } catch (e) {
        console.error("Ошибка при рендеринге сообщения:", e);
        cleanHtml = `<p class="${type}">>>> [Ошибка отображения сообщения]</p>`;
        textToSpeak = "";
    }

    // ЛОГИКА ГРУППИРОВКИ СИСТЕМНЫХ СООБЩЕНИЙ
    ...
    if (targetBubble) {
        // Если предыдущий пузырь тоже системный, просто добавляем текст внутрь него
        ...
        if (category === 'system') {
            ...
            // Логика сворачивания/разворачивания по клику
            bubble.addEventListener('click', () => {
                if (bubble.classList.contains('collapsed')) {
                    bubble.classList.remove('collapsed');
                    bubble.classList.add('expanded');
                    hintDiv.innerHTML = '<i class="fas fa-chevron-up"></i> Свернуть';
                } else {
            ...
            ttsBtn.title = "Озвучить сообщение";
            ttsBtn.onclick = () => speakText(textToSpeak, ttsBtn);
    ...
    if (type === 'gm-message' && isTTSEnabled) {
        ...
        const btn = gameLog.lastElementChild ? gameLog.lastElementChild.querySelector('.msg-tts-btn') : null;
        if (speakableText) speakText(speakableText, btn);
    }
}
...
async function sendApiRequest(promptTextForAI, isInitialPrompt = false, isDiceRollResponse = false, expiredEffects = [], isSummarizationRequest = false, skipLogic = false) {
    ...
    if (oldRetryBtn) {
        const wrapper = oldRetryBtn.closest('.message-wrapper');
        if (wrapper) wrapper.remove();
        else oldRetryBtn.remove();
    }
    ...
    try {
        ...
        if (isInitialPrompt) {
            ...
            showAiErrorModal(
                ...
                () => {
                    showLoadingScreen('loadingScreen.generatingWorld', 'Генерация мира...');
                    sendApiRequest(lastUserMessageForRetry, true, isDiceRollResponse, [], false, logicPhaseCompleted);
                }
            );
        } else {
            addLogMessage("Произошла ошибка генерации. Сервер не ответил или вернул неверный формат.", "system-message");
            showRetryButton(() => sendApiRequest(lastUserMessageForRetry, false, isDiceRollResponse, [], false, logicPhaseCompleted));
        }
