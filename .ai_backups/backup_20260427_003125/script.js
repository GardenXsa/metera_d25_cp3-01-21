...
function simulateOneDay() {
    ...
    for (let fId of fKeys) {
        ...
        for (let targetF of fKeys) {
            ...
            let change = Math.floor(Math.random() * 5) - 2; // ПЛАВНАЯ ДИПЛОМАТИЯ: от -2 до +2 в день (фикс биполярочки фракций при промотке)
...
async function runWorldSimulationTick() {
    ...
    try {
        ...
        // Вычисляем ТОЧНОЕ время, прошедшее с прошлого отчета ИИ
        if (!World.time.lastAiPulse) World.time.lastAiPulse = player.gameTime.totalPulses;
        const delta = player.gameTime.totalPulses - World.time.lastAiPulse;
        const dPass = Math.floor(delta / 288);
        const hPass = Math.floor((delta % 288) / 12);
        let timePassedStr = dPass > 0 ? `${dPass} дн. и ${hPass} ч.` : `${hPass} ч.`;
        if (delta === 0) timePassedStr = "менее часа";
        World.time.lastAiPulse = player.gameTime.totalPulses;

        const prompt = `### ДИРЕКТИВА: ДВИЖОК МИРА (WORLD SIMULATOR) v3.0
Твоя задача: проанализировать экономику и политику и сгенерировать ОДНО ГЛОБАЛЬНОЕ СОБЫТИЕ.

[КРИТИЧЕСКИЙ ТАЙМИНГ - ОБЯЗАТЕЛЬНО ПРОЧТИ!]: 
С момента твоей последней сводки прошло ровно: ${timePassedStr}.
Текущая дата: ${currentDateStr}

${worldSummary}

ПРИКАЗЫ:
1. ТЫ КАТЕГОРИЧЕСКИ ОБЯЗАН начать свой текст (поле narrative) с точного указания этого времени! Например: "Прошло ${timePassedStr}, и за это время мир изменился..." или "Спустя ${timePassedStr} после последних известий...".
2. ЗАПРЕЩЕНО писать "прошли одни сутки" или "прошел день", если в [КРИТИЧЕСКИЙ ТАЙМИНГ] указано другое время! Строго используй время из переменной: ${timePassedStr}.
3. Твоя главная цель — красиво описать (narrative) сухие цифры из сводки выше.
4. В поле 'narrative' пиши в стиле исторической хроники или Вестей из Эфира.
5. Твой ответ ДОЛЖЕН БЫТЬ СТРОГО ВАЛИДНЫМ JSON ОБЪЕКТОМ. Массив actions оставляй ПУСТЫМ [], так как всю математику мира уже сделал движок. Исключение: можешь использовать nexusDefine для создания глобальных эпохальных флагов. Формат:
{
  "narrative": "Твой художественный текст о глобальном событии...",
  "actions": []
}`;
        ...
        if (loaderDiv) loaderDiv.remove();
        isSimulatingWorld = false; // Снимаем блокировку до вызова модалки, чтобы кнопка повтора сработала
        showAiErrorModal(
            e.message || String(e),
            false,
