...
// Голод и Бунты (Причина: нет еды или работы)
    ...
    for (let fId of fKeys) {
        ...
        let stateUpkeep = f.regionCount * 300; // Снижено содержание, чтобы фракции не банкротились за неделю
        ...
        if (f.stability < 20 && Math.random() < 0.1) {
            ...
            f.stability = 60; // Новая власть жестко берет контроль
            f.resources.gold.amount += 5000; // Конфискация имущества старой элиты (чтобы не обанкротиться завтра же)
