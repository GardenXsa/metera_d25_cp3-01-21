...
function simulateOneDay() {
    ...
    // 2. Продвинутая Экономика
    const basePrices = { food: 1, wood: 2, ore: 5, weapons: 20 };
    let rKeys = Object.keys(World.regions);

    for (let rId of rKeys) {
        let r = World.regions[rId];
        if (!r.facilities) r.facilities = { farms: 1, lumbermills: 1, mines: 1, forges: 1 };

        let workforce = r.population / 100;
        ...
        r.resources.food += Math.floor(workforce * r.facilities.farms * 2 * weatherMod);
        r.resources.wood += Math.floor(workforce * r.facilities.lumbermills * weatherMod);
        r.resources.ore += Math.floor(workforce * r.facilities.mines);
        
        let oreToUse = Math.min(r.resources.ore, Math.floor(workforce * r.facilities.forges * 0.5));
        if (oreToUse > 0) {
            r.resources.ore -= oreToUse;
            r.resources.weapons += Math.floor(oreToUse * 0.2);
        }
        ...
        if (r.resources.food < 0) {
            ...
            r.population -= deaths;
            if (Math.random() < 0.2) generateWorldNews(`Ужасный голод в ${r.name}! Погибло ${deaths} человек.`, rId, 4, 'disaster');
        }
        if (r.resources.wood < 0) r.resources.wood = 0;

        for (let good in basePrices) {
            let supply = r.resources[good] || 1;
            let demand = (good === 'food') ? foodConsumed : (good === 'wood' ? woodConsumed : r.population * 0.001);
            if (demand < 1) demand = 1;
            let priceMod = Math.max(0.2, Math.min(10, demand / supply));
    ...
    if (DEBUG_MODE && typeof World !== 'undefined' && World) {
        ...
        for (let rId in World.regions) {
            ...
            html += `<div class="debug-item">
                     ...
                     Поп: ${Math.floor(r.population)}<br>
                     Склад: 🍖${Math.floor(r.resources.food)} 🪨${Math.floor(r.resources.ore)}
                     </div>`;
        }
        html += '</div></div>';
