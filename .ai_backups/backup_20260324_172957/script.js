...
let currentGmArchitecture = 'unified'; // 'unified', 'dual'
...
async function initializeApp() {
    ...
    currentGmArchitecture = localStorage.getItem('gmArchitecture') || 'unified';
...
function initSettingsUI() {
    ...
    const gmArchSelect = document.getElementById('gm-architecture-select');
    ...
    if (gmArchSelect) gmArchSelect.value = currentGmArchitecture;
...
function saveSettings() {
    ...
    const gmArch = document.getElementById('gm-architecture-select')?.value || 'unified';
    ...
    currentGmArchitecture = gmArch;
    localStorage.setItem('gmArchitecture', currentGmArchitecture);
    const cachingCheckbox = document.getElementById('prompt-caching-checkbox');
    if (cachingCheckbox) {
...
function setupEventListeners() {
    ...
    const toggleKeyViewBtn = document.getElementById('toggle-key-view');
    const geminiApiKeyInputEl = document.getElementById('gemini-api-key-input');
    if (toggleKeyViewBtn && geminiApiKeyInputEl) {
        toggleKeyViewBtn.addEventListener('click', (e) => {
            e.preventDefault();
            if (geminiApiKeyInputEl.type === 'password') {
                geminiApiKeyInputEl.type = 'text';
                toggleKeyViewBtn.innerHTML = '<i class="fas fa-eye-slash"></i>';
            } else {
                geminiApiKeyInputEl.type = 'password';
                toggleKeyViewBtn.innerHTML = '<i class="fas fa-eye"></i>';
            }
        });
    }
    ...
    userInput.addEventListener('keydown', (event) => {
        if (event.key === 'Enter' && !event.shiftKey && !isWaitingForAI) {
            ...
            handleUserInput();
            userInput.style.height = 'auto'; // Сброс высоты после отправки
        }
    });
    
    // Автоматическое растягивание поля ввода
...
function showEntityTooltip(event) {
    ...
    if (data.traits && data.traits.length > 0) {
        traitsHtml = `<p><strong>Черты:</strong> <span style="color:#9b59b6">${data.traits.join(', ')}</span></p>`;
    } else {
        traitsHtml = `<p><strong>Черты:</strong> <span style="color:#7f8c8d">???</span></p>`;
    }
...
function addLogMessage(message, type = "gm-message", isRestoring = false) {
    ...
    if (textToSpeak && textToSpeak.trim() !== '' && category !== 'system') {
        if (!bubbleElement.querySelector('.tts-speak-btn')) {
            ...
            ttsBtn.onclick = (e) => {
                e.stopPropagation();
                // Читаем актуальный текст из пузыря (без OOC и кнопок)
...
async function performAiFetch(systemInstruction, history, providerModel, currentInput = "") {
    ...
    let finalSystemInstruction = systemInstruction + "\n\n[CRITICAL SYSTEM DIRECTIVE]: YOU MUST OUTPUT ONLY VALID JSON. DO NOT WRAP IN MARKDOWN BLOCKS. YOUR ENTIRE RESPONSE MUST BE PARSABLE BY JSON.parse(). DO NOT FORGET THE 'actions' ARRAY FOR MECHANICS. IF THIS IS THE FIRST TURN, YOU MUST USE 'addItem' AND 'equipItem' FOR WEAPON AND ARMOR!";
    ...
    while (attempts < maxAttempts) {
        ...
        let messages = [];
        let sysMsg = { role: "system", content: finalSystemInstruction };
        
        if (usePromptCaching) {
            if (currentApiProvider === 'openrouter' || currentApiProvider === 'llmost' || currentApiProvider === 'deepseek') {
                sysMsg.content = [
                    {
                        type: "text",
        ...
        if (currentApiProvider === 'local') {
            ...
            requestBody = { model: providerModel || "local-model", messages: messages, temperature: 0.7, max_tokens: 4096, response_format: { type: "json_object" } };
        } else if (currentApiProvider === 'deepseek') {
            targetUrl = "https://api.deepseek.com/v1/chat/completions";
            headers['Authorization'] = 'Bearer ' + deepseekApiKey;
            requestBody = { model: providerModel, messages: messages, temperature: 0.7, max_tokens: 4096, response_format: { type: "json_object" } };
            ...
            requestBody = {
                ...
                contents: contents,
                generationConfig: { maxOutputTokens: 8192, temperature: 0.75, responseMimeType: "application/json" }
            };
            isGeminiFormat = true;
        }
        ...
        let response;
        try {
            response = await fetch(targetUrl, { method: 'POST', headers: headers, body: JSON.stringify(requestBody) });
        ...
        throw new Error("Не удалось извлечь текст из ответа API. Ответ: " + JSON.stringify(data).substring(0, 200));
    }
}
...
async function sendApiRequest(promptTextForAI, isInitialPrompt = false, isDiceRollResponse = false, expiredEffects = [], isSummarizationRequest = false, skipLogic = false) {
    ...
    try {
        ...
        if (currentGmArchitecture === 'unified') {
            console.log(">>> Запуск Единого Мастера Игры...");
            let systemPrompt = await prepareLogicPrompt(expiredEffects, promptTextForAI);
            const style = narrators[currentNarratorIndex] ? await (await fetch(narrators[currentNarratorIndex].promptFile)).text() : "";
            systemPrompt += `\n\n### ТВОЙ СТИЛЬ ПОВЕСТВОВАНИЯ:\n${style}\n\nТвоя задача: проанализировать ввод, провести расчеты в 'internal_thoughts', написать художественный ответ в 'narrative' и выдать системные команды в 'actions'.`;

            let rawResponse = await performAiFetch(systemPrompt, conversationHistory, modelIdForRequest, promptTextForAI);
            let result = parseAIResponse(rawResponse);

            if (!result || !result.narrative) throw new Error("GM вернул пустой ответ или нарушил формат JSON.");

            removeEtherLoader();
            
            if (result.internal_thoughts) {
                addCalculationMessage(`[МЫСЛИ GM]: ${result.internal_thoughts}`);
            }

            addLogMessage(result.narrative, "gm-message");
            
            if (!isInitialPrompt && !isSummarizationRequest) {
 ...
 */
function OLD_parseAIResponse(rawResponse) { return {}; }
...
function parseAIResponse(rawResponse) {
    ...
    if (!rawResponse) return { narrative, actions, logic_summary, internal_thoughts };

    const bt = String.fromCharCode(96, 96, 96);
    let cleanJsonString = rawResponse.replace(new RegExp('^' + bt + 'json', 'im'), '').replace(new RegExp('^' + bt, 'im'), '').replace(new RegExp(bt + '$', 'im'), '').trim();

    try {
        const parsed = JSON.parse(cleanJsonString);
        actions = parsed.actions || [];
        logic_summary = parsed.logic_summary || "";
        internal_thoughts = parsed.internal_thoughts || "";
        narrative = parsed.narrative || "";
        return { narrative, actions, logic_summary, internal_thoughts };
    } catch (jsonErr) {
        console.warn("Стандартный парсинг не удался, применяем авто-хирург JSON...", jsonErr);
        ...
        try {
            ...
            console.error("КРИТИЧЕСКАЯ ОШИБКА: JSON поврежден окончательно.", secondErr);
            
            const narrativeMatch = rawResponse.match(new RegExp('"narrative"\\s*:\\s*"([^"\\\\]*(?:\\\\.[^"\\\\]*)*)"'));
            if (narrativeMatch) narrative = narrativeMatch[1].replace(new RegExp('\\\\"', 'g'), '"').replace(new RegExp('\\\\n', 'g'), '\n');
            else narrative = rawResponse;
            
            return { narrative, actions: [], logic_summary: "", internal_thoughts: "Ошибка парсинга JSON." };
        }
    }
}


function IGNORED_OLD_parseAIResponse(rawResponse) {
    let narrative = "";
    let actions = [];
    let logic_summary = "";
    let internal_thoughts = "";
    ...
    let cleanJsonString = rawResponse.replace(new RegExp('^`{3}json', 'im'), '').replace(new RegExp('^`{3}', 'im'), '').replace(new RegExp('`{3}$', 'im'), '').trim();
    ...
    try {
        ...
        try {
            ...
            const narrativeMatch = rawResponse.match(/"narrative"\s*:\s*"([^"\\]*(?:\\.[^"\\]*)*)"/);
            if (narrativeMatch) narrative = narrativeMatch[1].replace(/\\"/g, '"').replace(/\\n/g, '\n');
            else narrative = rawResponse;
...
async function prepareLogicPrompt(expiredEffects, promptTextForAI) {
    try {
        let logicRules = await loadPromptFromFile('assets/promts/logic_rules.txt');
        ...
        if (currentGmArchitecture === 'unified') {
            jsonProtocol = "// --- ПРОТОКОЛ ФОРМАТИРОВАНИЯ ОТВЕТА (КАТЕГОРИЧЕСКИ СТРОГОЕ СОБЛЮДЕНИЕ!) ---\\nТВОЙ ОТВЕТ ВСЕГДА ДОЛЖЕН БЫТЬ ВАЛИДНЫМ JSON ОБЪЕКТОМ. НИЧЕГО КРОМЕ JSON.\\nКРИТИЧЕСКОЕ ПРАВИЛО СИНТАКСИСА: Ты ОБЯЗАН экранировать все внутренние кавычки в тексте (используй \\\"\\\"). ЗАПРЕЩЕНО забывать запятые между объектами в массиве \"actions\".\\n\\nСтруктура JSON-объекта:\\n{\\n  \"internal_thoughts\": \"ОБЯЗАТЕЛЬНОЕ ПОЛЕ. Сначала напиши сюда свои скрытые расчеты. Кто попал? Сколько урона? Какие предметы нужно выдать? Это твои мысли как системы.\",\\n  \"narrative\": \"Твой художественный текст для игрока. Опиши то, что ты решил в internal_thoughts. Используй диалоги и красивые описания.\",\\n  \"actions\": [\\n    {\\n      \"command\": \"имяКоманды\",\\n      \"args\": { \"аргумент1\": \"значение\" }\\n    }\\n  ]\\n}";
            archRules = "ROLE: UNIFIED GAME MASTER\\nТы — Мастер Игры. Ты одновременно математический движок и талантливый писатель.\\nАЛГОРИТМ ТВОЕЙ РАБОТЫ (СТРОГО):\\n1. Прочитай ввод игрока и системные броски кубиков.\\n2. В поле \"internal_thoughts\" напиши свои рассуждения: \"Игрок выбросил 15, защита врага 12. Попадание. Урон 5. У врага осталось 0 HP. Враг убит. Нужно выдать лут.\"\\n3. В поле \"narrative\" напиши красивый художественный текст, описывающий то, что ты решил в шаге 2.\\n4. В поле \"actions\" вызови нужные команды (например, updateEntityStat hp=0, addItem).";
            poetInst = "ИНСТРУКЦИЯ ПО КОМАНДАМ И НАРРАТИВУ (КРИТИЧЕСКИ ВАЖНО!):\\nЕсли ты использовал команды (addItem, addQuest, updateEntityStat: hp=0), ты КАТЕГОРИЧЕСКИ ОБЯЗАН описать эти события в поле `narrative`. Игрок должен понимать из текста, откуда у него взялся предмет или почему обновился квест.";
        } else {
            jsonProtocol = "// --- ПРОТОКОЛ ФОРМАТИРОВАНИЯ ОТВЕТА (КАТЕГОРИЧЕСКИ СТРОГОЕ СОБЛЮДЕНИЕ!) ---\\nТВОЙ ОТВЕТ ВСЕГДА ДОЛЖЕН БЫТЬ ВАЛИДНЫМ JSON ОБЪЕКТОМ. НИЧЕГО КРОМЕ JSON.\\nКРИТИЧЕСКОЕ ПРАВИЛО СИНТАКСИСА: Ты ОБЯЗАН экранировать все внутренние кавычки в тексте (используй \\\"\\\"). ЗАПРЕЩЕНО забывать запятые между объектами в массиве \"actions\".\\n\\nСтруктура JSON-объекта:\\n{\\n  \"logic_summary\": \"ОБЯЗАТЕЛЬНОЕ ПОЛЕ ДЛЯ СЧЕТОВОДА! Пиши сюда результаты бросков, кто кому нанес урон, кто промахнулся, что изменилось. Поэт будет читать это поле для описания.\",\\n  \"narrative\": \"Твой художественный текст (если ты Поэт).\",\\n  \"actions\": [\\n    {\\n      \"command\": \"имяКоманды\",\\n      \"args\": { \"аргумент1\": \"значение\" }\\n    }\\n  ]\\n}";
            archRules = "ROLE: HARDCODED LOGIC ENGINE\\nТы — программное ядро игры \"Хроники Метеры\". Ты не человек, ты — вычислительная машина. Твоя задача: анализировать ввод игрока и snapshot его данных, проводить скрытые расчеты и выдавать инструкции для движка.\\nПРАВИЛО ВЗАИМОДЕЙСТВИЯ:\\nТвое поле \"logic_summary\" — это единственный способ заставить Поэта описать механические действия.\\nПлохо: \"addQuest выполнен, addItem выполнен\". (Поэт это проигнорирует).\\nХОРОШО: \"Игрок прибыл в локацию. Я выдал квест 'Пропавший разведчик' от лица Капитана Элиаса и добавил в инвентарь Короткий меч. Поэт, опиши встречу с Капитаном и передачу меча\".";
            poetInst = "ИНСТРУКЦИЯ ПО КОМАНДАМ И УПРАВЛЕНИЕ ПОЭТОМ (КРИТИЧЕСКИ ВАЖНО!):\\nТы — ведущий разум. Поэт — лишь твой рупор. Чтобы Поэт ничего не забыл, ты ОБЯЗАН в конце поля \"logic_summary\" написать для него прямую инструкцию в формате:\\n\\nПОЭТ, ТВОЯ ЗАДАЧА ОПИСАТЬ:\\n1) [Опиши смерть врага X]\\n2) [Опиши, как игрок находит предмет Y]\\n3) [Опиши, как обновляется цель квеста Z]\\n\\nЕсли ты использовал команды (addItem, addQuest, updateEntityStat: hp=0), ты КАТЕГОРИЧЕСКИ ОБЯЗАН приказать Поэту описать эти события. Без твоего приказа Поэт может увлечься графоманией и забыть упомянуть, что игрок получил лут или квест.";
        }

        logicRules = logicRules.replace('{architecture_specific_rules}', archRules).replace('{poet_instruction}', poetInst);
        masterInstructions = masterInstructions.replace('{json_format_protocol}', jsonProtocol);
...
async function prepareNarrativePrompt(logicSummary, promptTextForAI) {
    ...
    let jsonProtocol = "// --- ПРОТОКОЛ ФОРМАТИРОВАНИЯ ОТВЕТА (КАТЕГОРИЧЕСКИ СТРОГОЕ СОБЛЮДЕНИЕ!) ---\\nТВОЙ ОТВЕТ ВСЕГДА ДОЛЖЕН БЫТЬ ВАЛИДНЫМ JSON ОБЪЕКТОМ. НИЧЕГО КРОМЕ JSON.\\nКРИТИЧЕСКОЕ ПРАВИЛО СИНТАКСИСА: Ты ОБЯЗАН экранировать все внутренние кавычки в тексте (используй \\\"\\\"). ЗАПРЕЩЕНО забывать запятые между объектами в массиве \"actions\".\\n\\nСтруктура JSON-объекта:\\n{\\n  \"logic_summary\": \"ОБЯЗАТЕЛЬНОЕ ПОЛЕ ДЛЯ СЧЕТОВОДА! Пиши сюда результаты бросков, кто кому нанес урон, кто промахнулся, что изменилось. Поэт будет читать это поле для описания.\",\\n  \"narrative\": \"Твой художественный текст (если ты Поэт).\",\\n  \"actions\": [\\n    {\\n      \"command\": \"имяКоманды\",\\n      \"args\": { \"аргумент1\": \"значение\" }\\n    }\\n  ]\\n}";
    masterInstructions = masterInstructions.replace('{json_format_protocol}', jsonProtocol);
        ...
        switch (command) {
            ...
            case 'setLocation':
                if (args.locationName) {
                    player.location = String(args.locationName).trim().replace(/_/g, ' ');
                ...
                break;
            case 'calculationLog':
                if (args.message) addCalculationMessage(String(args.message));
                break;
            case 'updateStat':
                if (args.stat && typeof args.change === 'number') {
                    const oldValue = player.stats[args.stat] || 0;
                    player.stats[args.stat] = oldValue + args.change;
                    if (args.stat === 'hp') player.stats.hp = Math.max(0, Math.min(player.stats.hp, player.stats.maxHp));
                    updateCharacterSheet();
                }
                break;
            case 'addItem':
                if (args.aiIdentifier && args.name) {
                    const newItemId = nextInternalItemId++;
                    player.inventory[newItemId] = { id: newItemId, ...args };
                    updateInventoryDisplay();
                }
                break;
            case 'setEntityState':
                if (args.aiIdentifier && args.property && typeof args.value === 'boolean') {
                    const ent = Object.values(player.visibleEntities).find(e => e.aiIdentifier?.toLowerCase() === args.aiIdentifier.toLowerCase());
                    if (ent && args.property.toLowerCase() === 'ishostile') {
                        ent.isHostile = args.value;
                        updateEnvironmentPanel();
                    }
                }
                break;
            case 'revealEntityTrait':
                if (args.aiIdentifier && args.trait) {
                    const ent = Object.values(player.visibleEntities).find(e => e.aiIdentifier?.toLowerCase() === args.aiIdentifier.toLowerCase());
                    if (ent) {
                        if (!ent.traits) ent.traits = [];
                        if (!ent.traits.includes(args.trait)) {
                            ent.traits.push(args.trait);
                            feedback = `[СИСТЕМА] Раскрыта черта характера ${ent.name}: ${args.trait}`;
                            updateEnvironmentPanel();
                        }
                    }
                }
                break;
            case 'setCombatState':
                let isActiveVal = (typeof args.isActive === 'string') ? (args.isActive.toLowerCase() === 'true') : !!args.isActive;
                ...
                document.querySelector('.input-area').style.boxShadow = isActiveVal ? 'inset 0 0 20px rgba(231, 76, 60, 0.3)' : 'none';
                break;
        }
    } catch (error) {
...
// --- ГЛОБАЛЬНЫЙ ФИКС ЗАЛИПАНИЯ ALT В WINDOWS ---
