...
async function generateVisionImage(prompt, btnElement, bubbleElement) {
    ...
    try {
        ...
        if (data.data && data.data[0] && data.data[0].url) {
            // Встраиваем изображение ПРЯМО В ПУЗЫРЬ ЧАТА
            const imgContainer = document.createElement('div');
            imgContainer.className = 'chat-illustration-container';
            imgContainer.innerHTML = `<img src="${data.data[0].url}" alt="Эфирное видение" title="Нажми, чтобы открыть оригинал" onclick="window.open(this.src, '_blank')"><div class="img-watermark">Эфирный Слепок</div>`;
            bubbleElement.appendChild(imgContainer);
            btnElement.style.display = 'none'; // Прячем кнопку
