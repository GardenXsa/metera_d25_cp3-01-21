...
const OwnershipService = {
    canAccess: function(actorId, containerId) {
        ...
        if (cont.type === 'faction_vault' && actorId === 'player') {
            const pRank = (player.factionData && player.factionData[cont.owner_id]?.rank) || 0;
            const reqRank = cont.custom_props?.required_rank || 1;
            if (pRank < reqRank) return false;
        }
        if (cont.lock_data && cont.lock_data.is_locked) return false;
        return true;
    }
};
...
const CoreInventorySystem = {
    createContainer: function(type, ownerId, maxWeight, maxSlots, locationData = null) {
        const id = "cont_" + generateUUID();
        const container = {
            id: id, type: type, max_weight_kg: maxWeight, max_slots: maxSlots, owner_id: ownerId,
            location: locationData || { world_coords: null, parent_entity: null, parent_container: null },
            lock_data: { is_locked: false, difficulty: 10, trap: null },
    ...
    createItem: function(prototypeId, quantity, containerId, customProps = {}) {
        ...
        const item = {
            id: id, prototype_id: prototypeId, name: proto ? proto.name : prototypeId, stack_size: quantity, container_id: containerId, slot_index: null, state: "idle",
            flags: { quest_item: false, bound_to_owner: null, stolen: false, magical: false, fragile: false },
            durability: proto ? (proto.durability || 100) : 100,
            custom_props: { weight_per_unit: 1, ...customProps },
            created_at: player ? player.stats.turnCount : 0, last_moved_at: player ? player.stats.turnCount : 0
        };
    ...
    moveItem: function(itemId, sourceContainerId, targetContainerId, quantity = null) {
        if (!ItemRegistry.has(itemId)) return { success: false, error: "Item not found" };
        let item = ItemRegistry.get(itemId);
        if (item.state === "in_trade" || item.state === "bound") return { success: false, error: "Item state locks movement" };

        if (!OwnershipService.canAccess('player', sourceContainerId) || !OwnershipService.canAccess('player', targetContainerId)) return { success: false, error: "Access denied by OwnershipService" };

        let actualTargetId = targetContainerId;
        if (!actualTargetId) {
            let groundPile = Array.from(ContainerRegistry.values()).find(c => c.type === 'ground_pile' && c.location === player.location);
            if (!groundPile) actualTargetId = this.createContainer('ground_pile', null, 9999, 999, player.location);
            else actualTargetId = groundPile.id;
        }

        if (ContainerRegistry.has(actualTargetId)) {
            const tgt = ContainerRegistry.get(actualTargetId);
            if (tgt.items.length >= tgt.max_slots) return { success: false, error: "Target container is full (slots)" };
            const currentWeight = this.getContainerWeight(actualTargetId);
            const itemWeight = (item.custom_props.weight_per_unit || 1) * (quantity !== null ? quantity : item.stack_size);
            if (currentWeight + itemWeight > tgt.max_weight_kg) return { success: false, error: "Target container is full (weight)" };
        }

        if (quantity !== null && quantity > 0 && quantity < item.stack_size) {
            item.stack_size -= quantity;
            const newItemId = this.createItem(item.prototype_id, quantity, sourceContainerId, item.custom_props);
            item = ItemRegistry.get(newItemId);
        }

        if (sourceContainerId && ContainerRegistry.has(sourceContainerId)) {
            const src = ContainerRegistry.get(sourceContainerId);
            src.items = src.items.filter(id => id !== item.id);
        }

        if (ContainerRegistry.has(actualTargetId)) {
            ContainerRegistry.get(actualTargetId).items.push(item.id);
            item.container_id = actualTargetId;
            
            const srcCont = sourceContainerId ? ContainerRegistry.get(sourceContainerId) : null;
            if (srcCont && srcCont.type === 'faction_vault' && actualTargetId === player.container_backpack) {
                item.flags.stolen = true;
                const regionId = srcCont.location?.region_id || srcCont.location;
                if (regionId && typeof World !== 'undefined' && World.regions[regionId]) {
                    const reg = World.regions[regionId];
                    if (reg.resources[item.prototype_id]) {
                        reg.resources[item.prototype_id].amount = Math.max(0, reg.resources[item.prototype_id].amount - item.stack_size);
                    }
                }
            }
        } else {
        ...
        item.last_moved_at = player ? player.stats.turnCount : 0;
        return { success: true };
    },
    ...
    destroyContainer: function(containerId) {
        const cont = ContainerRegistry.get(containerId);
        if (!cont) return false;
        let groundPileId = Array.from(ContainerRegistry.values()).find(c => c.type === 'ground_pile' && c.location === cont.location)?.id;
        if (!groundPileId) groundPileId = this.createContainer('ground_pile', null, 9999, 999, cont.location);
        const itemsToMove = [...cont.items];
        itemsToMove.forEach(itemId => this.moveItem(itemId, containerId, groundPileId));
    ...
    unlockContainer: function(containerId, actorId) {
        ...
        if (roll >= cont.lock_data.difficulty) {
            ...
            return { success: false, error: "Lockpick broke, failed to unlock" };
        }
    },
    confiscateStolen: function(sourceContainerId, destContainerId) {
        ...
        stolenItems.forEach(id => {
            if (this.moveItem(id, sourceContainerId, destContainerId).success) count++;
        });
        return count;
    },
    buildContainer: function(actorId, type, location) {
        ...
        return this.createContainer(type, actorId, 100, 20, location);
    }
};;

const TradeSystem = {
    activeTrades: new Map(),
    initiateTrade: function(initiatorId, targetId, initiatorContainerId, targetContainerId) {
        const tradeId = "trade_" + generateUUID();
        this.activeTrades.set(tradeId, { initiator: initiatorId, target: targetId, initiator_container: initiatorContainerId, target_container: targetContainerId, offer_items: [], request_items: [], status: "pending" });
        return tradeId;
    },
    addItemToTrade: function(tradeId, itemId, side) {
        if (!this.activeTrades.has(tradeId)) return false;
        const trade = this.activeTrades.get(tradeId);
        const item = ItemRegistry.get(itemId);
        if (!item || item.state !== "idle") return false;
        item.state = "in_trade";
        if (side === "offer") trade.offer_items.push(itemId);
        else trade.request_items.push(itemId);
        return true;
    },
    confirmTrade: function(tradeId) {
        ...
        for (const itemId of trade.offer_items) {
            const res = CoreInventorySystem.moveItem(itemId, trade.initiator_container, trade.target_container);
            if (!res.success) return this._rollbackTrade(tradeId, "Failed to move offer item");
            ItemRegistry.get(itemId).state = "idle";
        }
        for (const itemId of trade.request_items) {
            const res = CoreInventorySystem.moveItem(itemId, trade.target_container, trade.initiator_container);
            if (!res.success) return this._rollbackTrade(tradeId, "Failed to move request item");
            ItemRegistry.get(itemId).state = "idle";
        }
    ...
    _rollbackTrade: function(tradeId, reason) {
        const trade = this.activeTrades.get(tradeId);
        [...trade.offer_items, ...trade.request_items].forEach(itemId => {
    ...
    negotiateTrade: function(tradeId, newOfferItemIds, newRequestItemIds) {
        if (!this.activeTrades.has(tradeId)) return false;
        const trade = this.activeTrades.get(tradeId);
        [...trade.offer_items, ...trade.request_items].forEach(itemId => {
            if (ItemRegistry.has(itemId)) ItemRegistry.get(itemId).state = "idle";
        ...
        trade.request_items = [];
        let success = true;
        newOfferItemIds.forEach(id => { if(!this.addItemToTrade(tradeId, id, "offer")) success = false; });
        newRequestItemIds.forEach(id => { if(!this.addItemToTrade(tradeId, id, "request")) success = false; });
        return success;
    }
};;
...
function buildFullPlayerSnapshot() {
    ...
    if (World) {
        ...
        // 1. Локация игрока и влияние на атмосферу
        ...
        if (playerRegion) {
            ...
            // АНАЛИЗ ДЛЯ GM: Как симуляция влияет на улицу
            worldContextString += `[АТМОСФЕРА НА УЛИЦАХ (GM, ОПИШИ ЭТО ИГРОКУ)]:\n`;
            if (ownerFaction && Object.values(ownerFaction.diplomacy).includes("war")) {
                worldContextString += `- СТРАНА В СОСТОЯНИИ ВОЙНЫ: На улицах много патрулей, стража нервная, ходят слухи о призыве, беженцы просят милостыню.\n`;
            }
            if (ownerFaction && ownerFaction.stability < 40) {
                worldContextString += `- НИЗКАЯ СТАБИЛЬНОСТЬ (${Math.floor(ownerFaction.stability)}%): Власть слаба. На улицах открыто действуют бандиты, стража берет взятки, люди напуганы.\n`;
            }
            
            // Экономика
            let breadPrice = r.markets.bread || 5;
            let baseBreadPrice = ECONOMY_ITEMS['bread'] ? ECONOMY_ITEMS['bread'].basePrice : 5;
            if (breadPrice > baseBreadPrice * 2) {
                worldContextString += `- ГОЛОД И ДЕФИЦИТ: Цены на еду заоблачные. Нищие дерутся за корки хлеба, торговцы прячут товар.\n`;
            } else if (breadPrice <= baseBreadPrice) {
                worldContextString += `- ПРОЦВЕТАНИЕ: Рынки полны еды, люди сыты и спокойны.\n`;
            }
            worldContextString += `Рынок (Цены): Еда(${r.markets.bread}), Дерево(${r.markets.wood}), Руда(${r.markets.iron_ore}), Оружие(${r.markets.weapons})\n\n`;
        } else {
            worldContextString += "[ВАША ЛОКАЦИЯ] Игрок находится вне крупных симулируемых регионов. Дикие земли.\n\n";
        }

        // 2. Фракции и Армии (Кратко)
        worldContextString += "[ГЛОБАЛЬНАЯ ПОЛИТИКА]\n";
        let activeWars = [];
        for (let fId in World.factions) {
            let f = World.factions[fId];
            for (let target in f.diplomacy) {
        ...
        if (activeWars.length > 0) worldContextString += "Идут войны: " + activeWars.join(", ") + "\n";
        worldContextString += "\n";

        // 3. Историческая сводка для ИИ
        let historicalNews = World.news.filter(n => n.importance >= 4).slice(-10);
        let recentNews = World.news.filter(n => n.daysOld <= 30 && n.importance < 4).slice(-5);
        
        if (historicalNews.length > 0 || recentNews.length > 0) {
            worldContextString += "[СВЕЖИЕ СЛУХИ И ИСТОРИЯ (Используй в диалогах NPC)]\n";
            historicalNews.forEach(n => worldContextString += `- (${n.daysOld} дн. назад): ${n.text}\n`);
            recentNews.forEach(n => worldContextString += `- Слух (${n.daysOld} дн. назад): ${n.text}\n`);
            worldContextString += "\n";
        }
        worldContextString += "==================================================\n";
    }
...
ИНВЕНТАРЬ (ПОЛНЫЙ):
${JSON.stringify(player.container_backpack ? ContainerRegistry.get(player.container_backpack).items.map(id => { let it = ItemRegistry.get(id); return it ? { instance_id: it.id, prototype_id: it.prototype_id, name: it.custom_props.name, quantity: it.stack_size } : null; }).filter(Boolean) : {}, null, 2)}

ЭКИПИРОВКА (ПО СЛОТАМ):
${JSON.stringify(player.container_equipment ? ContainerRegistry.get(player.container_equipment).items.map(id => { let it = ItemRegistry.get(id); return it ? { slot: it.slot_index, instance_id: it.id, prototype_id: it.prototype_id, name: it.custom_props.name } : null; }).filter(Boolean) : {}, null, 2)}
...
function updateWorldChroniclesDisplay() {
    ...
    // --- ГЕНЕРАЦИЯ НОВОГО ИНТЕРФЕЙСА ФИЛЬТРОВ ---
        ...
        filterContainer.outerHTML = `
            <div class="chronicle-ui-container" id="chronicle-ui-container">
                <div class="chronicle-filter-row">
                    <button class="c-filter-btn ${currentChronicleFilter === 'all' ? 'active' : ''}" data-filter="all">Все</button>
                ...
                <div class="chronicle-filter-row">
                    <button class="c-filter-btn ${currentChronicleTimeFilter === 7 ? 'active' : ''}" data-time="7">7 дн</button>
                    <button class="c-filter-btn ${currentChronicleTimeFilter === 30 ? 'active' : ''}" data-time="30">30 дн</button>
                    <button class="c-filter-btn ${currentChronicleTimeFilter === 360 ? 'active' : ''}" data-time="360">1 Год</button>
                    <button class="c-filter-btn ${currentChronicleTimeFilter === 99999 ? 'active' : ''}" data-time="99999">Вся история</button>
                </div>
            </div>
        `;
    ...
    // Формируем единый список для рендера
    currentChronicleList = [];

    // Добавляем глобальные события (они всегда сверху)
    events.sort((a, b) => a.name.localeCompare(b.name)).forEach(ev => {
        currentChronicleList.push({ type: 'nexus', data: ev });
    });

    // Добавляем слухи (сортируем по свежести: новые сверху)
    ...
    if (currentChronicleList.length === 0) {
        listEl.innerHTML = `<li style="color:#7f8c8d; padding:10px; text-align:center;">В этой категории за выбранный период нет записей...</li>`;
        return;
    }
...
function renderChroniclePage(page) {
    ...
    // ПОЛНОСТЬЮ СТИРАЕМ ПРОШЛЫЙ ЧАНК (Оптимизация DOM)
    listEl.innerHTML = ''; 
    ...
    // ДОБАВЛЯЕМ ПАГИНАЦИЮ (Кнопки Вперед/Назад)
    ...
    // Возвращаем скролл наверх при переключении страницы
    listEl.scrollTop = 0;
}
            ...
            case 'addItem':
                if (args.aiIdentifier && args.name) {
                    ...
                    if (aiId.toLowerCase() === 'gold') {
                        player.stats.gold = Math.max(0, (player.stats.gold || 0) + quantity);
                        ...
                        const backpack = ContainerRegistry.get(player.container_backpack);
                        let existingItemId = backpack.items.find(id => {
                        ...
                        if (existingItemId) {
                            ...
                            if (backpack.items.length >= player.inventoryCapacity) {
                                ...
                                CoreInventorySystem.createItem(aiId, quantity, player.container_backpack, customProps);
            ...
            case 'removeItem':
                if (args.aiIdentifier) {
                    ...
                    let itemKey = backpack.items.find(id => {
                        let it = ItemRegistry.get(id);
                        return it && (it.prototype_id.toLowerCase() === searchTerm || (it.custom_props.name && it.custom_props.name.toLowerCase() === searchTerm));
                    });
                    if (itemKey) {
            ...
            case 'startIntrigue':
                if (args.id && args.type && args.initiator && args.target) {
                    ...
                    World.intrigues.push({
                        ...
                        progress: 0, requiredProgress: args.requiredProgress || 60, progressPerDay: args.progressPerDay || 5, discoveryChance: args.discoveryChance || 3,
...
case 'setEntityBinding':
            ...
            case 'createContainer':
                if (args.type && args.ownerId) {
                    const contId = CoreInventorySystem.createContainer(args.type, args.ownerId, args.maxWeight || 100, args.maxSlots || 30, args.location);
                ...
                break;
                                                case 'initiateTrade':
                if (args.targetId && args.offerItemIds && args.requestItemIds) {
                    if (!player.visibleEntities[args.targetId]) {
                        feedback = `[ERROR] Торговец не находится в зоне видимости.`;
                        break;
                    }
                    const npc = World.npcs[args.targetId];
                    if (!npc || !npc.inventory_id) {
                        feedback = `[ERROR] Торговец не найден или не имеет инвентаря.`;
                        break;
                    }
                    const targetCont = npc.inventory_id;
                    const tgtContObj = ContainerRegistry.get(targetCont);
                    
                    let regionId = Object.keys(World.regions).find(rId => player.location.toLowerCase().includes(World.regions[rId].name.toLowerCase()));

                    args.requestItemIds.forEach(req => {
                        let existing = tgtContObj.items.find(id => ItemRegistry.get(id)?.prototype_id === req.prototype_id);
                        let calcPrice = EconomySim.calculatePrice(req.prototype_id, regionId, true);
                        if (!existing) {
                            CoreInventorySystem.createItem(req.prototype_id || "gold", req.quantity || 1, targetCont, { name: req.name || "Товар", value: calcPrice });
                        } else {
                            let it = ItemRegistry.get(existing);
                            if (it.stack_size < req.quantity) it.stack_size = req.quantity;
                            it.custom_props.value = calcPrice;
                        }
                    });

                    const tradeId = TradeSystem.initiateTrade("player", args.targetId, player.container_backpack, targetCont);
                    let success = true;
                    args.offerItemIds.forEach(itemId => {
                        if (!TradeSystem.addItemToTrade(tradeId, itemId, "offer")) success = false;
                    });
                    
                    args.requestItemIds.forEach(req => {
                        const foundId = tgtContObj.items.find(id => {
                            let it = ItemRegistry.get(id);
                            return it && it.prototype_id === req.prototype_id && it.state === "idle";
                        });
                        if (foundId) {
                            if (!TradeSystem.addItemToTrade(tradeId, foundId, "request")) success = false;
                        } else {
                            success = false;
                        }
                    });
                    ...
                    if (success) {
                        ...
                        feedback = `[ТОРГОВЛЯ] Инициирована сделка. Цены скорректированы EconomySim. Спросите игрока: "Подтверждаете сделку?" и при согласии вызовите confirmTrade.`;
                    } else {
                        TradeSystem._rollbackTrade(tradeId, "Invalid items");
                        feedback = `[ERROR] Не удалось инициировать сделку. Проверьте instance_id предметов.`;
                    }
                } else {
                    feedback = `[ERROR] 'initiateTrade' требует 'targetId', 'offerItemIds' и 'requestItemIds'.`;
                }
                break; else {
                    feedback = `[ERROR] 'initiateTrade' требует 'targetId', 'offerItemIds' и 'requestItemIds'.`;
                }
                break; else {
                    feedback = `[ERROR] 'initiateTrade' требует 'targetId', 'offerItemIds' (массив instance_id) и 'requestItemIds' (массив объектов {prototype_id, quantity}).`;
                }
                break;
            case 'confirmTrade':
            ...
            case 'applyAoEDamage':
                if (args.location && args.damage) {
                    let destroyed = 0;
                    ContainerRegistry.forEach(cont => {
                        if (cont.location === args.location && cont.physical_props) {
                    ...
                    feedback = `[СИСТЕМА] AoE урон (${args.damage}) нанесен по локации ${args.location}. Разрушено контейнеров: ${destroyed}.`;
                }
                break;
            case 'dispelMagicPocket':
                if (args.containerId) {
                    CoreInventorySystem.destroyContainer(args.containerId);
            ...
            case 'fleePackAnimal':
                if (args.containerId) {
                    ...
                    if (cont) {
                        cont.location = "unknown_wilderness";
                        feedback = `[СОБЫТИЕ] Вьючное животное испугалось и сбежало вместе с контейнером ${args.containerId}!`;
                    }
                }
                break;
                if (args.type) {
                ...
                break;
            case 'negotiateTrade':
                if (player.active_trade_id && args.offerItemIds && args.requestItemIds) {
                    const res = TradeSystem.negotiateTrade(player.active_trade_id, args.offerItemIds, args.requestItemIds);
                    feedback = res ? `[ТОРГОВЛЯ] Условия сделки изменены. Ожидание подтверждения.` : `[ERROR] Ошибка изменения сделки.`;
                }
                break;
                if (player.active_trade_id) {
                    const res = TradeSystem.confirmTrade(player.active_trade_id);
                    if (res.success) {
                        feedback = `[ТОРГОВЛЯ] Сделка успешно завершена. Предметы перемещены.`;
                        player.active_trade_id = null;
                        updateInventoryDisplay();
                    } else {
                        feedback = `[ERROR] Ошибка сделки: ${res.error}`;
                        player.active_trade_id = null;
                    }
                } else {
            ...
            case 'negotiateTrade':
                ...
                break;
                if (args.tradeId) {
                    const res = TradeSystem.confirmTrade(args.tradeId);
                    feedback = res.success ? `[ТОРГОВЛЯ] Сделка успешно завершена.` : `[ERROR] Ошибка сделки: ${res.error}`;
                }
                break;
            ...
            case 'updateItemStat':
                if (args.aiIdentifier && args.stat && args.change !== undefined) {
                    ...
                    if (item) {
                        ...
                        item.durability = (item.durability || 100) + change;
            ...
            case 'gmPurchaseGoods':
                if (typeof World !== 'undefined' && World) {
                    if (!World.gmInterventionHistory) World.gmInterventionHistory = [];
                    let facBuy = World.factions[args.factionId];
                    let regBuy = World.regions[args.regionId];
                    let goodBuy = args.goodType;
                    let qtyBuy = parseInt(args.quantity) || 0;
                    if (!facBuy || !regBuy || qtyBuy <= 0) { feedback = `[ERROR] gmPurchaseGoods: Неверные аргументы.`; break; }
            ...
            case 'gmSellGoods':
                if (typeof World !== 'undefined' && World) {
                    if (!World.gmInterventionHistory) World.gmInterventionHistory = [];
                    let facSell = World.factions[args.factionId];
                    let regSell = World.regions[args.regionId];
                    let goodSell = args.goodType;
            ...
            case 'gmInvestInFacility':
                if (typeof World !== 'undefined' && World) {
                    if (!World.gmInterventionHistory) World.gmInterventionHistory = [];
                    let facInv = World.factions[args.factionId];
                    let regInv = World.regions[args.regionId];
                    if (!facInv || !regInv) { feedback = "[ERROR] gmInvestInFacility: Неверные данные."; break; }
                    ...
                    if (args.action === 'repair') {
                        ...
                        } else feedback = `[ERROR] У ${facInv.name} нет ${cost} з. на улучшение.`;
                    }
                    addCalculationMessage(feedback);
                }
                break;
            ...
            case 'gmModifyTradeSecurity':
                if (typeof World !== 'undefined' && World) {
                    if (!World.gmInterventionHistory) World.gmInterventionHistory = [];
                    let facSec = World.factions[args.factionId];
                    let regSec = World.regions[args.regionId];
                    if (!facSec || !regSec) { feedback = "[ERROR] gmModifyTradeSecurity: Неверные данные."; break; }
                    let change = parseInt(args.securityChange) || 0;
                    
                    if (change > 0) {
                        if (facSec.resources.gold.amount >= 1000 && facSec.resources.manpower.amount >= 100) {
                            facSec.resources.gold.amount -= 1000;
                            facSec.resources.manpower.amount -= 100;
                            regSec.caravanSecurity = (regSec.caravanSecurity || 0) + 1;
                            feedback = `[Безопасность] ${facSec.name} усилила патрули в ${regSec.name}.`;
                            World.gmInterventionHistory.push({ turn: player.stats.turnCount, command: command, args: args, result: "Increased" });
                        } else feedback = `[ERROR] У ${facSec.name} не хватает золота или рекрутов для патрулей.`;
                    } else if (change < 0) {
                        regSec.caravanSecurity = Math.max(0, (regSec.caravanSecurity || 0) - 1);
                        facSec.resources.manpower.amount += 100;
                        feedback = `[Безопасность] ${facSec.name} сняла патрули в ${regSec.name}.`;
                        World.gmInterventionHistory.push({ turn: player.stats.turnCount, command: command, args: args, result: "Decreased" });
                    }
                    addCalculationMessage(feedback);
                }
                break;
            ...
            case 'gmRaiseMilitia':
                if (typeof World !== 'undefined' && World) {
                    if (!World.gmInterventionHistory) World.gmInterventionHistory = [];
                    let facMil = World.factions[args.factionId];
                    let regMil = World.regions[args.regionId];
            ...
            case 'gmFrameForSabotage':
                if (typeof World !== 'undefined' && World) {
                    if (!World.gmInterventionHistory) World.gmInterventionHistory = [];
                    let facSab = World.factions[args.factionId];
                    let targetSab = World.factions[args.targetFactionId];
                    let regSab = World.regions[args.regionId];
                    if (!facSab || !targetSab || !regSab || facSab.resources.gold.amount < 3000) { feedback = "[ERROR] gmFrameForSabotage: Провал саботажа (нужно 3000 з)."; break; }
                    facSab.resources.gold.amount -= 3000;
                    regSab.moneySupply = Math.floor(regSab.moneySupply * 0.8);
                    let ownerSab = World.factions[regSab.owner];
                    if (ownerSab) ownerSab.relations[args.targetFactionId] -= 40;
                    feedback = `[Саботаж] ${facSab.name} устроила диверсию в ${regSab.name}, подставив ${targetSab.name}!`;
                    World.gmInterventionHistory.push({ turn: player.stats.turnCount, command: command, args: args, result: "Success" });
                    addCalculationMessage(feedback);
                }
                break;
            ...
            case 'gmDirectResourceInjection':
                if (typeof World !== 'undefined' && World) {
                    if (!World.gmInterventionHistory) World.gmInterventionHistory = [];
                    if (World.lastDirectInjectionDay === undefined) World.lastDirectInjectionDay = -999;
                    
                    let currentDayAbs = player && player.gameTime ? (player.gameTime.year * 360 + player.gameTime.month * 30 + player.gameTime.day) : 0;
                    
                    if (currentDayAbs - World.lastDirectInjectionDay < 7) {
                        feedback = `[ERROR] gmDirectResourceInjection на кулдауне. Осталось дней: ${7 - (currentDayAbs - World.lastDirectInjectionDay)}.`;
                        break;
                    }

                    let regInj = World.regions[args.regionId];
                    if (!regInj) { feedback = "[ERROR] Регион не найден."; break; }
                    
                    World.lastDirectInjectionDay = currentDayAbs;
                    
                    if (!regInj.resources[args.goodType]) regInj.resources[args.goodType] = {amount:0, quality: 1.0};
                    let qtyInj = parseInt(args.quantity) || 0;
                    regInj.resources[args.goodType].amount += qtyInj;
                    
                    feedback = `[Божественное вмешательство] В ${regInj.name} чудесным образом появилось ${qtyInj} ${args.goodType}.`;
                    World.gmInterventionHistory.push({ turn: player.stats.turnCount, command: command, args: args, result: "Success" });
                    addCalculationMessage(feedback);
                }
                break;
...
async function exitToMainMenu() {
    ...
    const performExit = () => {
        ...
        if (document.activeElement) document.activeElement.blur();
            ...
            if (player && tempPlayer === null && !IS_PRE_SIMULATING) {
            showCustomConfirm(
...
function processRulerDiplomacy() {
    ...
    for (let rId in World.rulers) {
        ...
        // ПАССИВНЫЕ ЭФФЕКТЫ ЛИЧНОСТИ
        if (ruler.personality.cruelty > 70) {
            faction.stability = Math.min(100, faction.stability + 1); // Жестокость подавляет бунты
            // Ухудшает репутацию у других
            for (let otherF in World.factions) {
                if (otherF !== ruler.factionId && World.factions[otherF].relations[ruler.factionId] > -100) {
                    World.factions[otherF].relations[ruler.factionId] -= 1;
                }
            }
        }

        // ИНТЕГРАЦИЯ С NEXUS (Чтение целей ГМ)
        if (player && player.nexusData) {
            let nexusGoal = player.nexusData[`${ruler.factionId}_goal`];
            if (nexusGoal) {
                ruler.gmOverride = nexusGoal.value;
            }
            let nexusHeir = player.nexusData[`${ruler.factionId}_heir`];
            if (nexusHeir && World.rulers[nexusHeir.value]) {
                ruler.heir = nexusHeir.value;
            }
        }

        if (ruler.gmOverride) {
            ruler.currentGoal = { type: "gm_override", targetFactionId: ruler.gmOverride };
            if (ruler.gmOverride.includes("invade_")) {
                let target = ruler.gmOverride.split("invade_")[1];
                if (World.factions[target] && faction.diplomacy[target] !== "war") {
                    faction.diplomacy[target] = "war";
                    World.factions[target].diplomacy[ruler.factionId] = "war";
                    generateWorldNews(`ВОЛЯ БОГОВ: ${ruler.name} внезапно объявляет войну ${World.factions[target].name}!`, "global", 5, 'war');
                }
            }
            continue;
        }
        ...
        if (Math.random() < 0.20) { // 20% шанс принять решение в день
            let targetF = fKeys[Math.floor(Math.random() * fKeys.length)];
            if (targetF === ruler.factionId) continue;

            let targetPower = World.factions[targetF].resources.manpower.amount;

            if (ruler.personality.cruelty > 65 && power > 15000 && ruler.personality.ambition > 60) {
                // МУДРОСТЬ: Избегание невыгодных войн
                if (ruler.personality.wisdom > 70 && targetPower > power * 1.2) {
                    ruler.currentGoal = { type: "plot_secretly", targetFactionId: targetF }; // Мудрый отступит
                } else {
                    ruler.currentGoal = { type: "declare_war", targetFactionId: targetF };
                    if (faction.diplomacy[targetF] !== "war") {
                        faction.diplomacy[targetF] = "war";
                        World.factions[targetF].diplomacy[ruler.factionId] = "war";
                        generateWorldNews(`АМБИЦИИ ПРАВИТЕЛЯ: ${ruler.name} объявляет войну ${World.factions[targetF].name}!`, "global", 5, 'war');
                    }
                }
            } else if (ruler.personality.paranoia > 60 && Math.random() < 0.4) {
                let intrigueTypes = ["assassination", "sabotage", "rebellion", "bribery"];
                let selectedType = intrigueTypes[Math.floor(Math.random() * intrigueTypes.length)];
                
                // Мятеж требует ресурсов
                if (selectedType === 'rebellion') {
                    if (wealth >= 5000) {
                        faction.resources.gold.amount -= 5000;
                    } else {
                        selectedType = 'sabotage'; // Нет денег - делаем саботаж
                    }
                }
                ...
                World.intrigues.push({
                    ...
                    type: selectedType,
                    initiatorFactionId: ruler.factionId,
                    targetFactionId: targetF,
                    targetRulerId: World.factions[targetF].rulerId,
                    ...
                    discoveryChance: 4, isDiscovered: false, actors: [], gmInitiated: false, startDay: player?.stats?.turnCount || 0
                });
            } else if (ruler.personality.diplomacy > 60 && security < 60) {
                if (Math.random() < 0.3 && ruler.heir && World.rulers[World.factions[targetF].rulerId]?.heir) {
                    ruler.currentGoal = { type: "marriage_alliance", targetFactionId: targetF };
                    faction.relations[targetF] = 100;
                    World.factions[targetF].relations[ruler.factionId] = 100;
                    generateWorldNews(`ДИНАСТИЧЕСКИЙ БРАК: Дома ${faction.name} и ${World.factions[targetF].name} объединились узами брака!`, "global", 5, 'misc');
                } else {
                    ruler.currentGoal = { type: "offer_alliance", targetFactionId: targetF };
                    faction.relations[targetF] = Math.min(100, (faction.relations[targetF] || 0) + 30);
                    generateWorldNews(`ДИПЛОМАТИЯ: ${ruler.name} отправляет дары во фракцию ${World.factions[targetF].name}, укрепляя союз.`, "global", 2, 'misc');
                }
            } else if (ruler.personality.stewardship > 60 && wealth < 20000) {
                ruler.currentGoal = { type: "trade_pact", targetFactionId: targetF };
                faction.resources.gold.amount += 5000;
            } else if (ruler.personality.cruelty > 50 && Math.random() < 0.3) {
                ruler.currentGoal = { type: "insult", targetFactionId: targetF };
                faction.relations[targetF] -= 20;
                World.factions[targetF].relations[ruler.factionId] -= 20;
                generateWorldNews(`СКАНДАЛ: ${ruler.name} публично оскорбил посла из ${World.factions[targetF].name}! Отношения ухудшаются.`, "global", 3, 'misc');
            } else if (ruler.personality.paranoia > 50) {
                ruler.currentGoal = { type: "send_spy", targetFactionId: targetF };
                if (!World.factions[targetF].spyNetwork) World.factions[targetF].spyNetwork = 0;
                World.factions[targetF].spyNetwork += 1; // Внедрение шпиона (снижает защиту)
                generateWorldNews(`СЛУХИ: В землях ${World.factions[targetF].name} замечены чужие шпионы. Защита от заговоров ослаблена.`, "global", 2, 'misc');
            }
        }
    }
}
...
function processIntrigues() {
    ...
    for (let i = World.intrigues.length - 1; i >= 0; i--) {
        ...
        // ВЛИЯНИЕ АГЕНТОВ И ШПИОНСКИХ СЕТЕЙ НА СКОРОСТЬ
        let actorBonus = (intr.actors && intr.actors.length > 0) ? intr.actors.length * 2 : 0;
        let spyNetworkBonus = World.factions[intr.targetFactionId]?.spyNetwork || 0;
        intr.progress += intr.progressPerDay + actorBonus + spyNetworkBonus;
        
        // Шпионская сеть также снижает шанс раскрытия
        let actualDiscoveryChance = Math.max(1, intr.discoveryChance - Math.floor(spyNetworkBonus / 2));

        if (!intr.isDiscovered && Math.random() * 100 < actualDiscoveryChance) {
        ...
        if (intr.progress >= intr.requiredProgress) {
            if (intr.type === "assassination" && intr.targetRulerId && World.rulers[intr.targetRulerId]) {
                ...
                World.factions[intr.targetFactionId].stability -= 25;
                // Сжигание складов и снижение урожая
                let targetRes = World.factions[intr.targetFactionId].resources;
                if (targetRes.gold) targetRes.gold.amount = Math.floor(targetRes.gold.amount * 0.7);
                for (let rId in World.regions) {
                    if (World.regions[rId].owner === intr.targetFactionId) {
                        if (World.regions[rId].resources.wheat) consumeBatch(World.regions[rId].resources, 'wheat', Math.floor(World.regions[rId].resources.wheat.amount * 0.5));
                        if (World.regions[rId].resources.wood) consumeBatch(World.regions[rId].resources, 'wood', Math.floor(World.regions[rId].resources.wood.amount * 0.5));
                    }
                }
                generateWorldNews(`ДИВЕРСИЯ: Экономика ${World.factions[intr.targetFactionId]?.name} пострадала от саботажников. Сгорели склады и урожай!`, "global", 3, 'disaster');
            } else if (intr.type === "rebellion") {
                World.factions[intr.targetFactionId].stability -= 45;
                // Свержение правителя
                if (intr.targetRulerId && World.rulers[intr.targetRulerId]) {
                    World.rulers[intr.targetRulerId].health = 0;
                    World.rulers[intr.targetRulerId].stats.hp = 0;
                }
                generateWorldNews(`МЯТЕЖ: В землях ${World.factions[intr.targetFactionId]?.name} вспыхнуло восстание! Правитель свергнут!`, "global", 5, 'war');
                checkRulerDeaths();
            } else if (intr.type === "bribery") {
                World.factions[intr.targetFactionId].stability -= 15;
    ...
    for (let rId in World.rulers) {
        ...
        if (r.alive && (r.health <= 0 || r.stats.hp <= 0)) {
            ...
            if (r.heir && World.rulers[r.heir]) {
                ...
                World.rulers[newHeirId] = createRulerForFaction(newHeirId, World.factions[r.factionId], player?.era || 'rebirth', true);
                heir.heir = newHeirId;
...
function createRulerForFaction(id, faction, era, isHeir = false) {
    ...
    return {
        ...
        relationships: {}, memory: [], inventory: { gold: 10000, items: {} },
        economy: { skillLevel: 10, isEmployed: true, workplaceId: null, dailyWage: 500, savings: 50000 },
        plotArmor: true, travelDestination: null, travelHoursLeft: 0, isHostile: false, xpReward: 1000
    };
}
...
function processRulerDiplomacy() {
    ...
    for (let rId in World.rulers) {
        ...
        if (player && player.nexusData) {
            ...
            // ИНТЕГРАЦИЯ С NEXUS (Смена наследника)
            let nexusHeir = player.nexusData[`${ruler.factionId}_heir`];
            if (nexusHeir && World.rulers[nexusHeir.value]) {
                ruler.heir = nexusHeir.value;
            }
        ...
        if (Math.random() < 0.20) { // 20% шанс принять решение в день
            ...
            if (ruler.personality.cruelty > 65 && power > 15000 && ruler.personality.ambition > 60) {
                ...
                ruler.currentGoal = { type: "send_spy", targetFactionId: targetF };
                generateWorldNews(`СЛУХИ: В землях ${World.factions[targetF].name} пойманы шпионы. Чьи они - неизвестно.`, "global", 2, 'misc');
            }
        }
    }
}
