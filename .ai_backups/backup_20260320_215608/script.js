...
async function populateLoadGameScreen() {
    ...
    if (!manualList || !autoList) {
        console.error("Критическая ошибка: Элементы списка сохранений не найдены в DOM!");
        return;
    }
    ...
    if (window.electronAPI && window.electronAPI.isElectron) {
        try {
            ...
            for (const save of fsSaves) {
                allSavesMap.set(`${save.slotType}_${save.slotId}`, save);
            }
        } catch (e) { console.error(e); }
    }

    try {
        const lsSaves = getAllSavesFromLocalStorage();
        [...(lsSaves.manual || []), ...(lsSaves.auto || [])].forEach(save => {
            const key = `${save.slotType}_${save.slotId}`;
            if (!allSavesMap.has(key)) allSavesMap.set(key, save);
        });
    } catch (e) { console.error(e); }
    
    const finalSaves = Array.from(allSavesMap.values());

    if (finalSaves.length === 0) {
        manualList.innerHTML = `<li>${t('loadGame.noManualSaves')}</li>`;
        autoList.innerHTML = `<li>${t('loadGame.noAutoSaves')}</li>`;
    } else {
        finalSaves.filter(s => s.slotType === 'manual').sort((a,b) => a.slotId - b.slotId).forEach(s => manualList.appendChild(createSaveListItem(s)));
        finalSaves.filter(s => s.slotType === 'auto').sort((a,b) => new Date(b.timestamp) - new Date(a.timestamp)).forEach(s => autoList.appendChild(createSaveListItem(s)));
    }
    updateDynamicUIText();
}
...
function applyAudioSettings() {
    ...
    // Музыка (основной плеер и меню)
    if (audioPlayer) audioPlayer.volume = audioSettings.music * master;
    if (menuMusicPlayer) menuMusicPlayer.volume = 0.3 * audioSettings.music * master;

    // Эффекты интерфейса (наведение и клик)
    if (typeof hoverSfx !== 'undefined') hoverSfx.volume = 0.15 * audioSettings.ui * master;
    if (typeof clickSfx !== 'undefined') clickSfx.volume = 0.3 * audioSettings.ui * master;
}
...
function t(key, variables = null, fallback = null) {
 ...
 function updateDynamicUIText() {
    ...
    const setElText = (id, key, vars = null) => {
        const el = document.getElementById(id);
        if (el) el.textContent = t(key, vars);
    };

    const setElHtml = (id, key, vars = null) => {
        const el = document.getElementById(id);
        if (el) el.innerHTML = t(key, vars);
    };

    // Заголовок инвентаря
    const invTitle = document.querySelector('.inventory .panel-toggle > span:first-child');
    if (invTitle) {
        invTitle.innerHTML = t('gameInterface.inventoryPanel.title', {
    ...
    // Общие заголовки панелей
    const selectors = {
        '.character-sheet .panel-toggle > span:first-child': 'gameInterface.characterPanel.title',
        '.quests .panel-toggle > span:first-child': 'gameInterface.questPanel.title',
        '.skills-panel .panel-toggle > span:first-child': 'gameInterface.skillsPanel.title',
        '.map-panel .panel-toggle > span:first-child': 'gameInterface.mapPanel.title',
        '.environment-panel .panel-toggle > span:first-child': 'gameInterface.environmentPanel.title',
        '.calculation-log-panel .panel-toggle > span:first-child': 'gameInterface.calcLogPanel.title'
    };

    for (const [sel, key] of Object.entries(selectors)) {
    ...
    if (gameTitleEl) {
        gameTitleEl.textContent = player ? `${t('appName')} | ${player.name}` : t('appName');
    }

    // Текст пустых списков
    if (calculationLog) {
        const emptyCalc = calculationLog.querySelector('p[data-i18n="gameInterface.calcLogPanel.empty"]');
        if (emptyCalc) emptyCalc.textContent = t('gameInterface.calcLogPanel.empty');
    }
    ...
    // Плейсхолдеры создания персонажа
    const racePh = document.querySelector('#char-race-select option[value=""]');
    if (racePh) racePh.textContent = t('characterCreation.racePlaceholder');
    const classPh = document.querySelector('#char-class-select option[value=""]');
    if (classPh) classPh.textContent = t('characterCreation.classPlaceholder');

    // Кнопка музыки
    if (toggleMusicButton) {
        const isPlaying = !audioPlayer?.paused;
        toggleMusicButton.title = t(isPlaying ? 'gameInterface.toggleMusicButtonTitlePause' : 'gameInterface.toggleMusicButtonTitlePlay');
    }

    // Кнопка TTS
    if (toggleTTSButton) {
        toggleTTSButton.title = t(isTTSEnabled ? 'gameInterface.toggleTTSButtonTitlePause' : 'gameInterface.toggleTTSButtonTitlePlay');
    }
}
...
function initSettingsUI() {
    ...
    const switchProviderView = (provider) => {
        ...
        if(modelLabel) {
            ...
            const labelBase = t('settingsMenu.modelIdLabel', null, 'ID Модели');
            modelLabel.textContent = `${labelBase} (${providerName}):`;
        }
    };
...
function setupEventListeners() {
    ...
    // Универсальный обработчик для всех кнопок "Назад" с атрибутом data-target
    ...
    window.switchSettingsTab = (event, tabId) => {
    ...
    // Показ/скрытие API ключа
    const toggleKeyBtn = document.getElementById('toggle-key-view');
    ...
    if (toggleKeyBtn && keyInput) {
        toggleKeyBtn.onclick = (e) => {
            ...
            toggleKeyBtn.innerHTML = isPass ? '<i class="fas fa-eye-slash"></i>' : '<i class="fas fa-eye"></i>';
        };
    }

    // Слайдеры звука
    ['master', 'music', 'ui'].forEach(type => {
        const slider = document.getElementById(`slider-${type}-vol`);
        const display = document.getElementById(`val-${type}-vol`);
        if (slider) {
            slider.oninput = (e) => {
                const val = e.target.value;
                if (display) display.textContent = val + '%';
                audioSettings[type] = val / 100;
                applyAudioSettings();
            };
        }
    });

    // Помощник для безопасного добавления события
    ...
    // --- Главное меню ---
    safeListen('new-game-button', 'click', startNewGameSetup);
    safeListen('open-map-button', 'click', () => {
    ...
    safeListen('close-map-button', 'click', () => {
        const modal = document.getElementById('map-modal-overlay');
        if (modal) {
            modal.classList.remove('visible');
            setTimeout(() => modal.style.display = 'none', 300);
    ...
    safeListen('load-game-button', 'click', () => showLoadGameScreen());
    safeListen('main-settings-button', 'click', () => {
        settingsReturnScreen = 'main-menu';
        setActiveScreen('settings-menu');
    });
    safeListen('help-button', 'click', () => setActiveScreen('help-screen'));
    safeListen('community-button', 'click', () => window.open('https://discord.com/invite/kDnTx2HAvT', '_blank'));

    // --- Настройки ---
    safeListen('save-settings-button', 'click', saveSettings);
    safeListen('settings-back-button', 'click', () => setActiveScreen(settingsReturnScreen));

    // --- Игровые элементы ---
    safeListen('roll-dice-button', 'click', handleRollDiceButtonClick);
    safeListen('char-era-select', 'change', updateEraDescription);
    safeListen('char-race-select', 'change', handleRaceOrClassChange);
    safeListen('char-class-select', 'change', handleRaceOrClassChange);
    safeListen('start-game-button', 'click', finalizeCharacterCreation);
    safeListen('quick-start-button', 'click', handleQuickStart);

    document.querySelectorAll('.stat-button').forEach(btn => btn.addEventListener('click', handleStatChange));

    safeListen('narrator-prev', 'click', () => showNarrator(currentNarratorIndex - 1));
    safeListen('narrator-next', 'click', () => showNarrator(currentNarratorIndex + 1));
    safeListen('confirm-narrator-button', 'click', startGameWithNarrator);

    safeListen('in-game-menu-button', 'click', openInGameMenu);
    safeListen('close-in-game-menu-button', 'click', closeInGameMenu);
    safeListen('menu-overlay', 'click', closeInGameMenu);
    safeListen('in-game-save-button', 'click', async () => {
        await promptManualSave();
        closeInGameMenu();
    });
    safeListen('in-game-settings-button', 'click', openSettingsFromGame);
    safeListen('in-game-exit-button', 'click', () => exitToMainMenu());

    document.querySelectorAll('.stat-increase-button').forEach(btn => btn.addEventListener('click', handleStatIncrease));

    safeListen('send-button', 'click', handleUserInput);
    ...
    safeListen('voice-input-button', 'click', toggleVoiceRecognition);
    ...
    if (inventoryTabsContainer) {
        inventoryTabsContainer.addEventListener('click', (event) => {
            if (event.target.classList.contains('tab-button')) {
                ...
                currentInventoryFilter = event.target.dataset.category;
                updateInventoryDisplay();
            }
        });
    }
    ...
    bodySlots.forEach(slot => {
        const el = document.getElementById(`equipment-slot-${slot}`);
        if (el) {
            el.onclick = () => {
                if (el.classList.contains('equipped')) unequipItem(slot);
            };
...
function handleQuickStart() {
    ...
    // Инициализируем базовые статы
    handleRaceOrClassChange();

    charNameInput.value = names[Math.floor(Math.random() * names.length)] + " " + (Math.floor(Math.random() * 900) + 100);
    charDescInput.value = "Авантюрист, прибывший из старой деревни на севере.";

    // Распределяем 10 очков случайно
    const statKeys = ['str', 'dex', 'int', 'con', 'cha', 'res'];
    while (availableStatPoints > 0) {
        const randomStat = statKeys[Math.floor(Math.random() * statKeys.length)];
        currentCreationStats[randomStat]++;
...
function handleUserInput() {
    ...
    // --- СИСТЕМА ТРАВМ И БЕЗУМИЯ ---
    let traumaInstruction = "";
    const currentHP = player.stats.hp;
    const resilience = player.stats.res || 10;

    // Инициализация КД, если его нет
    if (player.stats.traumaCooldown === undefined) player.stats.traumaCooldown = 0;

    // Уменьшаем КД каждый ход
    if (player.stats.traumaCooldown > 0) {
        player.stats.traumaCooldown--;
        console.log(`[Trauma System] КД активен: осталось ${player.stats.traumaCooldown} ходов.`);
    }

    // Проверка срабатывает только при КД = 0
    if (player.stats.traumaCooldown === 0 && currentHP > 0 && currentHP <= 15) {
        let baseChance = currentHP <= 5 ? 70 : 40;
        let finalChance = Math.max(5, baseChance - (resilience - 10) * 2.5);
        
        if (Math.random() * 100 < finalChance) {
            // Устанавливаем КД от 2 до 5 ходов
            player.stats.traumaCooldown = Math.floor(Math.random() * 4) + 2;
            console.log(`[Trauma System] Травма! Шанс был ${finalChance.toFixed(1)}%. КД установлен на ${player.stats.traumaCooldown} ходов.`);
            
            traumaInstruction = `\n\n[SYSTEM CRITICAL: Персонаж находится на грани смерти и получил ТЯЖЕЛУЮ ТРАВМУ (физическую или ментальную). Опиши это событие в нарративе, свяжи его с текущим контекстом и ОБЯЗАТЕЛЬНО наложи соответствующий дебафф командой addStatusEffect. Травма должна быть суровой (например: перелом, контузия, психоз). Следующая проверка на травму возможна через ${player.stats.traumaCooldown} ходов.]`;
        }
    }

    const messageWithTrauma = finalMessage + traumaInstruction;
    sendApiRequest(messageWithTrauma, false, false, expiredEffectsForGM, false);
...
function handleUserInput() {
    ...
    if (currentHP > 0 && currentHP <= 15) {
        let baseChance = currentHP <= 5 ? 70 : 40;
        // Стойкость снижает шанс: каждые 2 очка выше 10 снижают шанс на 5%
        let finalChance = Math.max(5, baseChance - (resilience - 10) * 2.5);
        
        if (Math.random() * 100 < finalChance) {
            console.log(`[Trauma System] Шанс ${finalChance.toFixed(1)}%. Бросок неудачен. Травма активирована!`);
            traumaInstruction = `\n\n[SYSTEM CRITICAL: Персонаж находится на грани смерти и получил ТЯЖЕЛУЮ ТРАВМУ (физическую или ментальную). Опиши это событие в нарративе, свяжи его с текущим контекстом и ОБЯЗАТЕЛЬНО наложи соответствующий дебафф командой addStatusEffect. Травма должна быть суровой (например: перелом, контузия, психоз).]`;
        }
    }
    const messageWithTrauma = finalMessage + traumaInstruction;
    sendApiRequest(messageWithTrauma, false, false, expiredEffectsForGM, false);
...
async function sendApiRequest(promptTextForAI, isInitialPrompt = false, isDiceRollResponse = false, expiredEffects = [], isSummarizationRequest = false, skipLogic = false) {
    ...
    // Флаг для отслеживания успеха фазы логики
    ...
    try {
        ...
        addLogMessage("Произошла ошибка генерации. Сервер не ответил или вернул неверный формат.", "system-message");
        
        // Повтор зависит от того, успел ли отработать Счетовод
        showRetryButton(() => sendApiRequest(lastUserMessageForRetry, isInitialPrompt, isDiceRollResponse, [], false, logicPhaseCompleted));
...
async function prepareLogicPrompt(expiredEffects, promptTextForAI, isInitial = false) {
    try {
        ...
        const snapshot = buildFullPlayerSnapshot() + `\nТЕКУЩАЯ КАРТА МИРА (Координаты): ${mapCoordsString}`;
...
document.addEventListener('DOMContentLoaded', () => {
    // Загрузка звука перед стартом
    const savedAudio = localStorage.getItem('audioSettings');
    if (savedAudio) {
        audioSettings = JSON.parse(savedAudio);
        ['master', 'music', 'ui'].forEach(type => {
            const s = document.getElementById(`slider-${type}-vol`);
            const d = document.getElementById(`val-${type}-vol`);
            if (s) s.value = audioSettings[type] * 100;
            if (d) d.textContent = (audioSettings[type] * 100).toFixed(0) + '%';
        });
    }
    applyAudioSettings();
    initializeApp();
