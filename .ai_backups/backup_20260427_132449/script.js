...
function simulateOneDay() {
    console.log("[WorldSim] Симуляция нового дня (Глубокая причинно-следственная связь)...");
    ...
    for (let fId of fKeys) {
        ...
        // СБОР АРМИИ (Требует Оружие, Еду и Людей из конкретного региона)
        if (atWarWith) {
            let homeRegionId = Object.keys(World.regions).find(r => World.regions[r].owner === fId && World.regions[r].resources.weapons?.amount > 100);
            if (homeRegionId && f.resources.manpower.amount > 500) {
                ...
                if (targetRegionId && !alreadyAttacking) {
                    let weaponsToTake = Math.min(200, homeRegion.resources.weapons.amount);
                    let foodToTake = Math.min(500, homeRegion.resources.bread?.amount || 0);
                    ...
                    f.resources.manpower.amount -= 500;
                    
                    let armyMorale = (weaponsToTake < 100 || foodToTake < 200) ? 60 : 100;
                    
                    let armyId = "army_" + Date.now() + Math.floor(Math.random()*1000);
                    f.armies.push({ id: armyId, size: 500, morale: armyMorale, location: homeRegionId, destination: targetRegionId, daysToMove: 3, siegeDays: -1 });
                    generateWorldNews(`Снаряженная армия ${f.name} (500 воинов) выступила из ${homeRegion.name} в поход на ${World.regions[targetRegionId].name}.`, homeRegionId, 4, 'war');
                }
            }
        }
