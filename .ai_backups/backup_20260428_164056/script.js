...
const CoreInventorySystem = {
    ...
    moveItem: function(itemId, sourceContainerId, targetContainerId, quantity = null) {
        ...
        // TODO (Phase 2): Реализация разделения стаков (stack splitting), если quantity < item.stack_size
        ...
        if (sourceContainerId && ContainerRegistry.has(sourceContainerId)) {
            ...
            src.items = src.items.filter(id => id !== itemId);
        }

        if (targetContainerId && ContainerRegistry.has(targetContainerId)) {
            ContainerRegistry.get(targetContainerId).items.push(itemId);
            item.container_id = targetContainerId;
        } else {
            item.container_id = null; // Выброшено в пустоту/на землю (потребует создания ground_pile в будущем)
        }
...
async function preSimulateWorldHistory(yearsToSimulate) {
    ...
    return new Promise((resolve) => {
        worldWorker.postMessage({ action: 'preSimulate', years: yearsToSimulate, World, player, globalLocations });
...
function updateWorldSimulation(pulses) {
    if (!World) return;
    worldWorker.postMessage({ action: 'updateTime', pulses, World, player, globalLocations });
}
...
case 'setEntityBinding':
            ...
            case 'createContainer':
                ...
                if (args.targetId && args.offerItemIds && args.requestItemIds) {
                    let targetCont = "temp_trade_cont_" + args.targetId;
                    if (!ContainerRegistry.has(targetCont)) {
                        CoreInventorySystem.createContainer("npc_inventory", args.targetId, 500, 100, { world_coords: null });
