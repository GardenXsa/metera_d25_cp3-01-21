...
// Голод и Бунты (Причина: нет еды или работы)
    ...
    for (let fId of fKeys) {
        ...
        let armyUpkeep = Math.floor(f.resources.manpower.amount * 0.05); // 1 золото на 20 солдат (чтобы экономики не коллапсировали мгновенно)
        ...
        if (f.resources.gold.amount >= totalExpenses) {
            ...
            if (f.stability > 30) f.stability -= 1; // Уменьшенный штраф стабильности
            f.resources.manpower.amount = Math.floor(f.resources.manpower.amount * 0.90); // Быстрое дезертирство: армия тает, пока не станет по карману
        }
