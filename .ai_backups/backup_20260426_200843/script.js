...
function initWorldSimulator() {
    console.log("[WorldSim] Инициализация нового мира...");
    let newWorld = {
    ...
    // Генерация стартовых NPC
    const names = ["Боб", "Грег", "Элиза", "Торбин", "Лиара", "Каэль", "Морган", "Сильвия"];
    const professions = ["Кузнец", "Фермер", "Стражник", "Торговец", "Маг"];
    for(let i=0; i<20; i++) {
        let id = "sim_npc_" + i;
        ...
        newWorld.npcs[id] = {
            id: id,
            name: names[Math.floor(Math.random() * names.length)] + " " + professions[Math.floor(Math.random() * professions.length)],
            type: "npc",
            homeLocation: home,
            currentLocation: home,
            currentActivity: "Спит",
            needs: { hunger: 100, rest: 100, social: 100, safety: 100 },
            personality: { aggression: Math.random()*10, sociability: Math.random()*10, greed: Math.random()*10 },
            inventory: { gold: Math.floor(Math.random()*100) },
            isAlive: true,
...
function simulateOneHour() {
    ...
    for (let id in World.npcs) {
        ...
        if (npc.travelDestination) {
            npc.travelHoursLeft--;
            npc.currentActivity = "В пути в " + World.regions[npc.travelDestination]?.name;
            if (npc.travelHoursLeft <= 0) {
                npc.currentLocation = npc.travelDestination;
                npc.travelDestination = null;
                npc.currentActivity = "Прибыл в город";
            }
            continue;
        }

        // Принятие решений на основе потребностей
        if (npc.needs.hunger < 30) {
            ...
            npc.needs.rest += 40;
        } else if (npc.needs.social < 30 && npc.personality.sociability > 5) {
            npc.currentActivity = "Общается в таверне";
            npc.needs.social += 30;
        } else {
            npc.currentActivity = "Работает";
            npc.inventory.gold += 1;
            npc.needs.rest -= 2;
        }
    }
...
function syncWorldWithPlayer() {
    ...
    // Если игрок находится в локации, где есть симулируемые NPC, добавляем их в visibleEntities
    ...
    if (playerRegion) {
        for (let id in World.npcs) {
            ...
            if (npc.currentLocation === playerRegion && npc.isAlive) {
                // Добавляем в окружение игрока, если его там еще нет
                if (!player.visibleEntities[id]) {
                    player.visibleEntities[id] = {
                        aiIdentifier: id,
                        name: npc.name,
                        type: "npc",
                        description: `Житель мира. Сейчас: ${npc.currentActivity}`,
                    ...
                    // Обновляем статус
                    player.visibleEntities[id].description = `Житель мира. Сейчас: ${npc.currentActivity}`;
                }
            } else if (npc.currentLocation !== playerRegion && player.visibleEntities[id]) {
                // Убираем, если NPC ушел
