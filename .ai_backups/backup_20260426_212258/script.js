...
function initWorldSimulator() {
    console.log("[WorldSim] Инициализация нового мира (Динамическая генерация)...");
    let newWorld = {
    ...
    let factionNames = ["Империя", "Свободные Города", "Горные Кланы", "Дети Леса", "Культ Бездны", "Технократы"];
    let fIndex = 0;

    for (let key in globalLocations) {
        if (key === 'startLocation') continue;
        let loc = globalLocations[key];
        let fName = factionNames[fIndex % factionNames.length];
        let factionId = "faction_" + fIndex;

        if (!newWorld.factions[factionId]) {
            newWorld.factions[factionId] = {
                name: fName,
                resources: { gold: 10000 + Math.floor(Math.random()*5000), food: 5000, manpower: 5000 },
        ...
        newWorld.regions[key] = {
            ...
            resources: { food: 1000, wood: 500, ore: 200, weapons: 100 },
            facilities: { farms: 3 + Math.floor(Math.random()*5), lumbermills: 2, mines: 2, forges: 2 },
            markets: { food: 1, wood: 2, ore: 5, weapons: 20 },
            caravans: []
        };
        newWorld.weather[key] = "Ясно";
        newWorld.animals[key] = { herbivores: 1000, carnivores: 100 };
        fIndex++;
    }
...
function updateWorldSimDebugDisplay() {
    ...
    if (DEBUG_MODE && typeof World !== 'undefined' && World) {
        ...
        // 1. ФРАКЦИИ
        html += '<h4>ФРАКЦИИ</h4><ul>';
        for (let fId in World.factions) {
            let f = World.factions[fId];
            html += `<li><b>${f.name}</b><br>
                     ...
                     <span class="sim-icon">⚖️</span>${Math.floor(f.stability)}%</li>`;
        }
        html += '</ul>';

        // 2. РЕГИОНЫ (Экономика и Экология)
        html += '<h4>РЕГИОНЫ</h4><ul>';
        for (let rId in World.regions) {
            let r = World.regions[rId];
            let an = World.animals[rId] || {herbivores: 0, carnivores: 0};
            html += `<li><b>${r.name}</b> (Поп: ${Math.floor(r.population)})<br>
                     Рынок: 🍖${r.markets.food} 🪵${r.markets.wood} 🪨${r.markets.ore} 🗡️${r.markets.weapons}<br>
                     Звери: 🦌${Math.floor(an.herbivores)} 🐺${Math.floor(an.carnivores)} | Погода: ${World.weather[rId]}</li>`;
        }
        html += '</ul>';

        // 3. АРМИИ И ВОЙНЫ
        html += '<h4>АРМИИ В ПОХОДЕ</h4><ul>';
        ...
        for (let fId in World.factions) {
            World.factions[fId].armies.forEach(a => {
                ...
                html += `<li><b style="color:#e74c3c">${World.factions[fId].name}</b>: ${a.size} ед.<br>(${loc} ➔ ${dest}, ${a.daysToMove} дн.)</li>`;
            });
        }
