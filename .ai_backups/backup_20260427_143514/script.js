...
async function preSimulateWorldHistory(yearsToSimulate) {
    ...
    return new Promise((resolve) => {
        function simulateChunk() {
            ...
            if (loadingText) {
                loadingText.textContent = `Симуляция истории: ${currentYear} / ${yearsToSimulate} лет...`;
            }
            ...
            if (currentDay < totalDays) {
                ...
                if (aliveNpcs.length < 30) {
                    ...
                    if (rKeys.length > 0) {
                        for(let i = aliveNpcs.length; i < 40; i++) {
...
function initWorldSimulator() {
    ...
    let newWorld = {
        ...
        regions: {}, factions: {}, npcs: {}, news: [], animals: {}, weather: {}, rulers: {}, intrigues: [], rulers: {}, intrigues: []
    };
    ...
    for (let key in globalLocations) {
        ...
        for (let resKey in ECONOMY_ITEMS) {
            ...
            regionResources[resKey] = { amount: baseAmount, quality: 1.0 + (Math.random() * 0.4 - 0.2) };
            ...
            if (caravan.hoursLeft <= 0) {
                if (destRegion) {
                    ...
                    for (let good in caravan.goods) {
                        ...
                        // Защита: если ресурса еще нет в регионе, создаем его
                        if (!destRegion.resources[good]) destRegion.resources[good] = { amount: 0, quality: 1.0 };
                        ...
                        let itemName = ECONOMY_ITEMS[good] ? ECONOMY_ITEMS[good].name : good;
...
function simulateOneDay() {
    ...
    for (let rId of rKeys) {
        ...
        if(r.facilities.farms) {
            r.resources.wheat.amount += Math.floor(workersPerSector * (r.facilities.farms.level / 5) * 5 * weatherMod);
            r.resources.cotton.amount += Math.floor(workersPerSector * (r.facilities.farms.level / 15) * weatherMod);
            r.resources.herbs.amount += Math.floor(workersPerSector * (r.facilities.farms.level / 20) * weatherMod);
        }
        if(r.facilities.lumbermills) r.resources.wood.amount += Math.floor(workersPerSector * (r.facilities.lumbermills.level / 10) * weatherMod);
        if(r.facilities.mines) {
            r.resources.iron_ore.amount += Math.floor(workersPerSector * (r.facilities.mines.level / 10));
            r.resources.gold_ore.amount += Math.floor(workersPerSector * (r.facilities.mines.level / 30));
        }
        ...
        for(let fType of foodTypes) {
            ...
            if(r.resources[fType] && r.resources[fType].amount > 0) {
                let eaten = Math.min(r.resources[fType].amount, foodConsumed);
                r.resources[fType].amount -= eaten;
                foodConsumed -= eaten;
            }
        }

        if(r.resources.clothes && r.resources.clothes.amount > 0) {
            let worn = Math.min(r.resources.clothes.amount, clothConsumed);
            r.resources.clothes.amount -= worn;
        }
...
// Голод и Бунты (Причина: нет еды или работы)
        ...
        if (employmentRate < 0.4 && Math.random() < 0.1) {
            ...
            if(r.resources.weapons) r.resources.weapons.amount = Math.max(0, r.resources.weapons.amount - 100);
    ...
    for (let rId of rKeys) {
        ...
        for (let good in ECONOMY_ITEMS) {
            ...
            for (let destId of rKeys) {
                ...
                if (World.factions[r.owner] && World.factions[r.owner].diplomacy[dest.owner] === 'war') continue;
            ...
            if (bestDest) {
                ...
                if (amount > 0) {
                    r.resources[good].amount -= amount;
                    r.moneySupply += cost;
                    r.caravans.push({
                        id: "caravan_" + Date.now() + Math.floor(Math.random()*1000),
                        origin: rId, destination: bestDest, goods: { [good]: amount },
    ...
    for (let fId of fKeys) {
        ...
        f.globalFood = 0; f.globalWeapons = 0;
        for (let rId of rKeys) {
            if (World.regions[rId].owner === fId) {
                f.globalFood += World.regions[rId].resources.bread?.amount || 0;
                f.globalWeapons += World.regions[rId].resources.weapons?.amount || 0;
            }
        }
    }
    ...
    for (let fId of fKeys) {
        ...
        // Стабильность зависит от еды
        if (f.globalFood < 1000) f.stability -= 3;
        else f.stability += 1;
        f.stability = Math.max(0, Math.min(100, f.stability));

        if (f.stability < 20 && Math.random() < 0.1) {
            generateWorldNews(`Государственный переворот! В землях фракции ${f.name} власть рушится из-за нищеты и голода.`, "global", 5, 'war');
            f.stability += 30; // Новая власть временно стабилизирует ситуацию
            f.resources.manpower.amount = Math.floor(f.resources.manpower.amount * 0.5); // Половина армии дезертирует
        }
        ...
        for (let targetF of fKeys) {
            ...
            rel = f.relations[targetF];
                        ...
                        // ОБЪЯВЛЕНИЕ ВОЙНЫ (ИСПРАВЛЕНО: Война - это крайняя мера)
            let canFight = f.globalWeapons > 1000 && f.resources.manpower.amount > 10000;
            if (rel < -80 && currentStatus !== "war" && canFight && f.stability > 40) {
            ...
            // ЗАКЛЮЧЕНИЕ МИРА (ИСПРАВЛЕНО: Адекватное завершение конфликтов)
            else if (currentStatus === "war") {
                // Мир заключается, если армия истощена, стабильность падает, или война идет слишком долго (рандомный шанс мира каждый день войны)
                if (f.resources.manpower.amount < 5000 || targetFaction.resources.manpower.amount < 5000 || f.stability < 30 || Math.random() < 0.03) {
        ...
        let atWarWith = Object.keys(f.diplomacy).find(k => f.diplomacy[k] === "war");
            ...
            let homeRegionId = Object.keys(World.regions).find(r => World.regions[r].owner === fId && World.regions[r].resources.weapons?.amount > 50);
            
            // Армия собирается, если есть хотя бы 100 рекрутов (убрана жесткая заглушка 500)
            if (homeRegionId && f.resources.manpower.amount > 100) {
                ...
                if (targetRegionId && !alreadyAttacking) {
                    ...
                    homeRegion.resources.weapons.amount -= weaponsToTake;
                    if(homeRegion.resources.bread) homeRegion.resources.bread.amount -= foodToTake;
        ...
        for (let i = f.armies.length - 1; i >= 0; i--) {
            ...
            // Осада города
                ...
                if (army.siegeDays === -1) {
                    ...
                    if(targetRegion.resources.bread) targetRegion.resources.bread.amount = Math.floor(targetRegion.resources.bread.amount * 0.8); // Сжигают амбары
    ...
    // Очистка старых новостей (ИСПРАВЛЕНО: История сохраняется)
    for (let i = World.news.length - 1; i >= 0; i--) {
        World.news[i].daysOld++;
        // Удаляем только мелкие слухи (важность < 4) старше года (360 дней).
        // Великие события (важность 4 и 5) остаются в летописи НАВСЕГДА.
        if (World.news[i].importance < 4 && World.news[i].daysOld > 360) {
...
function buildFullPlayerSnapshot() {
    ...
    if (World) {
        ...
        // Передаем ИИ все важные события за последнее время
                // ИСПРАВЛЕНО: Передаем ИИ важнейшие исторические события за все время + недавние слухи
        let historicalNews = World.news.filter(n => n.importance >= 4).slice(-25); // Топ-25 великих событий эпохи
        let recentNews = World.news.filter(n => n.daysOld <= 60 && n.importance < 4).slice(-15); // Последние бытовые слухи
        
        if (historicalNews.length > 0 || recentNews.length > 0) {
            worldContextString += "[ИСТОРИЧЕСКАЯ ЛЕТОПИСЬ И СВЕЖИЕ СЛУХИ]\n";
            historicalNews.forEach(n => worldContextString += `- ИСТОРИЯ (${n.daysOld} дн. назад): ${n.text}\n`);
...
async function startGameWithNarrator() {
    ...
    // --- НОВОВВЕДЕНИЕ: ПРЕД-СИМУЛЯЦИЯ МИРА (20 ЛЕТ) ---
    if (enableWorldSim) {
        await preSimulateWorldHistory(20);
        const loadingText = document.getElementById('loading-text');
...
let currentChronicleTimeFilter = 60;
let currentChronicleList = [];
let currentChronicleRenderIndex = 0;
const CHRONICLE_CHUNK_SIZE = 30;
let isChronicleScrollAttached = false;
...
function updateWorldChroniclesDisplay() {
    ...
    // --- ДОБАВЛЕНИЕ UI ФИЛЬТРА ВРЕМЕНИ ---
    const filterContainer = document.querySelector('.chronicle-filters');
    if (filterContainer && !document.getElementById('chronicle-time-filter')) {
        const timeSelect = document.createElement('select');
        timeSelect.id = 'chronicle-time-filter';
        timeSelect.style.cssText = 'background: rgba(0,0,0,0.5); border: 1px solid rgba(93, 173, 226, 0.4); color: #5dade2; padding: 2px 5px; border-radius: 4px; margin-left: auto; font-size: 0.85em; outline: none; cursor: pointer;';
        timeSelect.innerHTML = `
            <option value="7">За 7 дней</option>
            <option value="14">За 14 дней</option>
            <option value="30">За 30 дней</option>
            <option value="60" selected>За 60 дней</option>
            <option value="99999">За всё время</option>
        `;
        timeSelect.value = currentChronicleTimeFilter;
        timeSelect.onchange = (e) => {
            currentChronicleTimeFilter = parseInt(e.target.value);
            updateWorldChroniclesDisplay();
        };
        filterContainer.appendChild(timeSelect);
    }

    // Привязываем скролл для ленивой загрузки (один раз)
    if (!isChronicleScrollAttached) {
        listEl.addEventListener('scroll', () => {
            if (listEl.scrollTop + listEl.clientHeight >= listEl.scrollHeight - 50) {
                renderChronicleChunk();
            }
        });
        isChronicleScrollAttached = true;
    }
    ...
    // Добавляем слухи (сортируем по свежести)
    ...
    // Сбрасываем индекс и рендерим первый чанк
    currentChronicleRenderIndex = 0;
    renderChronicleChunk();
}

function renderChronicleChunk() {
    const listEl = document.getElementById('world-chronicles-list');
    if (!listEl || currentChronicleRenderIndex >= currentChronicleList.length) return;

    const chunk = currentChronicleList.slice(currentChronicleRenderIndex, currentChronicleRenderIndex + CHRONICLE_CHUNK_SIZE);
    ...
    currentChronicleRenderIndex += CHRONICLE_CHUNK_SIZE;
}
...
function updateTradeJournalDisplay() {
    ...
    const formatPrice = (key, price) => {
        let name = ECONOMY_ITEMS[key] ? ECONOMY_ITEMS[key].name : key;
        return `
    ...
    if (deficitGood) {
        let name = ECONOMY_ITEMS[deficitGood] ? ECONOMY_ITEMS[deficitGood].name : deficitGood;
...
function updateWorldSimDebugDisplay() {
    ...
    if (DEBUG_MODE && typeof World !== 'undefined' && World) {
        ...
        for (let fId in World.factions) {
            ...
            let gold = f.resources.gold ? Math.floor(f.resources.gold.amount) : 0;
            let manpower = f.resources.manpower ? Math.floor(f.resources.manpower.amount) : 0;
            ...
            if (intr.type === "assassination" && intr.targetRulerId && World.rulers[intr.targetRulerId]) {
                ...
                for (let rId in World.regions) {
                    if (World.regions[rId].owner === intr.targetFactionId) {
                        if (World.regions[rId].resources.wheat) World.regions[rId].resources.wheat.amount = Math.floor(World.regions[rId].resources.wheat.amount * 0.5);
                        if (World.regions[rId].resources.wood) World.regions[rId].resources.wood.amount = Math.floor(World.regions[rId].resources.wood.amount * 0.5);
                    }
                }
