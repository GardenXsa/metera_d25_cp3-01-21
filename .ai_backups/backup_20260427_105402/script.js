...
// ======================================================================
// --- БАЗА ДАННЫХ ЭКОНОМИКИ ---
const BASE_ECONOMY_PRICES = {
    // Базовое (старое)
    food: 1, wood: 2, ore: 4, herbs: 6, steel: 12, cloth: 10, weapons: 35, potions: 30, luxury: 80,
    // Сельское хозяйство
    wheat: 1, oats: 1, flour: 3, bread: 5, beer: 8,
    // Горное дело
    iron_ore: 3, coal: 2, copper_ore: 3, gold_nugget: 50,
    // Металлургия
    iron_ingot: 10, copper_ingot: 10, bronze: 15, gold_ingot: 200,
    // Животноводство
    hide: 3, leather: 8, meat: 4
};

// --- ПРОИЗВОДСТВЕННЫЕ ЦЕПОЧКИ ---
const CRAFTING_RECIPES = [
    { facility: 'mills', inputs: { wheat: 2 }, outputs: { flour: 1 } },
    { facility: 'bakeries', inputs: { flour: 1 }, outputs: { bread: 2 } },
    { facility: 'breweries', inputs: { wheat: 1, herbs: 1 }, outputs: { beer: 1 } },
    { facility: 'smelters', inputs: { iron_ore: 2, coal: 1 }, outputs: { iron_ingot: 1 } },
    { facility: 'smelters', inputs: { copper_ore: 2, coal: 1 }, outputs: { copper_ingot: 1 } },
    { facility: 'forges', inputs: { iron_ingot: 2, wood: 1 }, outputs: { weapons: 1, steel: 1 } },
    { facility: 'tanneries', inputs: { hide: 2 }, outputs: { leather: 1 } },
    { facility: 'weavers', inputs: { herbs: 2 }, outputs: { cloth: 1 } }, // Упрощенно: травы/лен в ткань
    { facility: 'alchemists', inputs: { herbs: 3 }, outputs: { potions: 1 } }
];
    ...
    for (let key in globalLocations) {
        ...
        newWorld.regions[key] = {
            ...
            facilities: {
                ...
                // Новые здания
                mills: { level: Math.floor(Math.random() * 5), durability: 100 },
                bakeries: { level: Math.floor(Math.random() * 5), durability: 100 },
                breweries: { level: Math.floor(Math.random() * 3), durability: 100 },
                smelters: { level: isDwarf ? 10 : Math.floor(Math.random() * 4), durability: 100 },
                tanneries: { level: Math.floor(Math.random() * 4), durability: 100 }
            },

            markets: { food: 1, wood: 2, ore: 5, herbs: 8, steel: 15, cloth: 10, weapons: 50, potions: 40, luxury: 100 },
            caravans: []
        };
        // Динамическая генерация базовых ресурсов для региона
        for (let res in BASE_ECONOMY_PRICES) {
            let baseAmount = Math.floor(Math.random() * 1000);
            if (isElf && (res === 'wood' || res === 'herbs' || res === 'wheat')) baseAmount *= 3;
            if (isDwarf && (res === 'iron_ore' || res === 'coal' || res === 'ore')) baseAmount *= 3;
            newWorld.regions[key].resources[res] = { amount: baseAmount, quality: 1.0 + (Math.random() * 0.4 - 0.2) };
        }
...
function simulateOneDay() {
    ...
    for (let rId of rKeys) {
        ...
        // 1. Базовая добыча сырья (Шахты, Фермы, Лесопилки)
        r.resources.food.amount += Math.floor(workersPerSector * (r.facilities.farms.level / 10) * 2 * weatherMod);
        r.resources.wheat.amount += Math.floor(workersPerSector * (r.facilities.farms.level / 10) * 1.5 * weatherMod);
        ...
        r.resources.iron_ore.amount += Math.floor(workersPerSector * (r.facilities.mines.level / 10));
        r.resources.coal.amount += Math.floor(workersPerSector * (r.facilities.mines.level / 10));
        r.resources.herbs.amount += Math.floor(workersPerSector * (r.facilities.farms.level / 20) * weatherMod);
        r.resources.hide.amount += Math.floor(workersPerSector * (r.facilities.farms.level / 15));
        ...
        // 2. УНИВЕРСАЛЬНЫЙ ДВИЖОК ПРОИЗВОДСТВА (CRAFTING ENGINE)
        for (let recipe of CRAFTING_RECIPES) {
            ...
            // Ищем "узкое горлышко" (bottleneck) по сырью
            ...
            // Если можем произвести хоть что-то
            if (maxCrafts > 0) {
                ...
                // Списываем сырье
                for (let inRes in recipe.inputs) {
                    r.resources[inRes].amount -= maxCrafts * recipe.inputs[inRes];
                }

                // Добавляем готовый продукт
                for (let outRes in recipe.outputs) {
                    let produced = maxCrafts * recipe.outputs[outRes];
                    if (!r.resources[outRes]) r.resources[outRes] = { amount: 0, quality: 1.0 };

                    let currentAmount = r.resources[outRes].amount;
                    let currentQuality = r.resources[outRes].quality;
                    let newTotal = currentAmount + produced;

                    r.resources[outRes].quality = ((currentAmount * currentQuality) + (produced * outputQuality)) / newTotal;
                    r.resources[outRes].amount += produced;
                }
            }
        }
    ...
    if (DEBUG_MODE && typeof World !== 'undefined' && World) {
        ...
        for (let rId in World.regions) {
            ...
            html += `<div class="debug-item">
                     ...
                     <div style="max-height: 60px; overflow-y: auto; border-top: 1px solid #333; margin-top: 5px; padding-top: 5px; font-size: 0.8em;">
                        ${Object.keys(r.resources).filter(k => r.resources[k].amount > 0).map(k => `<span style="display:inline-block; margin-right: 8px; color:#bdc3c7;">${k}: <b style="color:#f1c40f">${Math.floor(r.resources[k].amount)}</b> <i style="color:#7f8c8d">(q:${r.resources[k].quality.toFixed(1)})</i></span>`).join('')}
                     </div>
                     </div>`;
        }
        html += '</div></div>';
