...
function simulateOneDay() {
    ...
    for (let rId of rKeys) {
        ...
        let foodPerCapita = r.resources.food.amount / (r.population || 1);
    ...
    for (let fId of fKeys) {
        ...
        for (let targetF of fKeys) {
            ...
            // ОБЪЯВЛЕНИЕ ВОЙНЫ (Только если есть оружие!)
            if (rel < -50 && currentStatus !== "war" && f.globalWeapons > 500) {
                f.diplomacy[targetF] = "war";
                targetFaction.diplomacy[fId] = "war";
                let reason = f.globalFood < 1000 ? "из-за острой нехватки ресурсов" : "из-за давней вражды";
        ...
        if (atWarWith) {
            let homeRegionId = Object.keys(World.regions).find(r => World.regions[r].owner === fId && World.regions[r].resources.weapons?.amount > 200);
            if (homeRegionId && f.resources.manpower.amount > 500) {
                ...
                if (targetRegionId && !alreadyAttacking) {
                    // Списываем ресурсы на армию
                    homeRegion.resources.weapons.amount -= 200;
                    if(homeRegion.resources.bread) homeRegion.resources.bread.amount -= 500;
                    f.resources.manpower.amount -= 500;
                    
                    let armyId = "army_" + Date.now() + Math.floor(Math.random()*1000);
                    f.armies.push({ id: armyId, size: 500, morale: 100, location: homeRegionId, destination: targetRegionId, daysToMove: 3, siegeDays: -1 });
