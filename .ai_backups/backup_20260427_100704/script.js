...
function initWorldSimulator() {
    ...
    for (let key in globalLocations) {
        ...
        newWorld.regions[key] = {
            ...
            markets: { food: 1, wood: 2, ore: 5, herbs: 8, steel: 15, cloth: 10, weapons: 50, potions: 40, luxury: 100 },
            caravans: []
        };
...
function simulateOneDay() {
    ...
    for (let rId of rKeys) {
        ...
        for (let fId in r.facilities) {
            let fac = r.facilities[fId];
            if (fac.level > 0 && fac.durability < 50 && r.resources.wood.amount > 5) {
                fac.durability += 20;
                r.resources.wood.amount -= 5;
            }
        }

        // Сталь: требует Руду и Дерево
        let forge = r.facilities.forges;
        let steelCapacity = Math.floor(workersPerSector * (forge.level / 10));
        let oreToSteel = Math.min(r.resources.ore.amount, Math.min(r.resources.wood.amount, steelCapacity));
        
        if (oreToSteel > 0) {
            // Расчет качества: (Сырье + Оборудование) / 2
            let inputQuality = (r.resources.ore.quality + r.resources.wood.quality) / 2;
            let outputQuality = (inputQuality * 0.7 + (forge.durability/100) * 0.3);
            
            r.resources.ore.amount -= oreToSteel;
            r.resources.wood.amount -= Math.floor(oreToSteel * 0.5);
            
            let newSteel = Math.floor(oreToSteel * 0.8);
            // Обновляем среднее качество стали в регионе
            let totalSteel = r.resources.steel.amount + newSteel;
            if (totalSteel > 0) {
                r.resources.steel.quality = ((r.resources.steel.amount * r.resources.steel.quality) + (newSteel * outputQuality)) / totalSteel;
            }
            r.resources.steel.amount += newSteel;
        }
