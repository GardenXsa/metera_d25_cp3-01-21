...
function simulateOneDay() {
    ...
    for (let fId of fKeys) {
        ...
        for (let targetF of fKeys) {
            ...
            if (currentStatus === "war") change -= 2;
            if (currentStatus === "alliance") change += 2;
            
            f.relations[targetF] = Math.max(-100, Math.min(100, rel + change));
            rel = f.relations[targetF];

            if (rel < -80 && currentStatus !== "war") {
        ...
        if (atWarWith && f.resources.gold > 2000 && f.resources.manpower > 500) {
            f.resources.gold -= 2000;
            f.resources.manpower -= 500;
...
async function sendApiRequest(promptTextForAI, isInitialPrompt = false, isDiceRollResponse = false, expiredEffects = [], isSummarizationRequest = false, skipLogic = false) {
    ...
    try {
        ...
        if (enableWorldSim && World && player && !isInitialPrompt && !isSummarizationRequest) {
            if (!World.time.lastEventPulse) World.time.lastEventPulse = player.gameTime.totalPulses;
            // Проверяем, прошло ли 24 часа (288 пульсов) с последнего события
            if (player.gameTime.totalPulses - World.time.lastEventPulse >= 288) {
                await runWorldSimulationTick();
                World.time.lastEventPulse = player.gameTime.totalPulses;
            }
        }
...
async function runWorldSimulationTick() {
    ...
    try {
        ...
        const prompt = `Ты — ДВИЖОК МИРА (WORLD SIMULATOR). Твоя задача: проанализировать сухие цифры текущего состояния экономики, политики и экологии, и сгенерировать ОДНО крупное мировое событие, логично вытекающее из этих данных.\n\nТекущая дата в мире: ${currentDateStr}\n\n${worldSummary}\n\nЛокация игрока сейчас: ${player.location}\n\nПРИКАЗ:\n1. Верни СТРОГО JSON. Никакого текста до или после.\n2. Проанализируй данные. Если где-то 0 еды — опиши голодный бунт. Если идет война — опиши кровавую битву или осаду. Если стабильность фракции низкая — опиши восстание или покушение на лидера.\n3. В массиве actions используй 'nexusDefine', чтобы закрепить это событие в мире (category: 'World_Event', displayType: 'text', value: 'АКТИВНО').\n4. ЕСЛИ событие происходит в той же локации, где сейчас игрок, ты ИМЕЕШЬ ПРАВО выдать ему квест через 'addQuest' (например, помочь подавить бунт или спасти раненых).\n5. В поле 'narrative' напиши ЭПИЧНЫЙ ХУДОЖЕСТВЕННЫЙ ТЕКСТ (2-3 абзаца) в виде глобального слуха, видения или вести от гонцов. Обязательно упомяни текущую дату в тексте. Опиши масштабы, страх людей и последствия.`;
